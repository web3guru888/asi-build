...
class TestRunner {
  ...
  private async initializeToolSystem() {
    ...
    try {
      ...
      this.recordResult('Tool System Initialization', true, Date.now() - startTime, {
        toolsRegistered: initResult.toolsRegistered,
        categories: initResult.categoryCounts,
        errors: initResult.errors
      });

      console.log(`✅ Tool system initialized with ${initResult.toolsRegistered} tools`);
      console.log(`   Categories: ${Object.keys(initResult.categoryCounts).join(', ')}\n`);

    } catch (error) {
      this.recordResult('Tool System Initialization', false, Date.now() - startTime, {}, 
  ...
  private async testToolDiscovery() {
    ...
    try {
      ...
      const expectedTools = ['read', 'write', 'edit', 'bash', 'search', 'delete', 'move', 'list'];
      ...
      this.recordResult('Tool Discovery', false, Date.now() - startTime, {}, 
        (error as Error).message);
      console.error('❌ Tool discovery test failed:', (error as Error).message, '\n');
    }
  }
  ...
  private async testToolExecution() {
    ...
    try {
      ...
      const listResult = await this.toolManager.execute('list', {
        path: '.',
        maxResults: 10
      }, testContext);
      ...
      executionResults.push({ tool: 'list', success: listResult.success, itemsFound: listResult.data?.items?.length || 0 });
      console.log(`   ✅ List tool found ${listResult.data?.items?.length || 0} items`);

      // Test 2: Write tool (create a test file)
      console.log('   Testing write tool...');
      const writeResult = await this.toolManager.execute('write', {
        path: './test-file.txt',
      ...
      executionResults.push({ tool: 'write', success: writeResult.success, bytesWritten: writeResult.data?.bytesWritten });
      console.log(`   ✅ Write tool created file with ${writeResult.data?.bytesWritten || 0} bytes`);

      // Test 3: Read tool (read the test file)
      console.log('   Testing read tool...');
      const readResult = await this.toolManager.execute('read', {
        path: './test-file.txt'
      }, testContext);
      ...
      executionResults.push({ tool: 'read', success: readResult.success, contentLength: readResult.data?.content?.length || 0 });
      console.log(`   ✅ Read tool retrieved ${readResult.data?.content?.length || 0} characters`);

      // Test 4: Search tool (search in current directory)
      console.log('   Testing search tool...');
      const searchResult = await this.toolManager.execute('search', {
        query: 'ASI-Code',
        path: '.',
        filePattern: '*.txt',
        maxResults: 50
      ...
      executionResults.push({ tool: 'search', success: searchResult.success, matchesFound: searchResult.data?.statistics?.totalMatches || 0 });
      console.log(`   ✅ Search tool found ${searchResult.data?.statistics?.totalMatches || 0} matches`);
      ...
      this.recordResult('Tool Execution', false, Date.now() - startTime, {}, 
        (error as Error).message);
  ...
  private async testToolValidation() {
    ...
    try {
      ...
      if (this.toolManager.validateToolParameters) {
        ...
        console.log(`   ✅ Valid parameters: ${validResult.isValid ? 'PASS' : 'FAIL'}`);
      }
      ...
      if (this.toolManager.validateToolParameters) {
        ...
        validationResults.push({ test: 'invalid_params', result: invalidResult });
        console.log(`   ✅ Invalid parameters detected: ${!invalidResult.isValid ? 'PASS' : 'FAIL'}`);
      }
      ...
      if (this.toolManager.validateToolParameters) {
        ...
        console.log(`   ✅ Type validation: ${!typeResult.isValid ? 'PASS' : 'FAIL'}`);
      }
      ...
      this.recordResult('Tool Validation', false, Date.now() - startTime, {}, 
        (error as Error).message);
      console.error('❌ Tool validation test failed:', (error as Error).message, '\n');
    }
  }
  ...
  private async testBatchExecution() {
    ...
    try {
      ...
      for (const operation of batchOperations) {
        ...
        const result = await this.toolManager.execute(operation.name, operation.parameters, testContext);
        batchResults.push({
      ...
      console.log(`   ✅ Batch execution: ${successCount}/${batchResults.length} operations successful`);
      ...
      this.recordResult('Batch Execution', false, Date.now() - startTime, {}, 
        (error as Error).message);
      console.error('❌ Batch execution test failed:', (error as Error).message, '\n');
    }
  }
  ...
  private async testErrorHandling() {
    ...
    try {
      ...
      try {
        const result = await this.toolManager.execute('nonexistent-tool', {}, testContext);
        errorTests.push({ test: 'nonexistent_tool', handled: !result.success, error: result.error });
        console.log(`   ✅ Non-existent tool error handled: ${!result.success ? 'PASS' : 'FAIL'}`);
      } catch (error) {
        errorTests.push({ test: 'nonexistent_tool', handled: true, error: (error as Error).message });
        console.log(`   ✅ Non-existent tool error handled: PASS`);
      }

      // Test 2: Invalid file path
      console.log('   Testing invalid file path...');
      const invalidPathResult = await this.toolManager.execute('read', {
        path: '/nonexistent/path/to/file.txt'
      }, testContext);
      errorTests.push({ test: 'invalid_path', handled: !invalidPathResult.success, error: invalidPathResult.error });
      console.log(`   ✅ Invalid path error handled: ${!invalidPathResult.success ? 'PASS' : 'FAIL'}`);

      // Test 3: Permission denied (try to write without write permission)
      console.log('   Testing permission denied...');
      const permissionDeniedResult = await this.toolManager.execute('write', {
      ...
      errorTests.push({ test: 'permission_denied', handled: !permissionDeniedResult.success, error: permissionDeniedResult.error });
      console.log(`   ✅ Permission denied handled: ${!permissionDeniedResult.success ? 'PASS' : 'FAIL'}`);
      ...
      this.recordResult('Error Handling', false, Date.now() - startTime, {}, 
        (error as Error).message);
  ...
  private async testPermissionChecking() {
    ...
    try {
      ...
      const contexts = [
        ...
        },
        {
          ...
          context: {
            ...
            userId: 'test-user',
            permissions: ['read_files', 'write_files', 'execute_commands', 'delete_files', 'dangerous_operations'],
            workingDirectory: process.cwd(),
            environment: {},
            metadata: {}
          }
      ...
      for (const { name, context } of contexts) {
        ...
        console.log(`     Write: ${writeResult.success ? 'PASS' : 'FAIL'} (expected: ${name !== 'read_only' ? 'PASS' : 'FAIL'})`);
      }
      ...
      this.recordResult('Permission Checking', false, Date.now() - startTime, {}, 
        (error as Error).message);
      console.error('❌ Permission checking test failed:', (error as Error).message, '\n');
    }
  }
  ...
  private async testMetricsAndHealth() {
    ...
    try {
      ...
      this.recordResult('Metrics and Health', false, Date.now() - startTime, {}, 
        (error as Error).message);
      console.error('❌ Metrics and health monitoring test failed:', (error as Error).message, '\n');
    }
  }

  private recordResult(name: string, success: boolean, duration: number, details: any, error?: string) {
    this.results.push({
      name,
      success,
      duration,
  ...
  private async cleanup() {
    ...
    try {
      ...
      for (const file of testFiles) {
        try {
          await this.toolManager.execute('delete', {
            path: file,
            force: true
          }, {
