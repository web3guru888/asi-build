 ...
 */
export function setupEnhancedToolRoutes(
  app: Hono,
  server: DefaultASIServer
): void {
  ...
  app.get('/api/tools/discover', c => {
    try {
      ...
      if (
        'discoverTools' in toolManager &&
        typeof (toolManager as any).discoverTools === 'function'
      ) {
  ...
  app.get('/api/tools/:name/metrics', c => {
    try {
      ...
      if (
        'getMetrics' in toolManager &&
        typeof (toolManager as any).getMetrics === 'function'
      ) {
        const metrics = (toolManager as any).getMetrics(toolName);

        if (!metrics) {
          return c.json(
            { error: 'Tool not found or no metrics available' },
            404
          );
        }
  ...
  app.get('/api/tools/executions', c => {
    try {
      if (
        'getRunningExecutions' in toolManager &&
        typeof (toolManager as any).getRunningExecutions === 'function'
      ) {
  ...
  app.delete('/api/tools/executions/:id', async c => {
    try {
      ...
      if (
        'cancelExecution' in toolManager &&
        typeof (toolManager as any).cancelExecution === 'function'
      ) {
        const cancelled = await (toolManager as any).cancelExecution(
          executionId
        );
  ...
  app.get('/api/tools/health', async c => {
    try {
      ...
      if (
        'healthCheck' in toolManager &&
        typeof (toolManager as any).healthCheck === 'function'
      ) {
  ...
  app.get('/api/tools/documentation', c => {
    try {
      ...
      return c.json({
        ...
        categories: documentation.reduce(
