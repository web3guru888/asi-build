...
export class Orchestrator
  extends EventEmitter<OrchestrationEvents>
  implements IOrchestrator
{
  ...
  async scaleAgents(targetCount: number, type?: AgentType): Promise<void> {
    ...
    this.logger.info(
      `Scaling complete: ${currentCount} -> ${targetCount} agents`
    );
  }
  ...
  getSystemStatus(): SystemStatus {
    ...
    const activeTasks = Array.from(this.taskRegistry.values()).filter(
      t => t.status === 'in_progress' || t.status === 'assigned'
    );
    const queuedTasks = Array.from(this.taskRegistry.values()).filter(
      t => t.status === 'pending'
    );
  ...
  getMetrics(): OrchestratorMetrics {
    ...
    const agentUtilization =
      agents.length > 0 ? totalUtilization / agents.length : 0;

    // Queue depth
    const queueDepth = Array.from(this.taskRegistry.values()).filter(
      t => t.status === 'pending'
    ).length;
    ...
    return {
      ...
      queueDepth,
      successRate:
        totalTasks > 0 ? (this.completedTasks / totalTasks) * 100 : 100,
  ...
  private isSystemHealthy(): boolean {
    ...
    const activeSupervisors = Array.from(this.supervisors.values()).filter(
