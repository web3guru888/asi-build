...
export abstract class BaseAgent extends EventEmitter implements Agent {
  ...
  async initialize(): Promise<void> {
    this.logger.info(
      `Initializing agent ${this.id} of type ${this.config.type}`
    );
  ...
  canHandle(task: Task): boolean {
    ...
    if (task.constraints?.requiredCapabilities) {
      const hasAllCapabilities = task.constraints.requiredCapabilities.every(
  ...
  protected hasRequiredResources(required: any): boolean {
    ...
    if (
      required.memory &&
      (!available.memory || available.memory < required.memory)
    ) {
      return false;
    }
  ...
  protected async waitForTaskCompletion(timeout: number): Promise<void> {
    ...
    while (this.currentTasks.size > 0) {
      if (Date.now() - startTime > timeout) {
        this.logger.warn(
          `Timeout waiting for task completion in agent ${this.id}`
        );
        break;
      }
