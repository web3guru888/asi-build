...
export class SupervisorAgent extends BaseAgent implements ISupervisorAgent {
  ...
  private readonly taskAssignments: Map<string, string>; // taskId -> agentId
  private readonly decomposedTasks: Map<string, TaskDecomposition>; // parentTaskId -> decomposition
  ...
  async assignTask(task: Task, agentId?: string): Promise<void> {
    this.logger.info(
      `Assigning task ${task.id} to ${agentId || 'best available agent'}`
    );
  ...
  async decomposeTask(task: Task): Promise<TaskDecomposition> {
    ...
    this.logger.info(
      `Task ${task.id} decomposed into ${decomposition.subtasks.length} subtasks`
    );
  ...
  async handleAgentFailure(agentId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to recover agent ${agentId}, replacing:`,
        error
      );
  ...
  private async checkAgentHealth(): Promise<void> {
    for (const agent of this.managedAgents.values()) {
      ...
      if (metrics.performance.successRate < 50 && metrics.tasksCompleted > 10) {
        this.logger.warn(
          `Agent ${agent.id} has low success rate: ${metrics.performance.successRate}%`
        );
      }
    }
  }
