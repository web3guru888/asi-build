...
async function runSimpleTest(): Promise<void> {
  ...
  try {
    ...
    console.log(`   ✅ Found ${registry.findByType('worker').length} worker agents`);
    console.log(`   ✅ Found ${registry.findByCapability('basic-task').length} agents with basic-task capability`);
    console.log(`   ✅ Found ${registry.findAvailable().length} available agents`);
    
    // Test stats
    const stats = registry.getStats();
    console.log(`   ✅ Registry stats: ${stats.totalAgents} total, ${stats.availableAgents} available`);
    ...
    const selectedAgent = loadBalancer.selectAgent(task, registry.findAvailable());
    ...
    console.log('\n📋 Summary:');
    console.log('   ✅ AgentRegistry: Registration, retrieval, and statistics work correctly');
    console.log('   ✅ LoadBalancer: Agent selection and load balancing work correctly');
    console.log('   ✅ Integration: Both components integrate properly for task distribution');
