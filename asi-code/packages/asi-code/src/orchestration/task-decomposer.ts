...
export class TaskDecomposer implements ITaskDecomposer {
  ...
  createExecutionPlan(tasks: Task[], dependencies: TaskDependency[]): ExecutionPlan {
    ...
    for (const taskGroup of executionOrder) {
      currentPhase = {
        ...
        estimatedDuration: Math.max(...taskGroup.map(id => 
          this.estimateDuration(taskMap.get(id)!)
        ))
      };
    ...
    const estimatedDuration = phases.reduce((sum, phase) => 
      sum + phase.estimatedDuration, 0
    );
  ...
  private calculateComplexity(task: Task): number {
    ...
    const operations = task.description.match(/\b(and|then|after|before|while)\b/gi);
   ...
   */
  private inferDataDependencies(tasks: Task[], dependencies: TaskDependency[]): void {
   ...
   */
  private detectResourceConflicts(tasks: Task[], dependencies: TaskDependency[]): void {
    tasks.forEach((task, i) => {
      tasks.slice(i + 1).forEach(otherTask => {
   ...
   * Build dependency graph
   */
  private buildDependencyGraph(dependencies: TaskDependency[]): Map<string, Set<string>> {
   ...
   * Calculate critical path
   */
  private calculateCriticalPath(tasks: Task[], dependencies: TaskDependency[]): string[] {
    const taskMap = new Map(tasks.map(t => [t.id, t]));
    const graph = this.buildDependencyGraph(dependencies);
    const reverseGraph = this.buildReverseGraph(dependencies);
    
    // Find tasks with no dependencies (start nodes)
    const startNodes = tasks.filter(t => 
      !dependencies.some(d => d.to === t.id)
    ).map(t => t.id);
    
    // Find tasks with no dependents (end nodes)
    const endNodes = tasks.filter(t =>
    ...
    const calculateEarliestStart = (nodeId: string): number => {
      ...
      const maxPredecessor = Math.max(...deps.map(depId => {
    ...
    endNodes.forEach(nodeId => {
      const duration = earliestStart.get(nodeId)! + 
    ...
    while (currentNode) {
      ...
      currentNode = predecessors.find(predId => {
   ...
   */
  private buildReverseGraph(dependencies: TaskDependency[]): Map<string, Set<string>> {
    const graph = new Map<string, Set<string>>();
  ...
  private registerDefaultStrategies(): void {
    // Default strategy
    this.decompositionStrategies.set('default', new DefaultDecompositionStrategy());
    ...
    this.decompositionStrategies.set('deploy', new DeployDecompositionStrategy());
    this.decompositionStrategies.set('analysis', new AnalysisDecompositionStrategy());
  }
