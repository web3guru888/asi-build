...
async function main() {
  console.log('🚀 Starting Enhanced ASI-Code Server with Tool Registry System...');
  ...
  try {
    ...
    console.log(`   - Categories: ${Object.keys(toolInitResult.categoryCounts).join(', ')}`);
    console.log(`   - Initialization time: ${toolInitResult.initializationTime}ms`);
    ...
    server.app.get('/health/detailed', async (c) => {
      try {
        ...
        const toolHealth = await toolManager.healthCheck?.() || { status: 'unknown' };
    ...
    setTimeout(async () => {
      try {
        const systemInfo = toolManager.getSystemInfo?.() || {};
        const health = await toolManager.healthCheck?.() || { status: 'unknown' };
        
        console.log('📊 Tool System Status:');
        console.log(`   - Total tools: ${systemInfo.totalTools || 'unknown'}`);
        console.log(`   - Built-in tools: ${systemInfo.builtInTools?.length || 'unknown'}`);
        console.log(`   - Health: ${health.status}`);
        console.log(`   - Running executions: ${health.details?.runningExecutions || 0}`);
        
        if (systemInfo.categoryCounts) {
          console.log('   - Categories:');
          Object.entries(systemInfo.categoryCounts).forEach(([category, count]) => {
            console.log(`     * ${category}: ${count} tools`);
          });
        }
      } catch (error) {
        console.warn('⚠️  Could not retrieve tool system status:', (error as Error).message);
      }
    }, 1000);
