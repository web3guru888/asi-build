...
export class DefaultSATEngine extends EventEmitter implements SATEngine {
  ...
  async detectPatterns(files: string[]): Promise<ArchitecturalPattern[]> {
    ...
    for (const pattern of this.knownPatterns) {
      ...
      confidence = Math.min(100, (indicatorCount / pattern.indicators.length) * 100);
  ...
  async calculateMetrics(files: string[]): Promise<CodeMetrics> {
    ...
    for (const file of files) {
      try {
        ...
        const complexityIndicators = content.match(/(if|for|while|switch|catch|&&|\|\|)/g);
        totalComplexity += complexityIndicators ? complexityIndicators.length : 1;
    ...
    const cohesion = Math.max(0, 100 - (totalComplexity / Math.max(1, totalLines)) * 100);
  ...
  async generateRecommendations(analysis: ArchitecturalAnalysis): Promise<string[]> {
    const recommendations: string[] = [];

    // Pattern-based recommendations
    const primaryPattern = analysis.patterns[0];
    if (primaryPattern) {
      recommendations.push(`Detected ${primaryPattern.name} pattern with ${primaryPattern.confidence.toFixed(1)}% confidence`);
      
      if (primaryPattern.confidence < 60) {
        recommendations.push(`Consider strengthening ${primaryPattern.name} implementation for better architectural clarity`);
      }
    }

    // Metrics-based recommendations
    if (analysis.metrics.cyclomaticComplexity > analysis.metrics.linesOfCode * 0.1) {
      recommendations.push('High cyclomatic complexity detected. Consider refactoring complex functions');
    }

    if (analysis.metrics.coupling > 5) {
      recommendations.push('High coupling detected. Consider reducing dependencies between modules');
    }

    if (analysis.metrics.cohesion < 70) {
      recommendations.push('Low cohesion detected. Consider improving module organization');
    }

    if (analysis.patterns.length === 0) {
      recommendations.push('No clear architectural patterns detected. Consider implementing a well-defined architecture');
    }

    // General recommendations
    recommendations.push('Consider implementing automated architecture testing');
