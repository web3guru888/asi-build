...
export class ReadTool extends BaseTool {
  ...
  constructor() {
    const definition: ToolDefinition = {
      name: 'read',
      description: 'Read file contents with security controls and access validation',
      parameters: [
        {
          name: 'path',
          type: 'string',
  ...
  async execute(parameters: Record<string, any>, context: ToolExecutionContext): Promise<ToolResult> {
    ...
    try {
      ...
      if (length !== undefined || offset > 0) {
        // Partial read
        content = await this.readPartial(normalizedPath, encoding, offset, length, maxSize);
      } else {
        // Full read with size limit
        const readLength = Math.min(fileStats.size, maxSize);
        const buffer = Buffer.alloc(readLength);
        
        const fileHandle = await import('fs/promises').then(fs => fs.open(normalizedPath, 'r'));
        try {
          const { bytesRead } = await fileHandle.read(buffer, 0, readLength, 0);
          content = encoding === 'binary' ? buffer.subarray(0, bytesRead) : buffer.subarray(0, bytesRead).toString(encoding);
        } finally {
  ...
  private async performSecurityCheck(filePath: string, context: ToolExecutionContext): Promise<{ safe: boolean; reason?: string }> {
    // Check blocked paths
    ...
    if (ext && !this.allowedExtensions.has(ext) && !context.permissions.includes('read_any_file')) {
      return { safe: false, reason: `File type '${ext}' not allowed without read_any_file permission` };
    }
  ...
  private async readPartial(
    ...
    try {
      ...
      const readLength = length !== undefined 
      ...
      return encoding === 'binary' ? actualBuffer : actualBuffer.toString(encoding);
  ...
  async beforeExecute(parameters: Record<string, any>, context: ToolExecutionContext): Promise<void> {
    ...
    console.log(`[ReadTool] Reading file for user ${context.userId}: ${parameters.path}`);
  }
}
