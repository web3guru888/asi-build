...
export class WriteTool extends BaseTool {
  ...
  constructor() {
    const definition: ToolDefinition = {
      name: 'write',
      description:
        'Write content to files with security controls, atomic operations, and backup support',
      parameters: [
        {
          name: 'path',
          type: 'string',
          ...
          validation: {
            custom: (value: string) => {
              if (value.length > 5 * 1024 * 1024)
                return 'Content too large (max 5MB)';
              return true;
            },
          },
        },
  ...
  async execute(
    ...
    try {
      ...
      const writeResult = await this.performWrite(normalizedPath, content, {
        encoding,
        mode,
        atomic,
        append,
  ...
  private async performSecurityCheck(
    ...
    if (content.length > this.maxFileSize) {
      return {
        safe: false,
        reason: `Content too large (${content.length} bytes, max ${this.maxFileSize} bytes)`,
      };
    }
    ...
    if (
      ...
      !context.permissions.includes('write_any_file')
    ) {
      return {
        safe: false,
        reason: `File type '${ext}' not allowed without write_any_file permission`,
      };
    }
    ...
    if (!context.permissions.includes('write_executable_content')) {
      for (const pattern of dangerousPatterns) {
        if (pattern.test(content)) {
          return {
            safe: false,
            reason: 'Content contains potentially dangerous code',
          };
        }
      }
    }
  ...
  private async ensureParentDirectory(
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        throw new Error(
          `Cannot access parent directory: ${(error as Error).message}`
        );
      }
    }
  }
  ...
  async beforeExecute(
    ...
    const contentPreview =
      parameters.content.length > 100
        ? `${parameters.content.substring(0, 97)}...`
        : parameters.content;

    console.log(
      `[WriteTool] Writing to file for user ${context.userId}: ${parameters.path} (${parameters.content.length} bytes)`
    );
  }
}
