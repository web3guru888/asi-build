...
export class OrchestrationSubsystem extends BaseSubsystem {
  ...
  private readonly taskMapping: Map<string, string> = new Map(); // internal task ID -> orchestrator task ID
  ...
  async scaleAgents(factor: number): Promise<void> {
    ...
    this.logger.info(
      `Scaled agents from ${status.agentCount} to ${targetCount}`
    );
  }
...
export class KennyOrchestration {
  private readonly kenny: KennyIntegration;
  private readonly orchestrationSubsystem: OrchestrationSubsystem;
  ...
  constructor(kenny: KennyIntegration) {
    ...
    this.orchestrationSubsystem = new OrchestrationSubsystem(
      kenny,
      this.logger
    );
  ...
  async executeWorkflow(workflow: {
    ...
    while (processed.size < workflow.steps.length) {
      ...
      const taskPromises = readySteps.map(async step => {
        const taskId = await this.orchestrationSubsystem.submitTask(
          step.description,
          {
            metadata: {
              workflow: workflow.name,
