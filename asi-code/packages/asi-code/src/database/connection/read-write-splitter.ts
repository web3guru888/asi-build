...
export class ReadWriteSplitter {
  ...
  private readonly replicas: Map<string, ReplicaStatus> = new Map();
  private readonly stats: ReadWriteStats;
  ...
  private async initializeReadReplicas(): Promise<void> {
    ...
    const replicaPromises = this.config.readReplicas.map(
      async (replicaConfig, index) => {
        ...
        try {
          const poolConfig = {
            ...
            min: Math.floor(
              this.config.pool.min / this.config.readReplicas!.length
            ),
            max: Math.floor(
              this.config.pool.max / this.config.readReplicas!.length
            ),
            acquireTimeoutMillis: this.config.pool.acquireTimeoutMillis,
            createTimeoutMillis: this.config.pool.createTimeoutMillis,
            destroyTimeoutMillis: this.config.pool.destroyTimeoutMillis,
  ...
  private async performHealthChecks(): Promise<void> {
    const healthCheckPromises = Array.from(this.replicas.values()).map(
      async replica => {
        try {
          const startTime = Date.now();
          const client = await replica.pool.connect();
  ...
  private updateAverageTime(
    ...
    if (type === 'read') {
      const total =
        this.stats.averageReadTime * (this.stats.readQueries - 1) +
        executionTime;
      this.stats.averageReadTime = total / this.stats.readQueries;
    } else {
      const total =
        this.stats.averageWriteTime * (this.stats.writeQueries - 1) +
        executionTime;
    ...
    return {
      ...
      queries: {
        ...
        averageExecutionTime:
          (this.stats.averageReadTime + this.stats.averageWriteTime) / 2,
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      // Close replica pools
      const replicaPromises = Array.from(this.replicas.values()).map(
        async replica => {
          try {
