...
class IntegratedInformationTheory(BaseConsciousness):
    ...
    def _connect_layers(self, from_layer: List[str], to_layer: List[str], 
    ...
    def add_connection(self, connection: Connection) -> None:
        ...
        if (connection.from_element in self.elements and 
            connection.to_element in self.elements):
            ...
            self.logger.info(f"Added connection: {connection.from_element} -> {connection.to_element}")
            ...
            for subset in itertools.combinations(all_elements, size):
                ...
                if phi > self.phi_threshold:
                    complex = IntegratedComplex(
                        complex_id=f"complex_{len(complexes)}",
                        elements=subset,
                        phi_value=phi
                    )
    ...
    def get_phi_history(self) -> List[Dict[str, Any]]:
        ...
        return [
            {'timestamp': timestamp, 'phi': phi}
            for timestamp, phi in self.phi_history
        ]
