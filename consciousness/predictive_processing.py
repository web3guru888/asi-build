...
class PredictionError:
    ...
    def calculate_surprise(self) -> float:
        ...
        if self.precision > 0:
            return -math.log(max(1e-10, norm.pdf(self.error_magnitude, 0, 1/math.sqrt(self.precision))))
        return 0.0
...
class PredictiveProcessing(BaseConsciousness):
    ...
    def _initialize(self):
        ...
        self.logger.info(f"Initialized Predictive Processing with {len(self.hierarchy_levels)} levels")
    
    def add_sensory_input(self, input_type: str, input_data: Any) -> None:
        """Add sensory input to the system"""
        self.sensory_input[input_type] = {
            'data': input_data,
            'timestamp': time.time()
        }
    ...
    def _prepare_input_for_level(self, input_data: Any, level_id: int) -> np.ndarray:
        ...
        if level_id > 0:
            ...
            base_array = np.convolve(base_array, np.ones(min(3, len(base_array)))/min(3, len(base_array)), mode='same')
    ...
    def _check_prediction_errors(self, level_id: int, new_prediction: Prediction, 
        ...
        for pred_id, old_prediction in list(level.active_predictions.items()):
            if (old_prediction.prediction_id != new_prediction.prediction_id and
                old_prediction.prediction_type == new_prediction.prediction_type):
                
                # Calculate prediction error
                if isinstance(old_prediction.predicted_value, (int, float)) and isinstance(actual_input, (int, float)):
    ...
    def _update_precision_weights(self, level: HierarchicalLevel, error: PredictionError) -> None:
        ...
        new_precision = (
            (1.0 - self.precision_learning_rate) * current_precision +
            self.precision_learning_rate * error_factor
        )
    ...
    def make_action_prediction(self, action_type: str, action_parameters: Dict[str, Any]) -> Prediction:
    ...
    def update_from_action_outcome(self, action_prediction_id: str, actual_outcome: Any) -> None:
        ...
        if action_prediction_id in self.action_predictions:
            ...
            if isinstance(prediction.predicted_value, (int, float)) and isinstance(actual_outcome, (int, float)):
    ...
    def update(self) -> None:
        ...
        self.metrics.awareness_level = len(self.conscious_predictions) / max(1, len(self.current_predictions))
        
        # Calculate integration based on multi-level activity
        active_levels = sum(1 for level in self.hierarchy_levels.values() if level.active_predictions)
    ...
    def get_prediction_hierarchy_data(self) -> Dict[str, Any]:
        ...
        for level_id, level in self.hierarchy_levels.items():
            predictions_data = [
                pred.to_dict() for pred in level.active_predictions.values()
            ]
