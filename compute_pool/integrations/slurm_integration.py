...
class SlurmIntegration:
    ...
    async def _check_slurm_availability(self) -> None:
        """Check if Slurm commands are available"""
        commands_to_check = [
            self.sbatch_cmd, self.squeue_cmd, self.scancel_cmd,
            self.sinfo_cmd, self.scontrol_cmd, self.sacct_cmd
        ]
    ...
    async def _load_partition_info(self) -> None:
        ...
        try:
            result = await self._run_command([
                self.sinfo_cmd, "--format=%P,%a,%l,%D,%T,%N",
                "--noheader", "--parsable2"
            ])
    ...
    async def _load_node_info(self) -> None:
        ...
        try:
            result = await self._run_command([
                self.sinfo_cmd, "--Node", "--format=%N,%T,%C,%m,%G,%P",
                "--noheader", "--parsable2"
            ])
    ...
    async def _update_job_status(self, slurm_job_id: str) -> None:
        ...
        try:
            ...
            result = await self._run_command([
                self.squeue_cmd, "--job", slurm_job_id,
            ...
            result = await self._run_command([
                self.sacct_cmd, "--job", slurm_job_id,
    ...
    async def get_cluster_resources(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "gpus": {
                ...
                "utilization": ((total_gpus - idle_gpus) / total_gpus * 100) if total_gpus > 0 else 0
            },
