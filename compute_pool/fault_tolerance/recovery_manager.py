    ...
    async def start_monitoring(self) -> None:
        ...
        while True:
            try:
                ...
                self.check_history = [
                    (timestamp, healthy) for timestamp, healthy in self.check_history
...
class RecoveryManager:
    ...
    async def _execute_recovery_actions(self, execution_id: str) -> None:
        ...
        try:
            ...
            for i, action in enumerate(recovery_plan.recovery_actions):
                ...
                execution.logs.append(f"Executing action {i + 1}/{len(recovery_plan.recovery_actions)}: {action.value}")
                
                success = await self._execute_recovery_action(
                    action,
                    failure_event.affected_component,
                    execution
                )
            ...
            all_completed = execution.current_action_index >= len(recovery_plan.recovery_actions) - 1
            ...
            if execution.success:
                ...
                self._stats["average_recovery_time"] = (
                    (self._stats["average_recovery_time"] * (self._stats["recovered_failures"] - 1) + recovery_time)
                    / self._stats["recovered_failures"]
                ) if self._stats["recovered_failures"] > 0 else recovery_time
    ...
    def _get_failure_type_distribution(self) -> Dict[str, int]:
        ...
        for failure_type in FailureType:
            count = len([
                f for f in self.failure_events.values()
            ...
            for partition in psutil.disk_partitions():
                try:
                    ...
                    if (usage.used / usage.total) > 0.9:
                        self.logger.warning(f"Disk {partition.mountpoint} is {(usage.used / usage.total) * 100:.1f}% full")
                        return False
