...
async def basic_compute_pool_demo():
    ...
    try:
        ...
        for _ in range(12):  # Monitor for ~1 minute
            ...
            logger.info(f"Pool status: {pool_status['active_jobs']} active, "
            ...
            for job_id in job_ids:
                ...
                if job_status:
                    logger.info(f"Job {job_id}: {job_status['status']} - Progress: {job_status['progress']}%")
        ...
        logger.info(f"Total jobs processed: {final_status['statistics']['jobs_completed'] + final_status['statistics']['jobs_failed']}")
...
async def advanced_scheduling_demo():
    ...
    try:
        ...
        for _ in range(15):
            pool_status = await pool_manager.get_pool_status()
            logger.info(f"Active: {pool_status['active_jobs']}, Pending: {pool_status['pending_jobs']}")
