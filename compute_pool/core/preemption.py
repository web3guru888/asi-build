...
class PreemptionManager:
    ...
    async def find_preemption_candidates(
        ...
        return [self._find_job_by_id(job_id, running_jobs) for job_id in selected if self._find_job_by_id(job_id, running_jobs)]
    ...
    async def _create_migration_plan(
        ...
        try:
            ...
            network_bandwidth = await self._estimate_network_bandwidth_requirement(checkpoint_size, migration_type)
            
            # Estimate migration duration
            duration = await self._estimate_migration_duration(checkpoint_size, network_bandwidth, migration_type)
    ...
    async def execute_migration(self, plan: MigrationPlan) -> bool:
        ...
        try:
            self.logger.info(f"Starting migration of job {plan.job_id} from {plan.source_node} to {plan.target_node}")
            ...
            self._stats["average_migration_time"] = (
                (self._stats["average_migration_time"] * (self._stats["total_migrations"] - 1) + duration)
    ...
    async def _estimate_migration_duration(
        ...
        transfer_time = (checkpoint_size * 8) / (network_bandwidth * 1024 * 1024)  # Convert to seconds
    ...
    async def get_preemption_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            ...
            "active_migrations": len([p for p in self.migration_history if time.time() - p.estimated_duration < 300]),
            "preemption_success_rate": (
                self._stats["successful_preemptions"] / max(self._stats["total_preemptions"], 1) * 100
            ),
