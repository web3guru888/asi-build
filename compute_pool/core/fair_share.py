...
class UserAccount:
    ...
    usage_history: List[Tuple[float, float]] = field(default_factory=list)  # (timestamp, resource_seconds)
...
class FairShareManager:
    ...
    async def update_usage(
        ...
        user.usage_history = [
            (timestamp, usage) for timestamp, usage in user.usage_history
            if timestamp > cutoff_time
        ]
        
        self.logger.debug(f"Updated usage for user {user_id}: {total_resource_seconds} resource-seconds")
    ...
    async def _calculate_fairness_metrics(self) -> None:
        ...
        for user in self.users.values():
            if user.shares > 0:
                effective_usage = sum(
                    usage for timestamp, usage in user.usage_history
    ...
    async def get_user_stats(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        for resource_type, quota in user_quotas.items():
            quota_status[resource_type] = {
                ...
                "utilization_percent": (quota.current_usage / quota.hard_limit * 100) if quota.hard_limit > 0 else 0
            }
