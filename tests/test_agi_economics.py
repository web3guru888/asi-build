...
_fake_module(
    "asi_build.agi_economics.algorithms.auction_mechanisms",
...
class TestCoreTypes:
    ...
    def test_token_balance_available_amount(self):
        tb = TokenBalance(
            token_type=TokenType.AGIX, amount=Decimal("100"), locked_amount=Decimal("30")
        )
...
class TestTokenEconomics:
    ...
    def test_token_supply_info_circulating(self):
        si = TokenSupplyInfo(
            ...
            max_supply=Decimal("10000"),
            burned_supply=Decimal("200"),
...
def _make_provider(
    pid="prov1", amount=Decimal("100"), cost=Decimal("1"), reputation=0.8, reliability=0.9
) -> ResourceProvider:
    resources = {
...
def _make_request(
    rid="req1", amount=Decimal("10"), budget=Decimal("999"), deadline_offset=3600
) -> ServiceRequest:
...
def _buy_order(
    agent="buyer1", price=Decimal("10"), qty=Decimal("5"), service="compute", oid=None
) -> MarketOrder:
    return MarketOrder(
...
def _sell_order(
    agent="seller1", price=Decimal("10"), qty=Decimal("5"), service="compute", oid=None
) -> MarketOrder:
    return MarketOrder(
