...
class SignalProcessor:
    ...
    def __init__(self, config: BCIConfig):
        ...
        # Real-time buffers
        self.signal_buffer = np.zeros((self.num_channels, int(self.sampling_rate * 5)))  # 5 sec buffer
        self.buffer_index = 0
    ...
    def _design_filters(self):
        ...
        self.notch_b, self.notch_a = iirnotch(
            notch_freq, 30, self.sampling_rate
        )
    ...
    def _apply_filters(self, data: np.ndarray) -> np.ndarray:
        ...
        for ch in range(self.num_channels):
            filtered_data[ch, :] = filtfilt(
                self.bandpass_b, self.bandpass_a, filtered_data[ch, :]
            )
        
        # Notch filter
        for ch in range(self.num_channels):
            filtered_data[ch, :] = filtfilt(
                self.notch_b, self.notch_a, filtered_data[ch, :]
            )
    ...
    def train_ica_model(self, training_data: np.ndarray):
        ...
        try:
            ...
            n_components = min(self.config.signal_processing.ica_components, 
                             self.num_channels)
