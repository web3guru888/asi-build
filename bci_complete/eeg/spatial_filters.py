...
class SpatialFilterBank:
    ...
    def _create_car_matrix(self) -> np.ndarray:
        ...
        car_matrix = np.eye(self.n_channels) - np.ones((self.n_channels, self.n_channels)) / self.n_channels
        return car_matrix
    ...
    def apply_pca(self, data: np.ndarray, n_components: Optional[int] = None, explained_variance: float = 0.95) -> np.ndarray:
    ...
    def apply_bipolar_montage(self, data: np.ndarray, pairs: Optional[List[Tuple[str, str]]] = None) -> Tuple[np.ndarray, List[str]]:
    ...
    def compute_spatial_patterns(self, data: np.ndarray, labels: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    def apply_source_localization(self, data: np.ndarray, method: str = 'minimum_norm') -> np.ndarray:
    ...
    def _minimum_norm_estimation(self, data: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # Regularized inverse
            reg_param = 0.1
            inverse_op = linalg.pinv(leadfield.T @ leadfield + reg_param * np.eye(n_sources)) @ leadfield.T
    ...
    def apply_filter(self, data: np.ndarray, filter_name: str, **kwargs) -> Tuple[np.ndarray, Optional[List[str]]]:
