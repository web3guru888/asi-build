...
class BCIMetrics:
    ...
    def information_transfer_rate(accuracy: float, n_classes: int, 
        ...
        bits_per_selection = np.log2(n_classes) + p * np.log2(p) + (1-p) * np.log2((1-p)/(n_classes-1))
        ...
        if y_proba is not None:
            try:
                ...
                if len(unique_classes) == 2:
                    ...
                    metrics['roc_auc_ovo'] = float(roc_auc_score(y_true, y_proba, 
    ...
    @staticmethod
    def temporal_metrics(timestamps: np.ndarray, predictions: np.ndarray, 
        ...
        if window_size > 0:
            ...
            if sliding_accuracies:
                ...
                metrics['accuracy_trend'] = float(np.corrcoef(
                    range(len(sliding_accuracies)), sliding_accuracies
                )[0, 1]) if len(sliding_accuracies) > 1 else 0.0
    ...
    def signal_quality_metrics(data: np.ndarray, sampling_rate: float) -> Dict[str, float]:
        ...
        metrics['mean_dynamic_range'] = float(np.mean([cm['dynamic_range'] for cm in channel_metrics]))
    ...
    @staticmethod
    def user_experience_metrics(response_times: List[float], 
                              error_rates: List[float],
                              user_ratings: Optional[List[int]] = None) -> Dict[str, float]:
        ...
        if response_times:
            ...
            metrics['responsiveness_score'] = float(np.mean(response_times) + 2 * np.std(response_times))
        ...
        if user_ratings:
            ...
            metrics['user_satisfaction_score'] = float(np.mean(user_ratings) / 5.0)  # Assuming 1-5 scale
    ...
    def comprehensive_report(y_true: np.ndarray, y_pred: np.ndarray,
                           ...
                           selection_time: Optional[float] = None) -> Dict[str, Any]:
