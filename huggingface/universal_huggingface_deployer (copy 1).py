...
#!/usr/bin/env python3
"""
Kenny - AI-Powered GUI Automation System

Copyright (c) 2025 VBRL Holdings Inc. (vbrl.ai)

This software is licensed under the Sustainable Use License.
For commercial use, please contact licensing@vbrl.ai

For the full license text, see the LICENSE file in the project root.
For enterprise licensing, see LICENSE_EE file in the project root.
"""
...
class UniversalHuggingFaceDeployer:
    ...
    async def deploy_anything(self, 
    ...
    async def _parse_resource(self, resource: Union[str, Dict]) -> HFResource:
        ...
        if isinstance(resource, dict):
            return HFResource(
                ...
                framework=ModelFramework(resource.get("framework", "custom")) if resource.get("framework") else None,
        ...
        return HFResource(
            url=url,
            resource_type=HFResourceType.UNKNOWN,
            repo_id=repo_id,
            revision=revision
        )
    ...
    async def _create_inference_endpoint(self, resource: HFResource, **kwargs) -> DeploymentResult:
        ...
        try:
            ...
            endpoint_config = {
                ...
                "revision": resource.revision,
                "task": resource.task,
                "framework": str(resource.framework.value) if resource.framework else "transformers",
            ...
            logger.info(f"  Scale-to-zero: Enabled ({self.mlops_config['idle_timeout_minutes']} min)")
    ...
    async def _load_model_locally(self, resource: HFResource, **kwargs) -> DeploymentResult:
        ...
        try:
            ...
            elif resource.framework == ModelFramework.SENTENCE_TRANSFORMERS and SENTENCE_TRANSFORMERS_AVAILABLE:
    ...
    async def list_deployments(self) -> List[Dict[str, Any]]:
        ...
        for dep_id, result in self.deployed_resources.items():
            deployments.append({
                ...
                "framework": result.resource.framework.value if result.resource.framework else None,
                "endpoints": {
                    "api": result.api_url,
