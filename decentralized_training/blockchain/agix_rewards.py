...
class AGIXRewardCalculator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.early_submission_multiplier = Decimal(str(config.get('early_submission_multiplier', '1.2')))
        ...
        # Pool settings
        self.round_reward_pool = Decimal(str(config.get('round_reward_pool', '10000')))  # 10k AGIX per round
        self.min_reward_per_node = Decimal(str(config.get('min_reward_per_node', '1')))
        
        self.logger = logging.getLogger(__name__)
    
    def calculate_reward(self, contribution: ComputeContribution, 
                        node_history: List[ComputeContribution],
                        round_deadline: float) -> RewardCalculation:
    ...
    def _calculate_consistency_bonus(self, base_reward: Decimal, 
                                   history: List[ComputeContribution]) -> Decimal:
        ...
        if participation_rate >= self.consistency_threshold and avg_accuracy >= self.high_accuracy_threshold:
            return base_reward * (self.consistency_multiplier - Decimal('1'))
        
        return Decimal('0')
    
    def distribute_round_rewards(self, contributions: List[ComputeContribution],
                               node_histories: Dict[str, List[ComputeContribution]],
                               round_deadline: float) -> List[RewardCalculation]:
...
class AGIXTokenManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.agix_contract = self.w3.eth.contract(
            address=config['agix_token_address'],
            abi=config['agix_token_abi']
        )
        
        # Reward distribution contract
        self.reward_contract = self.w3.eth.contract(
            address=config['reward_contract_address'],
            abi=config['reward_contract_abi']
        )
...
class ReputationBasedRewards:
    ...
    def calculate_reputation_adjusted_reward(self, 
                                           base_reward: Decimal,
                                           reputation_score: float,
                                           node_stake: Decimal) -> Decimal:
        ...
        stake_multiplier = 1 + np.log10(max(1, float(node_stake) / 1000))  # Log base relative to 1k AGIX
        stake_multiplier = min(stake_multiplier, 3.0)  # Cap at 3x
        
        # Combined multiplier
        reputation_multiplier = 0.5 + (normalized_reputation * 1.5)  # 0.5x to 2x range
        
        adjusted_reward = base_reward * Decimal(str(reputation_multiplier)) * Decimal(str(stake_multiplier))
...
class AntiSybilMechanism:
    ...
    def detect_sybil_nodes(self, contributions: List[ComputeContribution],
                          node_metadata: Dict[str, Dict[str, Any]]) -> List[str]:
        ...
        for ip_addr, nodes in ip_groups.items():
            if len(nodes) > self.ip_clustering_threshold:
                ...
                self.logger.warning(f"Suspicious IP clustering detected: {ip_addr} ({len(nodes)} nodes)")
    ...
    def filter_rewards_for_sybils(self, reward_calculations: List[RewardCalculation],
                                 suspicious_nodes: List[str]) -> List[RewardCalculation]:
...
class RewardDistributionManager:
    ...
    async def process_round_rewards(self, 
        ...
        for contrib in contributions:
            ...
            self.contribution_history[contrib.node_id] = self.contribution_history[contrib.node_id][-50:]
        ...
        self.logger.info(f"Distributed {total_distributed} AGIX across {successful_distributions} nodes")
    ...
    def get_treasury_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'suspicious_nodes_count': len(self.anti_sybil.detect_sybil_nodes([], self.node_metadata))
        }