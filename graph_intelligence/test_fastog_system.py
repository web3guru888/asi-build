...
from graph_intelligence.fastog_reasoning import (
    FastToGReasoningEngine, ReasoningRequest, ReasoningMode
)
from graph_intelligence.schema_manager import SchemaManager
from graph_intelligence.community_detection import CommunityDetectionEngine
from graph_intelligence.community_pruning import CommunityPruningSystem
from graph_intelligence.community_to_text import CommunityTextGenerator
...
async def test_fastog_complete():
    ...
    try:
        ...
        logger.info(f"  ✅ Pruning successful: {pruning_result.original_count} → {pruning_result.pruned_count} communities")
    ...
    try:
        ...
        communities = schema_manager.find_nodes(
        ...
        if communities:
            ...
            text_result = text_generator.generate_community_summary(
                community_id=communities[0].get('id', 'test'),
    ...
    try:
        ...
        if result.final_recommendation:
            logger.info(f"  ✅ Generated recommendation: {list(result.final_recommendation.keys())}")
    ...
    try:
        ...
        for mode in [ReasoningMode.COMMUNITY_BASED, ReasoningMode.TRADITIONAL, ReasoningMode.HYBRID]:
            ...
            modes_tested.append({
                'mode': mode.value,
                'time': duration,
                'communities': result.communities_analyzed
            })
            
            logger.info(f"  ✅ {mode.value}: {duration:.2f}s ({result.communities_analyzed} communities)")
