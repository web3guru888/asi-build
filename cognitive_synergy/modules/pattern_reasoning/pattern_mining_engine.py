...
class PatternMiningEngine:
    ...
    def _detect_cycles(self, time_series: np.ndarray) -> List[Dict[str, Any]]:
        ...
        for i in range(2, min(len(autocorr), len(time_series)//2)):
            if (i < len(autocorr) - 1 and
                autocorr[i] > autocorr[i-1] and
                autocorr[i] > autocorr[i+1] and
                autocorr[i] > 0.3 * autocorr[0]):  # Significant correlation
    ...
    def _find_graph_motifs(self, graph: nx.DiGraph) -> List[Dict[str, Any]]:
        ...
        triangles = [triangle for triangle in nx.enumerate_all_cliques(graph.to_undirected()) 
                    if len(triangle) == 3]
    ...
    def _validate_patterns(self, patterns: List[Pattern]) -> List[Pattern]:
        ...
        for pattern in patterns:
            ...
            if (pattern.support >= self.min_support and 
    ...
    def _compute_hierarchy_relationship(self, pattern1: Pattern, pattern2: Pattern) -> Optional[str]:
        """Compute hierarchical relationship between two patterns"""
        # Simple hierarchy based on complexity and support
        if pattern1.type == pattern2.type:
            if (pattern1.complexity < pattern2.complexity and 
                pattern1.support < pattern2.support):
                return 'pattern1_sub'
            elif (pattern2.complexity < pattern1.complexity and 
                  pattern2.support < pattern1.support):
                return 'pattern2_sub'
