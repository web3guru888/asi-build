...
class NLGenerator:
    ...
    async def generate(
        ...
        try:
            ...
            parsed_logic = await self._parse_logical_expression(
                logical_expression, formalism
            )
            
            # Generate using multiple strategies
            template_result = await self._generate_with_templates(
                parsed_logic, formalism, context
            )
            ...
            # Choose best result
            if template_result and (not neural_result or template_result.confidence > neural_result.confidence):
        ...
        for pattern_type, pattern in patterns.items():
            ...
            if match:
                ...
                return {
                    ...
                    "rule_type": getattr(PLNRuleType, pattern_type.upper(), PLNRuleType.INHERITANCE)
                }
        ...
        if rule_type and rule_type in self.pln_templates:
            ...
            if template:
                ...
                return GeneratedText(
                    ...
                    metadata={"method": "template", "rule_type": rule_type.value if rule_type else "unknown"}
                )
    ...
    async def _generate_generic_with_templates(
        ...
        if logic_type == "implication":
            ...
            text = template.format(
                left=parsed_logic.get("premise", ""),
                right=parsed_logic.get("conclusion", "")
            )
        elif logic_type == "conjunction":
            template = self.operator_templates["∧"][self.config.style]
            text = template.format(
                left=parsed_logic.get("premise1", ""),
                right=parsed_logic.get("premise2", "")
            )
    ...
    async def _post_process_generated_text(
        self,
    ...
    def _formalize_text(self, text: str) -> str:
        ...
        if not any(word in text.lower() for word in ["therefore", "consequently", "moreover", "furthermore"]):
            if "." in text:
                sentences = text.split(".")
                if len(sentences) > 1:
    ...
    def _update_stats(self, generated_text: GeneratedText):
        ...
        self.stats["average_confidence"] = \
            (current_avg * (total_texts - 1) + new_confidence) / total_texts
        
        # Update style usage
        style_key = generated_text.style.value
        self.stats["styles_used"][style_key] = \
            self.stats["styles_used"].get(style_key, 0) + 1
