...
from ..core.bridge import NLLogicBridge, LogicFormalism, TranslationResult
...
class QueryInterface:
    ...
    async def _process_nl_to_logic(self, request: QueryRequest) -> Dict[str, Any]:
        ...
        result = await self.bridge.translate_nl_to_logic(
            request.input_text,
            target_formalism=target_formalism,
    ...
    async def _process_validation_request(self, request: QueryRequest) -> Dict[str, Any]:
        ...
        # Validate expression
        is_valid = self.bridge.logic_systems.validate_expression(
            request.input_text,
            formalism
        )
        
        # Parse for additional information
        parsed = await self.bridge.logic_systems.parse_expression(
            request.input_text,
            formalism
        )
    ...
    def get_session_history(self, session_id: str) -> Dict[str, Any]:
        ...
        session_responses = [
            r for r, q in zip(self.response_history, self.query_history)
            if q.session_id == session_id
        ]
    ...
    def _update_stats(
        ...
        self.stats["queries_by_type"][type_key] = \
            self.stats["queries_by_type"].get(type_key, 0) + 1
        ...
        self.stats["success_rate"] = successful_queries / total_queries if total_queries > 0 else 1.0
    ...
    def _update_session_stats(
        ...
        stats["average_confidence"] = \
            (stats["average_confidence"] * (stats["query_count"] - 1) + response.confidence) / stats["query_count"]
        
        # Update query type counts
        query_type_key = request.query_type.value
        stats["query_types"][query_type_key] = \
            stats["query_types"].get(query_type_key, 0) + 1
