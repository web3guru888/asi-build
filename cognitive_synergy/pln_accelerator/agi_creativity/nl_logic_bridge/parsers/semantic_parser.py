...
class SemanticParser:
    ...
    async def parse(
        ...
        try:
            ...
            comp_structure = await self._build_compositional_structure(
                cleaned_text, doc, frames
            )
    ...
    async def _extract_relations(
        ...
        for i, entity1 in enumerate(entities):
            for entity2 in entities[i+1:]:
                ...
                relation = self._infer_relation_from_dependency_path(
                    doc, entity1, entity2
                )
    ...
    def _infer_relation_from_dependency_path(
        ...
        ent1_tokens = [token for token in doc if 
                      token.idx >= entity1[2] and token.idx < entity1[3]]
        ent2_tokens = [token for token in doc if 
                      token.idx >= entity2[2] and token.idx < entity2[3]]
        ...
        for token in doc:
            if token.pos_ == "ADP":  # Preposition
                ...
                if len(potential_heads) > 1:
                    ambiguities.append(f"PP attachment ambiguity: '{token.text}' could modify different elements")
    ...
    def _update_stats(self, frames: List[SemanticFrame], confidence: float):
        ...
        self.stats["average_confidence"] = \
            (current_avg * (total_sentences - 1) + confidence) / total_sentences
