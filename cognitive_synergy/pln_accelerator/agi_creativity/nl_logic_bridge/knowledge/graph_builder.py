...
class KnowledgeGraphBuilder:
    ...
    async def build_graph(
        ...
        try:
            ...
            for result in translation_results:
                ...
                nl_nodes, nl_edges = await self._extract_from_natural_language(
                    result.source_text
                )
                ...
                # Merge edges
                edges = self._merge_edges(edges, nl_edges, result.source_text, result.target_representation)
                edges = self._merge_edges(edges, logic_edges, result.source_text, result.target_representation)
            ...
            self.logger.info(f"Built knowledge graph {graph_id} with {len(nodes)} nodes and {len(edges)} edges")
    ...
    def _calculate_graph_statistics(
        ...
        avg_node_confidence = sum(node.confidence for node in nodes.values()) / len(nodes) if nodes else 0.0
        
        # Edge statistics
        edge_type_counts = Counter(edge.edge_type.value for edge in edges.values())
        avg_edge_confidence = sum(edge.confidence for edge in edges.values()) / len(edges) if edges else 0.0
        avg_edge_strength = sum(edge.strength for edge in edges.values()) / len(edges) if edges else 0.0
        ...
        connected_components = list(nx.connected_components(temp_graph)) if temp_graph.nodes() else []
    ...
    async def query_graph(
        ...
        try:
            ...
            graphs_to_query = [self.graphs[graph_id]] if graph_id and graph_id in self.graphs else list(self.graphs.values())
            ...
            for term in terms:
                if term in node_label_lower or node_label_lower in term:
                    matches.append({
                        ...
                        "confidence": node.confidence,
                        "relevance_score": len([t for t in terms if t in node_label_lower]) / len(terms)
                    })
                    break
