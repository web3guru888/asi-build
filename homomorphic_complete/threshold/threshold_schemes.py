...
class ThresholdRSA(ThresholdScheme):
    ...
    def _share_secret(self, secret: int, threshold: int, num_parties: int, modulus: int) -> List[Tuple[int, int]]:
...
class ThresholdECDSA(ThresholdScheme):
    ...
    def _share_secret(self, secret: int) -> List[Tuple[int, int]]:
        ...
        coeffs = [secret] + [random.randint(0, self.curve_order - 1) 
                            for _ in range(self.threshold - 1)]
        
        shares = []
        for party_id in range(1, self.num_parties + 1):
            share_value = sum(coeff * (party_id ** i) for i, coeff in enumerate(coeffs)) % self.curve_order
            shares.append((party_id, share_value))
