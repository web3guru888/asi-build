...
class ThresholdEncryption:
    ...
    def partial_decrypt(self, ciphertext: Tuple[int, int], 
                       share: Tuple[int, int]) -> Tuple[int, int]:
    ...
    def combine_partial_decryptions(self, partials: List[Tuple[int, int]], 
                                  ciphertext: Tuple[int, int]) -> int:
        ...
        for i, (party_id, partial_result) in enumerate(partials[:self.threshold]):
            ...
            combined_partial = (combined_partial * 
                              pow(partial_result, lagrange_coeff, self.prime)) % self.prime
    ...
    def _share_secret(self, secret: int) -> List[Tuple[int, int]]:
        ...
        coeffs = [secret] + [random.randint(0, self.prime - 1) 
                            for _ in range(self.threshold - 1)]
        
        # Generate shares
        shares = []
        for party_id in range(1, self.num_parties + 1):
            share_value = sum(coeff * (party_id ** i) 
                            for i, coeff in enumerate(coeffs)) % self.prime
