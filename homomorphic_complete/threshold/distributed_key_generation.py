...
class DistributedKeyGeneration:
    ...
    def generate_distributed_key(self) -> Dict[str, Any]:
        ...
        for i in range(self.num_parties):
            ...
            for j in range(self.num_parties):
                ...
                share_value = sum(coeffs[k] * ((j + 1) ** k) 
                                for k, coeffs in enumerate(party_polynomials[i])) % self.prime
    ...
    def verify_share(self, party_id: int, share: int, 
                    verification_info: List[List[int]]) -> bool:
        ...
        for i, commitments in enumerate(verification_info):
            ...
            for j, commitment in enumerate(commitments):
                party_contribution = (party_contribution * 
                                    pow(commitment, (party_id ** j), self.prime)) % self.prime
