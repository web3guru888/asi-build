...
from .base import SecurityLevel, SchemeType, ParameterException
...
class ParameterGenerator:
    ...
    def generate_parameters(self, 
                          scheme_type: SchemeType,
                          security_level: SecurityLevel,
                          custom_params: Optional[Dict] = None) -> FHEParameters:
        """
        ...
        if key not in self.SECURE_PARAMETERS:
            raise ParameterException(f"No predefined parameters for {scheme_type.value} "
        ...
        coeff_modulus = self._generate_coefficient_modulus(
            base_params["coeff_modulus_bits"]
        )
        ...
        self.logger.info(f"Generated {scheme_type.value} parameters for "
    ...
    def estimate_noise_growth(self, 
                            parameters: FHEParameters,
                            operation_sequence: List[str]) -> float:
        """
    ...
    def suggest_parameters(self,
                          scheme_type: SchemeType,
                          required_depth: int,
                          batch_size: Optional[int] = None) -> Tuple[SecurityLevel, FHEParameters]:
        """
        ...
        for security_level in [SecurityLevel.LOW, SecurityLevel.MEDIUM, SecurityLevel.HIGH]:
            try:
                ...
                if max_depth >= required_depth:
                    ...
                    self.logger.info(f"Recommended {security_level.value} security "
