...
class BGVCiphertext(CiphertextBase):
    ...
    def __init__(self, polynomials: List[Polynomial], level: int = 0, 
                 noise_budget: Optional[int] = None, is_ntt_form: bool = False):
        """
    ...
    def __str__(self) -> str:
        return f"BGVCiphertext(size={self.size}, level={self.level}, noise_budget={self.noise_budget})"
...
class BGVScheme(FHECore):
    ...
    def __init__(self, config: FHEConfiguration):
        ...
        logger.info(f"BGV scheme initialized with plaintext modulus {self.parameters.plaintext_modulus}")
    ...
    def encrypt(self, plaintext: Union[BGVPlaintext, List[int], int], 
                public_key: Optional[PublicKey] = None) -> BGVCiphertext:
        """
    ...
    def decrypt(self, ciphertext: BGVCiphertext, 
                secret_key: Optional[SecretKey] = None) -> BGVPlaintext:
        """
    ...
    def add(self, ciphertext1: BGVCiphertext, ciphertext2: BGVCiphertext) -> BGVCiphertext:
        ...
        result = BGVCiphertext(
            ...
            result_std.level if hasattr(result_std, 'level') else ct1.level
        )
    ...
    def add_plain(self, ciphertext: BGVCiphertext, 
                  plaintext: Union[BGVPlaintext, List[int], int]) -> BGVCiphertext:
        """
    ...
    def multiply_plain(self, ciphertext: BGVCiphertext, 
                      plaintext: Union[BGVPlaintext, List[int], int]) -> BGVCiphertext:
        """
