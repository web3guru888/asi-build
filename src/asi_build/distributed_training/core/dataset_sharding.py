...
class ShardingStrategy(ABC):
    ...
    @abstractmethod
    def shard_data(self, dataset: data.Dataset, num_shards: int, 
                   node_capabilities: Dict[str, Dict[str, Any]]) -> List[DataShard]:
...
class IIDSharding(ShardingStrategy):
    ...
    def _calculate_shard_sizes(self, num_shards: int, total_size: int,
                             node_capabilities: Dict[str, Dict[str, Any]]) -> List[int]:
        """Calculate shard sizes based on node capabilities"""
        ...
        capabilities = [node_capabilities[node_id].get('compute_power', 1.0) for node_id in node_ids]
    ...
    def _calculate_label_distribution(self, dataset: data.Dataset, indices: List[int]) -> Dict[str, int]:
...
class AdaptiveSharding(ShardingStrategy):
    ...
    def shard_data(self, dataset: data.Dataset, num_shards: int,
        ...
        for label, indices in class_indices.items():
            ...
            while idx_pointer < len(shuffled_indices):
                ...
                min_node = min(shard_class_counts.keys(), 
                             key=lambda n: shard_class_counts[n][label])
        ...
        for i, (node_id, node_indices) in enumerate(shard_data.items()):
            if node_indices:
                label_dist = {str(label): count for label, count in shard_class_counts[node_id].items()}
    ...
    def _calculate_node_scores(self, node_capabilities: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
...
class DistributedDatasetManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.strategies = {
            ...
            'geographic': GeographicSharding(
                location_clusters=config.get('location_clusters', 5)
            ),
    ...
    async def register_dataset(self, dataset: data.Dataset, dataset_id: str,
                             data_type: str = "vision") -> DatasetMetadata:
        ...
        self.logger.info(f"Registered dataset {dataset_id}: {total_samples} samples, {num_classes} classes")
        
        return metadata
    
    async def shard_dataset(self, dataset_id: str, strategy: str,
                           target_nodes: List[str]) -> List[DataShard]:
        ...
        node_caps = {node_id: self.node_capabilities.get(node_id, {}) 
                    for node_id in target_nodes}
        
        self.logger.info(f"Sharding dataset {dataset_id} using {strategy} strategy for {len(target_nodes)} nodes")
        ...
        self.logger.info(f"Sharding completed in {sharding_time:.2f}s: created {len(shards)} shards")
    ...
    def get_dataset_stats(self, dataset_id: str) -> Dict[str, Any]:
        ...
        for shard in shards:
            ...
            if shard_total > 0:
                normalized_dist = {label: count / shard_total 
                                 for label, count in shard.label_distribution.items()}
    ...
    def _calculate_global_class_distribution(self, dataset: data.Dataset) -> Dict[str, int]:
        ...
        scaled_counts = {label: int(count * scale_factor) 
                        for label, count in class_counts.items()}
        
        return scaled_counts
    
    def _calculate_heterogeneity(self, shard_distributions: List[Dict[str, float]], 
                               global_distribution: Dict[str, int]) -> float:
        ...
        global_probs = {label: count / total_global 
    ...
    async def rebalance_shards(self, dataset_id: str, 
                             performance_metrics: Dict[str, Dict[str, float]]) -> List[DataShard]:
        ...
        for node_id, metrics in performance_metrics.items():
            ...
            avg_time = np.mean([m.get('avg_training_time', 0) for m in performance_metrics.values()])
        ...
        self.logger.info(f"Rebalancing: moving load from {len(slow_nodes)} slow nodes to {len(fast_nodes)} fast nodes")
        ...
        remaining_shards = [shard for shard in current_shards 
                          if shard.node_id not in slow_nodes]
        ...
        if indices_to_redistribute:
            ...
            for i, node_id in enumerate(fast_nodes):
                ...
                if node_indices:
                    # Create new shard
                    dataset = self.datasets[dataset_id]
                    label_dist = self._calculate_label_distribution_from_indices(dataset, node_indices)
    ...
    def _calculate_label_distribution_from_indices(self, dataset: data.Dataset, 
                                                 indices: List[int]) -> Dict[str, int]:
