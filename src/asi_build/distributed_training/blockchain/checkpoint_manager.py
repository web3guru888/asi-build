...
class FilecoinClient:
    ...
    async def create_storage_deal(self, ipfs_hash: str, file_size: int, 
                                duration_epochs: int = 518400) -> Optional[str]:
...
class BlockchainCheckpointManager:
    ...
    async def load_checkpoint(self, checkpoint_id: str) -> Optional[torch.nn.Module]:
        ...
        try:
            ...
            try:
                success = await self.ipfs_client.download_file(
                    checkpoint.ipfs_hash, temp_path
                )
    ...
    async def list_checkpoints(self, creator_address: Optional[str] = None) -> List[ModelCheckpoint]:
    ...
    async def get_checkpoint_metrics(self, checkpoint_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'verification_success_rate': sum(1 for v in verifications if v.is_valid) / max(1, len(verifications))
        }
    ...
    async def _record_checkpoint_on_blockchain(self, checkpoint: ModelCheckpoint):
        ...
        try:
            ...
            if receipt.status == 1:
                ...
                self.logger.error(f"Blockchain transaction failed for checkpoint {checkpoint.checkpoint_id}")
                
        except Exception as e:
            self.logger.error(f"Failed to record checkpoint on blockchain: {e}")
            raise
    
    async def _load_checkpoint_from_blockchain(self, checkpoint_id: str) -> Optional[ModelCheckpoint]:
