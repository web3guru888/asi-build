...
class DistributedHashTable:
    ...
    def store(self, key: str, value: Any) -> bool:
        ...
        self.storage[key] = {"value": value, "timestamp": time.time(), "ttl": 3600}  # 1 hour TTL
        return True
                    ...
                    for bootstrap_node in self.bootstrap_nodes:
                        if bootstrap_node.peer_id not in self.connected_peers:
                            peer_info = await self._perform_handshake(
                                bootstrap_node.ip_address, bootstrap_node.port
                            )
                            if peer_info:
                                self.connected_peers[peer_info.peer_id] = peer_info
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        for peer_data in data.get("peers", []):
                            ...
                            if (
                                new_peer.peer_id != self.node_id
                                and new_peer.peer_id not in self.connected_peers
                            ):

                                connected_peer = await self._perform_handshake(
                                    new_peer.ip_address, new_peer.port
                                )
