...
class PRIMUSFoundation:
    ...
    def compute_synergy(self, primitive1: str, primitive2: str) -> float:
        ...
        try:
            path_length = nx.shortest_path_length(
                self.understanding_graph, primitive1, primitive2
            )
    ...
    def _compute_type_synergy(self, type1: str, type2: str) -> float:
        ...
        return synergy_matrix.get((type1, type2), 
                                synergy_matrix.get((type2, type1), 0.1))
    ...
    def _mine_patterns(self):
        ...
        active_primitives = [
            name for name, prim in self.primitives.items()
            if prim.activation > 0.5
        ]
    ...
    def _update_understanding_graph(self):
        ...
        for name, pattern in self.pattern_space.items():
            if (isinstance(pattern, dict) and 
                ...
                'synergy' in pattern and
                pattern['synergy'] > self.synergy_threshold):
                
                components = pattern['components']
                if len(components) >= 2:
                    self.understanding_graph.add_edge(
                        components[0], components[1],
                        weight=pattern['synergy']
                    )
    
    def _update_motivations(self):
        """Update motivation system based on current state"""
        # Curiosity increases with new patterns
        new_patterns = sum(1 for p in self.pattern_space.values()
                          if isinstance(p, dict) and 
                          time.time() - p.get('timestamp', 0) < 10)
        
        self.motivation_system['curiosity'] = min(1.0, 
            self.motivation_system['curiosity'] + 0.1 * new_patterns)
        
        # Learning motivation based on understanding level
        self.motivation_system['learning'] = (
            0.7 * self.motivation_system['learning'] +
            0.3 * (1.0 - self.current_state.understanding_level)
        )
    ...
    def _adapt_system(self):
        ...
        pattern_rate = len([p for p in self.pattern_space.values()
                           if isinstance(p, dict) and
                           time.time() - p.get('timestamp', 0) < 60])
    ...
    def inject_stimulus(self, stimulus: Dict[str, Any]):
        ...
        if 'type' in stimulus:
            if stimulus['type'] == 'learning':
                self.motivation_system['learning'] = min(1.0, 
                    self.motivation_system['learning'] + 0.2)
            elif stimulus['type'] == 'social':
                self.motivation_system['social_interaction'] = min(1.0,
                    self.motivation_system['social_interaction'] + 0.2)
