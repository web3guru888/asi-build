...
class PLNExtractor:
    ...
    async def _extract_semantic_rules(
        self, text: str, semantic_parse: Optional[Dict[str, Any]]
    ) -> List[PLNRule]:
    ...
    async def _post_process_rules(
        self, rules: List[PLNRule], context: Optional[Dict[str, Any]]
    ) -> List[PLNRule]:
    ...
    def _remove_duplicate_rules(self, rules: List[PLNRule]) -> List[PLNRule]:
        ...
        for rule in rules:
            ...
            signature = (rule.rule_type, tuple(rule.premise), rule.conclusion)
    ...
    def _update_stats(self, rules: List[PLNRule]):
        ...
        if rules:
            ...
            self.extraction_stats["average_confidence"] = (
                current_avg * (total_rules - len(rules)) + avg_confidence * len(rules)
            ) / total_rules
