...
class NLLogicBridge:
    ...
    async def translate_nl_to_logic(
        ...
        try:
            ...
            resolved_ambiguities = await self._resolve_ambiguities(ambiguities, session_context)
    ...
    async def translate_logic_to_nl(
        ...
        try:
            # Update session context
            if session_id:
                self.context_manager.update_session_context(session_id, logical_expression, context)
                session_context = self.context_manager.get_session_context(session_id)
            else:
                session_context = context or {}
        ...
        try:
            if source_type == "natural_language":
                ...
                backward_result = await self.translate_nl_to_logic(
                    forward_result.target_representation, source_formalism, context, session_id
                )
            ...
            self.logger.info(
                f"Bidirectional translation completed with consistency: {consistency_score}"
            )
    ...
    async def build_knowledge_graph(
        ...
        try:
            ...
            for text in texts:
                result = await self.translate_nl_to_logic(text, formalism, session_id=session_id)
    ...
    async def get_translation_alternatives(
        self, text: str, formalism: LogicFormalism, max_alternatives: Optional[int] = None
        ...
        try:
            ...
            alternative_parses = await self.semantic_parser.generate_alternatives(text, max_alts)
    ...
    async def _detect_ambiguities(self, text: str, semantic_parse: Dict[str, Any]) -> List[str]:
    ...
    async def _resolve_ambiguities(
        self, ambiguities: List[str], context: Dict[str, Any]
    ) -> List[str]:
        ...
        for ambiguity in ambiguities:
            if self.config.enable_commonsense:
                resolution = await self.commonsense_reasoner.resolve_ambiguity(ambiguity, context)
    ...
    async def _calculate_logic_to_nl_confidence(
        self, logical_expression: str, nl_text: str, formalism: LogicFormalism
    ) -> float:
    ...
    async def _calculate_consistency(
        self, original: str, forward_result: TranslationResult, backward_result: TranslationResult
    ) -> float:
    ...
    def get_session_stats(self, session_id: str) -> Dict[str, Any]:
        ...
        avg_processing_time = (
            sum(r.metadata.get("processing_time", 0) for r in session_history) / total_translations
        )
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        avg_processing_time = (
            sum(r.metadata.get("processing_time", 0) for r in self.translation_history)
            / total_translations
        )
        ...
        return {
            ...
            "high_confidence_rate": len(
                [
                    r
                    for r in self.translation_history
