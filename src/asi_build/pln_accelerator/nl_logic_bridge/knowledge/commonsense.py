...
class CommonsenseReasoner:
    ...
    async def enhance_context(
        ...
        try:
            ...
            self.logger.info(f"Enhanced context with {len(commonsense_knowledge)} pieces of commonsense knowledge")
    ...
    async def _extract_concepts_from_text(self, text: str) -> List[str]:
        ...
        stop_words = {
            ...
            "should", "may", "might", "can", "must", "shall", "not", "very", "more"
        }
    ...
    async def _get_concept_knowledge(self, concept: str) -> List[CommonsenseKnowledge]:
        ...
        try:
            ...
            for rel in conceptnet_knowledge:
                knowledge.append(CommonsenseKnowledge(
                    subject=rel.get("subject", concept),
                    relation=rel.get("relation", "related_to"),
                    object=rel.get("object", ""),
                    confidence=rel.get("weight", 0.5),
                    source="conceptnet",
                    reasoning_type=self._map_relation_to_reasoning_type(rel.get("relation", "")),
                    context=rel
                ))
    ...
    async def _extract_interpretations(
        self,
        ambiguity: str,
        context: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _score_interpretation(
        self,
        interpretation: str,
        context: Dict[str, Any]
    ) -> float:
        ...
        if "commonsense_knowledge" in context:
            ...
            for item in knowledge_items:
                if any(word in interpretation.lower() for word in [
    ...
    async def _build_reasoning_chain(
        ...
        relevant_knowledge = [
            ...
            if k.reasoning_type == reasoning_type and k.confidence > 0.3
        ]
                ...
                for p_k in p_knowledge:
                    if p_k.object in [c_k.subject for c_k in c_knowledge]:
                        ...
                        connecting_knowledge = [
                            c_k for c_k in c_knowledge 
                            if c_k.subject == p_k.object
                        ]
