...
class HumanInTheLoopController:
    ...
    def submit_human_decision(
        self, loop_id: str, operator_id: str, decision: str, reasoning: str, confidence: float
    ) -> bool:
        ...
        if operator_id in self.decision_queues:
            self.decision_queues[operator_id] = [
                item for item in self.decision_queues[operator_id] if item["id"] != loop_id
            ]
    ...
    def _find_available_operator(self, specialization: str) -> Optional[str]:
        ...
        for operator_id, operator_data in self.human_operators.items():
            if (
                specialization in operator_data["specializations"]
                and operator_data["active_sessions"] < 5  # Max concurrent sessions
                and operator_data["availability"].get("online", False)
            ):
...
class DemocraticOverrideSystem:
    ...
    def request_override(
        ...
        requested_by: str,
        stakeholder_type: str,
        severity: OverrideSeverity = None,
    ) -> str:
        ...
        try:
            ...
            if not capability or override_type not in capability.allowed_override_types:
                raise PermissionError(
                    f"Stakeholder type {stakeholder_type} cannot request {override_type.value}"
                )
            ...
            if severity not in capability.severity_limits:
                raise PermissionError(
                    f"Severity {severity.value} not allowed for {stakeholder_type}"
                )
    ...
    def vote_on_override(
        self, request_id: str, voter_id: str, vote: str, reasoning: str, voting_power: float = 1.0
    ) -> bool:
    ...
    def execute_override(
        self, request_id: str, executor_id: str, execution_method: str = "automatic"
    ) -> bool:
    ...
    def appeal_override(
        self, request_id: str, appellant_id: str, appeal_reason: str, new_evidence: Dict[str, Any]
    ) -> str:
    ...
    def trigger_emergency_stop(
        self, trigger_entity: str, reason: str, affected_systems: List[str]
    ) -> str:
    ...
    def create_human_intervention_point(
        self, decision_context: Dict[str, Any], required_approval_level: str
    ) -> str:
    ...
    def _assess_severity(
        self, override_type: OverrideType, evidence: Dict[str, Any]
    ) -> OverrideSeverity:
    ...
    def _get_required_approvals(
        self, severity: OverrideSeverity, stakeholder_type: str
    ) -> List[str]:
    ...
    def _get_emergency_contacts(self, severity: OverrideSeverity) -> List[str]:
        ...
        if severity in [OverrideSeverity.EMERGENCY, OverrideSeverity.CRITICAL]:
            return ["emergency_response_team", "safety_officer", "governance_chair", "ethics_chair"]
        return ["governance_committee"]
    ...
    def _check_voting_completion(self, override_request: OverrideRequest):
        ...
        approval_votes = sum(
            1 for vote in override_request.votes.values() if vote["vote"] == "approve"
        )
    ...
    def _execute_override_action(
        self, override_request: OverrideRequest, execution_method: str
    ...
    def _initialize_emergency_protocols(self):
        ...
        self.emergency_protocols["human_safety"] = EmergencyProtocol(
            ...
            trigger_conditions=[
                ...
                TriggerCondition.EMERGENCY_SITUATION,
            ],
            ...
            response_time_limits={
                ...
                "assessment": timedelta(minutes=5),
            },
    ...
    def get_override_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requests_by_type": {
                override_type.value: len(
                    [r for r in self.override_requests.values() if r.override_type == override_type]
                )
                for override_type in OverrideType
            },
            "requests_by_severity": {
                severity.value: len(
                    [r for r in self.override_requests.values() if r.severity == severity]
                )
                for severity in OverrideSeverity
            },
            "requests_by_status": {
                status.value: len(
                    [r for r in self.override_requests.values() if r.status == status]
                )
                for status in OverrideStatus
            },
            "emergency_executions": len(
                [e for e in self.executions.values() if e.execution_method == "emergency"]
            ),
