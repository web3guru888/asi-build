...
class QuadraticVotingSystem:
    ...
    def cast_quadratic_vote(
        ...
        try:
            ...
            if used_credits + cost > self.max_credits_per_voter:
                return (
                    False,
                    f"Insufficient credits. Need {cost}, have {self.max_credits_per_voter - used_credits}",
                )
            ...
            self.vote_records[voter_id] = [
                v for v in self.vote_records[voter_id] if v.proposal_id != proposal_id
            ]

            self.vote_records[voter_id].append(vote)

            logger.info(
                f"Quadratic vote cast: {voter_id} voted {vote_direction} "
    ...
    def _get_used_credits(self, voter_id: str, current_proposal_id: str) -> float:
        ...
        return sum(
            vote.token_cost
            for vote in self.vote_records[voter_id]
...
class LiquidDemocracy:
    ...
    def delegate_voting_power(
        ...
        try:
            ...
            self.delegations[delegator_id] = [
                d
                for d in self.delegations[delegator_id]
    ...
    def calculate_effective_voting_power(self, stakeholder_id: str, scope: str) -> float:
        ...
        for delegator_id, delegations in self.delegations.items():
            for delegation in delegations:
                if (
                    delegation.delegate_id == stakeholder_id
                    and (delegation.scope == scope or delegation.scope == "general")
                    and self._is_delegation_active(delegation)
                ):
    ...
    def get_delegation_chain(self, stakeholder_id: str, scope: str) -> List[str]:
        ...
        while current in self.delegations:
            for delegation in self.delegations[current]:
                if (
                    (delegation.scope == scope or delegation.scope == "general")
                    and self._is_delegation_active(delegation)
                    and delegation.delegate_id not in visited
                ):
...
class MultiStakeholderConsensus:
    ...
    def register_stakeholder(self, stakeholder: StakeholderProfile) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Registered stakeholder: {stakeholder.name} ({stakeholder.category.value})"
            )
            return True
        except Exception as e:
    ...
    def initiate_consensus_process(
        self,
        proposal_id: str,
        method: VotingMethod,
        ...
        try:
            ...
            process = ConsensusProcess(
                ...
                consensus_threshold=(
                    custom_thresholds.get("consensus", 0.65) if custom_thresholds else 0.65
                ),
                participation_threshold=(
                    custom_thresholds.get("participation", 0.15) if custom_thresholds else 0.15
                ),
                votes={},
                delegations=[],
                deliberation_log=[],
    ...
    def cast_consensus_vote(
        self, process_id: str, voter_id: str, vote_data: Dict[str, Any]
    ) -> Tuple[bool, str]:
    ...
    def _cast_quadratic_consensus_vote(
        self, process: ConsensusProcess, stakeholder: StakeholderProfile, vote_data: Dict[str, Any]
    ) -> Tuple[bool, str]:
        ...
        if success:
            ...
            effective_power = (
                vote_data["intensity"] * category_weight * stakeholder.credibility_score
            )
    ...
    def _cast_liquid_democracy_vote(
        self, process: ConsensusProcess, stakeholder: StakeholderProfile, vote_data: Dict[str, Any]
    ) -> Tuple[bool, str]:
    ...
    def _cast_weighted_vote(
        self, process: ConsensusProcess, stakeholder: StakeholderProfile, vote_data: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """Cast a weighted vote."""
        category_weight = process.stakeholder_weights.get(stakeholder.category, 1.0)
        effective_power = (
            category_weight * stakeholder.credibility_score * stakeholder.voting_power_base
        )
    ...
    def finalize_consensus(self, process_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            consensus_reached = (
                results["participation_rate"] >= process.participation_threshold
                and results["support_ratio"] >= process.consensus_threshold
            )
            ...
            logger.info(
                f"Finalized consensus {process_id}: {'REACHED' if consensus_reached else 'FAILED'}"
            )
    ...
    def _calculate_simple_results(self, process: ConsensusProcess) -> Dict[str, Any]:
        ...
        opposition_votes = sum(
            1 for vote in process.votes.values() if vote["direction"] == "against"
        )
    ...
    def get_consensus_summary(self, process_id: str) -> Optional[Dict[str, Any]]:
        ...
        summary = {
            ...
            "participation_stats": {
                ...
                "participation_rate": (
                    len(process.votes) / len(self.stakeholders) if self.stakeholders else 0
                ),
            },
            "thresholds": {
    ...
    def get_stakeholder_influence_analysis(self, process_id: str) -> Dict[str, Any]:
        ...
        individual_power = {
            voter_id: vote["effective_power"] for voter_id, vote in process.votes.items()
        }
