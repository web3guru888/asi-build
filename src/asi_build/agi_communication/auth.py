...
class InterAGIAuthenticator:
    ...
    async def _authenticate_pki(self, agi_id: str) -> bool:
        ...
        if credentials.is_revoked or (
            credentials.expires_at and datetime.now() > credentials.expires_at
        ):
            return False
    ...
    async def _handle_trust_query(self, message: CommunicationMessage):
        ...
        if queried_agi in self.trust_records:
            ...
            trust_info = {
                ...
                "interaction_count": trust_record.successful_interactions
                + trust_record.failed_interactions,
                "success_rate": (
                    trust_record.successful_interactions
                    / max(
                        1, trust_record.successful_interactions + trust_record.failed_interactions
                    )
                ),
            }
        else:
    ...
    async def _handle_challenge(self, message: CommunicationMessage):
        ...
        if agi_id in self.credentials:
            credentials = self.credentials[agi_id]
            response = self.crypto_manager.sign_message(challenge_data, credentials.private_key)
