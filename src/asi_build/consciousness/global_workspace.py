...
class GlobalWorkspaceTheory(BaseConsciousness):
    ...
    def _initialize(self):
        ...
        default_processors = [
            CognitiveProcessor(
                "visual", "visual_processing", interests={"visual", "image", "screen", "ui"}
            ),
            CognitiveProcessor(
                "linguistic", "language_processing", interests={"text", "language", "speech", "nlp"}
            ),
            CognitiveProcessor(
                "motor", "motor_control", interests={"action", "movement", "control", "automation"}
            ),
            CognitiveProcessor(
                "executive",
                "executive_control",
            ...
            CognitiveProcessor(
                "memory", "memory_systems", interests={"memory", "recall", "learning", "experience"}
            ),
            CognitiveProcessor(
                "emotional",
        ...
        self.logger.info(
            f"Initialized Global Workspace with {len(self.cognitive_processors)} processors"
        )
    ...
    def update(self) -> None:
        ...
        active_processors = sum(
            1 for p in self.cognitive_processors.values() if p.current_load > 0.1
        )
        ...
        if (
            len(self.workspace_buffer) > 0
            and current_time - self.last_broadcast_time > self.broadcast_interval * 2
        ):
            self._trigger_competition()
        ...
        with self.workspace_lock:
            return {
                ...
                "active_processors": len(
                    [p for p in self.cognitive_processors.values() if p.active]
                ),
                "current_broadcast": (
                    self.current_broadcast.broadcast_id if self.current_broadcast else None
                ),
                "conscious_content": (
                    self.conscious_content.content_id if self.conscious_content else None
                ),
                ...
                "processor_loads": {
                    pid: p.current_load for pid, p in self.cognitive_processors.items()
                },
            }
