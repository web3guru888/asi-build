...
class TemporalConsciousness(BaseConsciousness):
    ...
    def register_temporal_event(self, content: Dict[str, Any], event_type: str,
                              duration: float = 0.1) -> TemporalEvent:
    ...
    def _estimate_subjective_duration(self, objective_duration: float, 
    ...
    def _update_present_moment(self, new_event: TemporalEvent) -> None:
        ...
        self.present_moment_events = [
            event_id for event_id in self.present_moment_events
            if (event_id in self.temporal_events and
                current_time - self.temporal_events[event_id].onset_time <= self.present_moment_window)
        ]
        ...
        if len(self.present_moment_events) > 5:
            ...
            self.present_moment_events.sort(
                key=lambda eid: self.temporal_events[eid].temporal_salience if eid in self.temporal_events else 0,
                reverse=True
            )
    ...
    def _attempt_temporal_binding(self, new_event: TemporalEvent) -> None:
        ...
        for recent_event in recent_events:
            ...
            if binding_strength > 0.5:
                ...
                self.logger.debug(f"Temporal binding formed: {recent_event.event_id} <-> {new_event.event_id}")
    
    def _calculate_temporal_binding_strength(self, event1: TemporalEvent, 
    ...
    def create_temporal_window(self, start_time: float, duration: float) -> TemporalWindow:
        ...
        for event_id, event in self.temporal_events.items():
            if (start_time <= event.onset_time <= end_time or
                start_time <= event.get_offset_time() <= end_time or
                (event.onset_time <= start_time and event.get_offset_time() >= end_time)):
        ...
        for i, event_id1 in enumerate(event_ids):
            for event_id2 in event_ids[i+1:]:
                if ((event_id1, event_id2) in self.active_temporal_bindings or
                    (event_id2, event_id1) in self.active_temporal_bindings):
                    window_bindings += 1
    ...
    def focus_temporal_attention(self, target_time: float, window_size: Optional[float] = None) -> None:
        ...
        for event in self.temporal_events.values():
            ...
            if time_distance <= window_size / 2:
                ...
                event.temporal_salience = min(1.0, event.temporal_salience * (1.0 + enhancement * 0.3))
    ...
    def get_temporal_flow_state(self) -> Dict[str, Any]:
        ...
        present_moment_richness = len(self.present_moment_events) / 5.0  # Normalized to max 5 events
        
        # Temporal binding activity
        recent_bindings = sum(1 for binding in self.active_temporal_bindings 
                            if any(self.temporal_events.get(event_id, {}).get('onset_time', 0) > current_time - 5.0
    ...
    def update(self) -> None:
        ...
        # Clean up old events
        old_events = [
            event_id for event_id, event in self.temporal_events.items()
    ...
    def _update_subjective_time_rate(self) -> None:
        ...
        if recent_events:
            ...
            alpha = 0.1
            self.subjective_time_rate = alpha * target_rate + (1 - alpha) * self.subjective_time_rate
        else:
    ...
    def get_current_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_events': len([e for e in self.temporal_events.values() 
                                if current_time - e.onset_time <= 60.0]),
            'present_moment_events': len(self.present_moment_events),
            'active_temporal_bindings': len(self.active_temporal_bindings),
