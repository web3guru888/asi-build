...
class AttentionSchemaTheory(BaseConsciousness):
    ...
    def _initialize(self):
        ...
        for process_id in self.attention_processes:
            schema = AttentionSchema(
                schema_id=f"schema_{process_id}", modeled_process=process_id, confidence=0.5
            )
        ...
        with self.attention_lock:
            ...
            if len(self.attention_targets) > self.max_simultaneous_targets * 2:
                ...
                sorted_targets = sorted(
                    self.attention_targets.items(), key=lambda x: (x[1].salience, x[1].timestamp)
                )
                ...
                if process.current_target:
                    ...
                    if target:
                        ...
                        if shift_probability < 0.5:
                            ...
                            alternatives = [
                                (tid, t)
                                for tid, t in self.attention_targets.items()
                                if tid != process.current_target
                                and t.salience > self.salience_threshold
                            ]
                            if alternatives:
    ...
    def _calculate_shift_probability(
        self, process: AttentionProcess, target: AttentionTarget
    ) -> float:
    ...
    def generate_awareness(self) -> AwarenessState:
        ...
        for schema in self.attention_schemas.values():
            if schema.predicted_target and schema.confidence > 0.5:
                ...
                awareness.awareness_strength[target_id] = (
                    schema.confidence * schema.predicted_strength
                )
