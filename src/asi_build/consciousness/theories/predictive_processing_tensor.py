...
class PredictiveProcessingMetrics:
    ...
    def _initialize_predictive_networks(self):
        ...
        self.temporal_predictor = nn.LSTM(
            input_size=512, hidden_size=256, num_layers=2, batch_first=True, dropout=0.2
        ).to(self.device)

        # Temporal prediction head
        self.temporal_prediction_head = nn.Sequential(
            nn.Linear(256, 128), nn.ReLU(), nn.Linear(128, 512)
        ).to(self.device)
    ...
    def compute_prediction_metrics(
        ...
        try:
            ...
            if prediction_metrics_sequence:
                ...
                prediction_errors = [
                    1.0 - m.predictive_accuracy for m in prediction_metrics_sequence
                ]
    ...
    def _adapt_network_sizes(self, input_size: int):
        ...
        if hasattr(self.predictive_models[0][0], "in_features"):
            if self.predictive_models[0][0].in_features != input_size:
                ...
                self.predictive_models[0][0] = nn.Linear(input_size, input_size // 2).to(
                    self.device
                )
                self.predictive_models[0][2] = nn.Linear(input_size // 2, input_size // 4).to(
                    self.device
                )
                self.predictive_models[0][4] = nn.Linear(input_size // 4, input_size).to(
                    self.device
                )
    ...
    def _build_predictive_hierarchy(
        self, current_state: torch.Tensor, previous_states: torch.Tensor
    ) -> PredictiveHierarchy:
        """
        ...
        for level in range(self.num_levels):
            ...
            precision = self.error_precision_estimators[level](torch.abs(prediction_error)).squeeze(
                -1
            )  # [batch]
    ...
    def _compute_hierarchical_consistency(self, levels: List[PredictionLevel]) -> float:
        ...
        for i in range(len(levels) - 1):
            ...
            if (
                current_level.prediction.shape[0] >= next_level.prediction.shape[0]
                and next_level.prediction.shape[0] > 0
            ):

                # Downsample current level to match next level
                current_downsampled = F.adaptive_avg_pool1d(
                    current_level.prediction.unsqueeze(0), next_level.prediction.shape[0]
                ).squeeze(0)

                # Compute similarity
                similarity = F.cosine_similarity(
                    current_downsampled.unsqueeze(0), next_level.prediction.unsqueeze(0), dim=1
                ).item()
    ...
    def _compute_active_inference_score(
        self,
        ...
        try:
            ...
            controller_input = torch.cat(
                [current_state.mean(dim=0), precision_weights], dim=0  # Average across batch
            )
            ...
            if previous_states.shape[1] > 1:
                ...
                prediction_improvement = torch.mean(
                    torch.abs(controlled_state - current_state.mean(dim=0))
                ).item()
    ...
    def _compute_precision_weighted_error(self, levels: List[PredictionLevel]) -> float:
        ...
        for level in levels:
            ...
            precision_weight = (
                level.precision.item()
                if level.precision.numel() == 1
                else torch.mean(level.precision).item()
            )
    ...
    def _compute_temporal_coherence(
        self, previous_states: torch.Tensor, current_state: torch.Tensor
    ) -> float:
        ...
        try:
            ...
            temporal_prediction = self.temporal_prediction_head(
                lstm_output[:, -1, :]
            )  # Last timestep

            # Compare with actual current state
            prediction_error = torch.mean(
                torch.abs(temporal_prediction - current_state), dim=1
            )  # [batch]
    ...
    def _compute_consciousness_metrics(
        ...
        surprise_minimization = self._compute_surprise_minimization(hierarchy, current_state)
    ...
    def _compute_precision_modulation_efficacy(self, hierarchy: PredictiveHierarchy) -> float:
        ...
        errors = [
            torch.mean(torch.abs(level.prediction_error)).item() for level in hierarchy.levels
        ]
    ...
    def _compute_surprise_minimization(
        self, hierarchy: PredictiveHierarchy, current_state: torch.Tensor
    ) -> float:
    ...
    def _compute_temporal_consistency(
        self, metrics_sequence: List[ConsciousPredictionMetrics]
    ) -> float:
    ...
    def analyze_prediction_dynamics(
        self, activations_sequence: List[torch.Tensor]
    ) -> Dict[str, float]:
        """
        ...
        dynamics = {
            ...
            "error_trend": (
                float(np.polyfit(range(len(prediction_errors)), prediction_errors, 1)[0])
                if len(prediction_errors) > 1
                else 0.0
            ),
            "predictive_consciousness_score": float(
                1.0 - np.mean(error_array)
        ...
        if active_inference_scores:
            ...
            dynamics["active_inference_progression"] = (
                float(np.mean(np.diff(active_inference_scores)))
                if len(active_inference_scores) > 1
                else 0.0
            )

        return dynamics

    def measure_predictive_depth(
        self, neural_activations: torch.Tensor, prediction_horizons: List[int] = [1, 2, 3, 5]
    ) -> Dict[str, float]:
        """
            ...
            for horizon in prediction_horizons:
                if seq_len > horizon + self.temporal_depth:
                    ...
                    if horizon_errors:
                        ...
                        depth_metrics[f"prediction_accuracy_{horizon}_steps"] = 1.0 / (
                            1.0 + np.mean(horizon_errors)
                        )
                    else:
    ...
    def assess_free_energy_minimization(self, neural_activations: torch.Tensor) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if free_energies:
                ...
                free_energy_metrics["free_energy_trend"] = (
                    float(np.polyfit(range(len(free_energies)), free_energies, 1)[0])
                    if len(free_energies) > 1
                    else 0.0
                )
                free_energy_metrics["free_energy_minimization"] = float(
                    1.0 / (1.0 + np.mean(free_energies))
                )
                free_energy_metrics["free_energy_stability"] = float(
                    1.0 / (1.0 + np.std(free_energies))
                )
            else:
                free_energy_metrics = {
