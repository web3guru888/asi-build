...
class ByzantineRobustAggregator(BaseAggregator):
    ...
    def __init__(self, aggregator_id: str = "byzantine_robust", config: Dict[str, Any] = None):
        ...
        self.logger.info(f"Byzantine-robust aggregator initialized with method: {self.defense_method}")
    ...
    def aggregate(self, client_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enable_anomaly_detection:
                byzantine_scores = self._detect_byzantine_clients(
                    client_weights_list, client_ids
                )
            else:
    ...
    def _detect_byzantine_clients(self, client_weights_list: List[List[np.ndarray]],
                                client_ids: List[str]) -> List[float]:
        ...
        for i, client_weights in enumerate(flattened_weights):
            ...
            # Combine scores
            byzantine_score = (
                0.4 * distance_from_median + 
                0.3 * max_z_score + 
                0.3 * mahalanobis_dist
            )
        ...
        for i, (client_id, score) in enumerate(zip(client_ids, byzantine_scores)):
            if score > self.anomaly_threshold:
                self.logger.warning(f"Potential Byzantine client detected: {client_id} (score: {score:.3f})")
        
        return byzantine_scores
    
    def _krum_aggregation(self, client_weights_list: List[List[np.ndarray]],
                         data_sizes: List[int], 
                         client_ids: List[str]) -> Tuple[List[np.ndarray], Dict[str, Any]]:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                dist = self._compute_weight_distance(
                    client_weights_list[i], client_weights_list[j]
                )
                distances[i, j] = dist
                distances[j, i] = dist
    ...
    def _multi_krum_aggregation(self, client_weights_list: List[List[np.ndarray]],
                              data_sizes: List[int],
                              client_ids: List[str]) -> Tuple[List[np.ndarray], Dict[str, Any]]:
        ...
        for size in selected_data_sizes:
            aggregation_weights.append(size / total_samples if total_samples > 0 else 1.0 / len(selected_weights))
    ...
    def _trimmed_mean_aggregation(self, client_weights_list: List[List[np.ndarray]],
                                data_sizes: List[int]) -> Tuple[List[np.ndarray], Dict[str, Any]]:
        ...
        for layer_idx in range(num_layers):
            ...
            for coord_idx in range(flat_weights.shape[1]):
                ...
                trimmed_values = coord_values_sorted[trim_count:-trim_count] if trim_count > 0 else coord_values_sorted
    ...
    def _coordinate_wise_median(self, client_weights_list: List[List[np.ndarray]],
                              data_sizes: List[int]) -> Tuple[List[np.ndarray], Dict[str, Any]]:
    ...
    def _clustering_based_aggregation(self, client_weights_list: List[List[np.ndarray]],
                                    data_sizes: List[int],
                                    client_ids: List[str]) -> Tuple[List[np.ndarray], Dict[str, Any]]:
        ...
        if largest_cluster_label != -1:
            ...
            if sum(cluster_data_sizes) > 0:
                aggregation_weights = [size / sum(cluster_data_sizes) for size in cluster_data_sizes]
            else:
            ...
            aggregation_weights = self.compute_client_weights([{"data_size": size} for size in data_sizes])
    ...
    def _faba_aggregation(self, client_weights_list: List[List[np.ndarray]],
                         data_sizes: List[int],
                         client_ids: List[str]) -> Tuple[List[np.ndarray], Dict[str, Any]]:
        ...
        if sum(selected_data_sizes) > 0:
            final_aggregation_weights = [size / sum(selected_data_sizes) for size in selected_data_sizes]
        else:
        ...
        defense_info = {
            ...
            "removed_clients": [client_ids[i] for i in range(len(client_ids)) if i not in selected_indices],
    ...
    def _compute_weight_distance(self, weights1: List[np.ndarray], 
    ...
    def _record_defense_statistics(self, result: Dict[str, Any], 
                                 client_ids: List[str],
        ...
        defense_record = {
            ...
            "high_risk_clients": [
                client_ids[i] for i, score in enumerate(byzantine_scores)
                if score > self.anomaly_threshold
            ]
        }
    ...
    def get_byzantine_defense_stats(self) -> Dict[str, Any]:
        ...
        defense_stats = {
            ...
            "avg_byzantine_score": np.mean([
                np.mean(record["byzantine_scores"]) 
                for record in self.defense_history
            ]) if self.defense_history else 0.0,
            "high_risk_detections": sum([
                len(record["high_risk_clients"]) 
                for record in self.defense_history
            ])
        }
