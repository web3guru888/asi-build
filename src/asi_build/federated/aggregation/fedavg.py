...
class FedAvgAggregator(BaseAggregator):
    ...
    def aggregate(self, client_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            aggregation_weights = self._compute_aggregation_weights(client_updates, data_sizes)

            # Perform weighted averaging
            aggregated_weights = self._weighted_averaging(client_weights_list, aggregation_weights)
    ...
    def _compute_aggregation_weights(
        self, client_updates: List[Dict[str, Any]], data_sizes: List[int]
    ) -> List[float]:
    ...
    def _weighted_averaging(
        self, client_weights_list: List[List[np.ndarray]], aggregation_weights: List[float]
    ) -> List[np.ndarray]:
    ...
    def _apply_server_optimization(self, aggregated_weights: List[np.ndarray]) -> List[np.ndarray]:
        ...
        if self.server_momentum_buffer is None:
            self.server_momentum_buffer = [np.zeros_like(weight) for weight in aggregated_weights]
