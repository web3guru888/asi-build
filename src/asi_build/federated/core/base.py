...
class FederatedModel(abc.ABC):
    ...
    def get_model_size(self) -> Dict[str, int]:
        ...
        trainable_params = sum(
            [tf.reduce_prod(var.shape) for var in self.model.trainable_variables]
        )
...
class FederatedClient(abc.ABC):
    ...
    def get_client_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "config": (
                self.config.to_dict() if hasattr(self.config, "to_dict") else str(self.config)
            ),
        }
...
class FederatedServer(abc.ABC):
    ...
    def register_client(self, client: FederatedClient) -> bool:
        ...
        self.logger.info(
            f"Client {client.client_id} registered. Total clients: {len(self.clients)}"
        )
        return True
    ...
    def start_round(self) -> bool:
        ...
        if len(self.clients) < self.config.min_clients:
            self.logger.warning(
                f"Insufficient clients. Need {self.config.min_clients}, have {len(self.clients)}"
            )
            return False
    ...
    def finish_round(self) -> Dict[str, Any]:
        ...
        if self.client_updates:
            client_losses = [
                update.get("metrics", {}).get("loss", 0) for update in self.client_updates.values()
            ]
            client_accuracies = [
                update.get("metrics", {}).get("accuracy", 0)
        ...
        self.logger.info(
            f"Round {self.round_number} completed in {round_time:.2f}s with {len(self.client_updates)} clients"
        )
    ...
    def check_convergence(self, threshold: float = 0.001) -> bool:
        ...
        recent_losses = [
            m.get("avg_client_loss", float("inf")) for m in self.global_metrics_history[-5:]
        ]
    ...
    def get_server_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_round_time": (
                np.mean(self.server_metrics["round_times"])
                if self.server_metrics["round_times"]
                else 0
            ),
            "config": (
                self.config.to_dict() if hasattr(self.config, "to_dict") else str(self.config)
