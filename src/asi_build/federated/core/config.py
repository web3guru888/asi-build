    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "FederatedConfig":
        ...
        if "server" in config_dict:
            ...
            if "aggregation_type" in server_config:
                server_config["aggregation_type"] = AggregationType(
                    server_config["aggregation_type"]
                )
            if "communication_protocol" in server_config:
                server_config["communication_protocol"] = CommunicationProtocol(
                    server_config["communication_protocol"]
                )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if "server" in config_dict and config_dict["server"]:
            if "aggregation_type" in config_dict["server"]:
                config_dict["server"]["aggregation_type"] = config_dict["server"][
                    "aggregation_type"
                ].value
            if "communication_protocol" in config_dict["server"]:
                config_dict["server"]["communication_protocol"] = config_dict["server"][
                    "communication_protocol"
                ].value
            if "privacy" in config_dict["server"] and config_dict["server"]["privacy"]:
                if "mechanism" in config_dict["server"]["privacy"]:
                    config_dict["server"]["privacy"]["mechanism"] = config_dict["server"][
                        "privacy"
                    ]["mechanism"].value
        ...
        if "client" in config_dict and config_dict["client"]:
            if "privacy" in config_dict["client"] and config_dict["client"]["privacy"]:
                if "mechanism" in config_dict["client"]["privacy"]:
                    config_dict["client"]["privacy"]["mechanism"] = config_dict["client"][
                        "privacy"
                    ]["mechanism"].value
...
SECURE_CONFIG = FederatedConfig(
    ...
    server=ServerConfig(
        privacy=PrivacyConfig(mechanism=PrivacyMechanism.DIFFERENTIAL_PRIVACY, epsilon=1.0),
        security=SecurityConfig(enable_tls=True, authentication_required=True),
    ),
)
...
CROSS_SILO_CONFIG = FederatedConfig(
    ...
    server=ServerConfig(
        ...
        privacy=PrivacyConfig(mechanism=PrivacyMechanism.SECURE_MULTI_PARTY),
    ),
)
