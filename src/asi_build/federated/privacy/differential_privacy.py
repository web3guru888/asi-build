...
class AdaptiveGaussianMechanism(GaussianMechanism):
    ...
    def add_noise(
        self, data: np.ndarray, sensitivity: float, gradient_norm: Optional[float] = None
    ) -> np.ndarray:
...
class DifferentialPrivacyManager:
    ...
    def add_noise_to_gradients(
        self, gradients: List[np.ndarray], gradient_norm: Optional[float] = None
    ) -> List[np.ndarray]:
    ...
    def add_noise_to_weights(
        self, weights: List[np.ndarray], weight_sensitivity: Optional[float] = None
    ) -> List[np.ndarray]:
    ...
    def get_privacy_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget_exhausted": (
                self.total_epsilon_spent > self.epsilon or self.total_delta_spent > self.delta
            ),
        }

    def check_privacy_budget(self) -> bool:
        """Check if privacy budget is still available."""
        return self.total_epsilon_spent <= self.epsilon and self.total_delta_spent <= self.delta
    ...
    def estimate_remaining_queries(self) -> int:
        ...
        queries_by_epsilon = (
            epsilon_remaining / epsilon_per_query if epsilon_per_query > 0 else float("inf")
        )
        queries_by_delta = (
            delta_remaining / delta_per_query if delta_per_query > 0 else float("inf")
        )
...
class RenyiDifferentialPrivacy:
    ...
    def compute_rdp(
        self, noise_multiplier: float, num_steps: int, sampling_probability: float = 1.0
    ) -> List[float]:
        ...
        for alpha in self.rdp_orders:
            if alpha == 1:
                ...
                rdp_alpha = alpha * sampling_probability**2 * num_steps / (2 * noise_multiplier**2)
