    ...
    def personalize_model(
        ...
        for i, (global_w, local_w) in enumerate(zip(global_weights, local_weights)):
            if i in self.personal_layers:
                ...
                mixed_weight = self.mixing_ratio * local_w + (1 - self.mixing_ratio) * global_w
...
class FedPerAvg(BaseAggregator):
    ...
    def __init__(self, aggregator_id: str = "fedperavg", config: Dict[str, Any] = None):
        ...
        self.personalization_strategy = self.config.get(
            "personalization_strategy", "local_finetuning"
        )
    ...
    def _create_personalization_strategy(self) -> PersonalizationStrategy:
        ...
        if self.personalization_strategy == "local_finetuning":
            ...
            return LayerWisePersonalization(
                personal_layers=self.personal_layers, mixing_ratio=self.alpha
            )
    ...
    def personalize_for_client(
        self, client_id: str, global_weights: List[np.ndarray], client_data: tf.data.Dataset
    ) -> List[np.ndarray]:
...
class PersonalizedFederatedLearning:
    ...
    def training_round(self, client_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        results = {"round": self.current_round, "global_training": None, "personalization": None}
        ...
        if self.enable_personalization and self.current_round % self.personalization_frequency == 0:
            ...
            for update in client_updates:
                ...
                if client_data and self.global_model_weights:
                    ...
                    personalization_results.append(
                        {"client_id": client_id, "personalized": True, "timestamp": time.time()}
                    )
            ...
            self.logger.info(
                f"Personalization completed for {len(personalization_results)} clients"
            )
