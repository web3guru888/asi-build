...
class SignalUtilities:
    ...
    @staticmethod
    def notch_filter(data: np.ndarray, notch_freq: float, sampling_rate: float, 
    ...
    @staticmethod
    def compute_psd(data: np.ndarray, sampling_rate: float, 
    ...
    def interpolate_bad_samples(data: np.ndarray, bad_mask: np.ndarray) -> np.ndarray:
        ...
        for idx in bad_indices:
            ...
            if left_idx >= 0 and right_idx < len(data):
                clean_data[idx] = np.interp(idx, [left_idx, right_idx], 
