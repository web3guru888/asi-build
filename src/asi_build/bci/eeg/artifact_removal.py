...
class ArtifactRemover:
    ...
    def _remove_line_noise(self, data: np.ndarray) -> np.ndarray:
        ...
        line_freqs = (
            [50, 100, 150] if self.config.signal_processing.notch_freq == 50 else [60, 120, 180]
        )
    ...
    def _adaptive_filtering(self, data: np.ndarray) -> np.ndarray:
        ...
        for ch in range(self.n_channels):
            ...
            clean_data[ch, :] = self._adaptive_moving_average(data[ch, :], window_size)
            ...
            for idx in outlier_indices:
                ...
                if left_idx >= 0 and right_idx < len(ch_data):
                    clean_data[ch, idx] = np.interp(
                        idx, [left_idx, right_idx], [ch_data[left_idx], ch_data[right_idx]]
                    )
                elif left_idx >= 0:
    ...
    def train_ica_model(self, training_data: np.ndarray) -> bool:
        ...
        try:
            ...
            self.ica_model = FastICA(
                n_components=n_components, whiten="unit-variance", random_state=42, max_iter=1000
            )
        ...
        try:
            ...
            for comp_idx, component in enumerate(ica_data):
                ...
                if len(autocorr) > cardiac_samples:
                    periodic_strength = (
                        np.max(autocorr[cardiac_samples // 2 : cardiac_samples]) / autocorr[0]
                    )
        ...
        try:
            ...
            if data_sample is not None:
                ...
                for i in range(n_components):
                    ...
                    axes[i].set_title(
                        f"IC {i+1}" + (" (Artifact)" if i in self.artifact_components else "")
                    )
    ...
    def load_model(self, filepath: str) -> bool:
        ...
        try:
            ...
            if (
                model_data["channels"] == self.channels
                and model_data["sampling_rate"] == self.sampling_rate
            ):
