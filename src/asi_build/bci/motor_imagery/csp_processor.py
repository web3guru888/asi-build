...
class CSPProcessor(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        if filter_bank:
            ...
            self.frequency_bands = [mu_band, beta_band, (mu_band[0], beta_band[1])]  # Combined
        ...
        self.logger.info(
            f"CSP Processor initialized with {len(self.frequency_bands)} frequency bands"
        )
    ...
    def _apply_frequency_filter(
        self, X: np.ndarray, low_freq: float, high_freq: float
    ) -> np.ndarray:
    ...
    def select_best_frequency_bands(self, n_bands: int = 3) -> List[int]:
        ...
        self.logger.info(
            f"Selected best frequency bands: {[self.frequency_bands[i] for i in best_bands]}"
        )
