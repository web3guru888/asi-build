...
class SSVEPDetector:
    ...
    def __init__(self, config: BCIConfig):
        ...
        self.logger.info(
            f"SSVEP Detector initialized for frequencies: {self.target_frequencies} Hz"
        )
    ...
    def _fbcca_detection(self, data: np.ndarray) -> Dict[float, float]:
        ...
        for freq in self.target_frequencies:
            ...
            for low_freq, high_freq in self.filter_bank:
                ...
                if low_freq <= freq <= high_freq or any(
                    low_freq <= freq * h <= high_freq for h in self.harmonics
                ):
    ...
    def _combine_method_scores(
        self, method_scores: Dict[str, Dict[float, float]]
    ) -> Dict[float, float]:
        """Combine scores from different methods"""
        combined_scores = {}

        # Weight for each method
        method_weights = {"psd": 0.2, "cca": 0.3, "fbcca": 0.4, "msi": 0.1}
            ...
            for harmonic in self.harmonics:
                ...
                if harmonic_freq < self.sampling_rate / 2:
                    ...
                    neighbor_powers = np.concatenate(
                        [psd[start_idx:freq_idx], psd[freq_idx + 1 : end_idx]]
                    )
    ...
    def calibrate(
        self, calibration_data: List[Tuple[np.ndarray, float]], method: str = "cca"
    ) -> Dict[str, Any]:
        """
