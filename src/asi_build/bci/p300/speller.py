...
class P300Speller:
        ...
        if prediction.predicted_class == 'target':
            ...
            if 'char' in stimulus_id:
                ...
                if len(parts) >= 3:
                    ...
                    if (0 <= row_idx < self.matrix_size[0] and 
                        0 <= col_idx < self.matrix_size[1]):
    ...
    async def calibrate(self, 
                       ...
                       trials_per_character: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            for char_idx, target_char in enumerate(target_characters):
                self.logger.info(f"Calibrating character {char_idx + 1}/{len(target_characters)}: '{target_char}'")
            ...
            self.logger.info(f"P300 calibration completed: {training_result.get('accuracy', 0.0):.3f} accuracy")
    ...
    async def _run_calibration_trials(self, 
                                    target_char: str,
                                    target_position: Tuple[int, int],
                                    n_trials: int) -> List[P300Trial]:
