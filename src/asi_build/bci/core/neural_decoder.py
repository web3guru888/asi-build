...
class NeuralDecoder:
    ...
    def _initialize_models(self):
        ...
        # SVM for P300
        self.models["p300"] = SVC(probability=True, kernel="rbf", C=1.0, random_state=42)
    ...
    def _create_ensemble_classifier(
        self, base_classifiers: List[Tuple[str, Any]]
    ) -> VotingClassifier:
        ...
        return VotingClassifier(
            ...
            voting=(
                "soft"
                if all(hasattr(clf[1], "predict_proba") for clf in base_classifiers)
                else "hard"
            ),
        )
    ...
    def _extract_task_features(
        self, processed_signal: ProcessedSignal, task: str
    ...
    async def train_classifier(self, task: str, training_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X,
                y,
                test_size=self.config.classification.validation_split,
                random_state=42,
                stratify=y,
            )
            ...
            cv_scores = cross_val_score(
                model, X_train_scaled, y_train, cv=self.config.classification.cross_validation_folds
            )
    ...
    def _prepare_training_data(
        self, training_data: List[Dict], task: str
    ...
    def _extract_task_features_from_dict(
        self, features: Dict[str, Any], task: str
    ) -> Dict[str, Any]:
        ...
        if task == "motor_imagery":
            ...
            return {k: v for k, v in features.items() if any(ch in k for ch in ["O1", "O2", "Oz"])}
        else:
            return features
