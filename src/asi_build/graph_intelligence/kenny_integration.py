...
class KennyAutomationBridge:
    ...
    async def execute_intelligent_automation(
        ...
        try:
            ...
            logger.info(
                f"✅ Intelligent automation prepared: {len(execution_plan)} steps planned "
    ...
    def _convert_reasoning_to_plan(
        self, reasoning_result: FastToGResult, context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _convert_action_to_kenny_step(
        self, action: Dict[str, Any], step_number: int, context: Dict[str, Any]
        ...
        if action_type == "click":
            ...
            return {
                ...
                "fallback_strategies": [
                    {
                        "method": "coordinate_click_then_type",
                        "coordinates": action.get("coordinates", []),
                    }
                ],
            }
                ...
                "fallback_strategies": [
                    {
                        ...
                        "shortcut": self._infer_shortcut(action.get("element_text", "")),
                    }
                ],
            }
            ...
            return {
                ...
                "fallback_strategies": [{"method": "step_by_step_execution"}],
            }
    ...
    async def execute_automation_plan(self, task_id: str) -> bool:
        ...
        try:
                ...
                if step_success:
                    ...
                    if step.get("fallback_strategies"):
                        for fallback in step["fallback_strategies"]:
                            ...
                            fallback_success = await self._execute_fallback_strategy(
                                fallback, step, task
                            )
                            if fallback_success:
                                success_count += 1
            ...
            task.success_rate = (
                success_count / len(task.execution_plan) if task.execution_plan else 0.0
            )
            ...
            if task.success_rate >= 0.8:  # 80% success threshold
                ...
                logger.info(
                    f"🎉 Automation completed successfully: {task.success_rate:.1%} success rate"
                )
                return True
            else:
                task.status = AutomationStatus.FAILED
    ...
    async def _execute_automation_step(self, step: Dict[str, Any], task: AutomationTask) -> bool:
        ...
        if not success:
            error_msg = (
                f"Step {step['step_number']} failed: {step.get('reasoning', 'Unknown error')}"
            )
    ...
    async def _execute_fallback_strategy(
        self, fallback: Dict[str, Any], original_step: Dict[str, Any], task: AutomationTask
    ) -> bool:
    ...
    def ingest_automation_feedback(
        self, task_id: str, screen_data: Dict[str, Any], result_data: Dict[str, Any]
    ) -> IngestionResult:
        """
        ...
        ingestion_context = {
            ...
            "communities_used": (
                task.reasoning_result.communities_analyzed if task.reasoning_result else 0
            ),
            "automation_result": result_data.get("success", False),
            "execution_time": (
                task.completion_time - task.start_time if task.completion_time > 0 else 0
            ),
        }
        ...
        if ocr_data:
            ...
            if task.success_rate >= 0.8 and task.reasoning_result:
                workflow_data = {
                    ...
                    "steps": [
                        step for step in task.execution_plan if step["step_type"] != "validate"
                    ],
                    "status": "completed",
                    "success_rate": task.success_rate,
                    "avg_duration": (
            ...
            logger.info(
                f"✅ Ingested automation feedback: {result.nodes_created} nodes, "
    ...
    def get_integration_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_action_generation_time": sum(m.action_generation_time for m in metrics)
            / len(metrics),
            "avg_total_integration_time": sum(m.total_integration_time for m in metrics)
            / len(metrics),
            ...
            "completed_tasks": len(
                [t for t in self.active_tasks.values() if t.status == AutomationStatus.COMPLETED]
            ),
            "failed_tasks": len(
                [t for t in self.active_tasks.values() if t.status == AutomationStatus.FAILED]
            ),
        }
    ...
    def cleanup_completed_tasks(self, max_age_hours: int = 24):
        ...
        for task_id, task in self.active_tasks.items():
            if (
                task.status in [AutomationStatus.COMPLETED, AutomationStatus.FAILED]
