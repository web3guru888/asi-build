...
try:
    from kenny_intelligence import (
        ...
        ActionExecutor, KennyIntelligence, Context, ActionType
    )
except ImportError:
...
def test_end_to_end_intelligence():
    ...
    try:
        ...
        with manager.driver.session() as session:
            ...
            for i in range(3):
                ...
                """, id=elem_id, text=f"Button {i}")
                
                # Link to workflow
                session.run("""
                    MATCH (w:Workflow {id: $wid}), (e:UIElement {id: $eid})
                    CREATE (w)-[:CONTAINS]->(e)
                """, wid=workflow_id, eid=elem_id)
    ...
    try:
        ...
        with driver.session() as session:
            ...
            """, id=pattern_id, sequence=json.dumps(["click", "type", "submit"]))
