...
class TestDataGenerator:
    ...
    def generate_workflows(self, community_ids: List[str], count: int = 10) -> List[str]:
        ...
        for i in range(count):
            ...
            related_communities = random.sample(
                community_ids, min(random.randint(1, 3), len(community_ids))
            )
        ...
        for i in range(count):
            error = ErrorNode(
                ...
                resolution_strategy=random.choice(
                    ["retry", "alternative_path", "user_intervention", "skip"]
                ),
            )
    ...
    def generate_relationships(
        self, all_node_ids: Dict[str, List[str]], relationship_count: int = 50
    ) -> int:
