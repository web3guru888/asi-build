...
class ModularityPruner:
    ...
    def prune_communities(
        self, communities: List[Dict[str, Any]], context: Dict[str, Any] = None
        ...
        logger.info(
            f"✅ Modularity pruning: {len(communities)} → {len(pruned_communities)} communities "
    ...
    def _calculate_community_score(
        self, community: Dict[str, Any], context: Dict[str, Any] = None
    ) -> CommunityScore:
...
class LLMFinePruner:
    ...
    def __init__(self, max_communities_for_llm: int = 20, semantic_threshold: float = 0.7):
    ...
    def prune_communities(
        ...
        if not communities or len(communities) <= self.max_communities_for_llm:
            logger.info(
                f"🔍 Skipping LLM pruning: {len(communities)} communities (under threshold)"
            )
        ...
        selected_indices = self._simulate_llm_selection(community_descriptions, task_description)
        ...
        pruning_stats = {
            ...
            "pruning_ratio": (
                1 - (len(selected_communities) / len(communities)) if communities else 0
            ),
            "method": "llm_semantic",
        ...
        logger.info(
            f"✅ LLM fine pruning: {len(communities)} → {len(selected_communities)} communities "
            f"({pruning_stats['pruning_ratio']:.2%} pruned)"
        )

        return selected_communities, pruning_stats

    def _create_community_description(
        self, community: Dict[str, Any], schema_manager: SchemaManager
    ) -> Dict[str, Any]:
    ...
    def _create_evaluation_prompt(
        self,
        community_descriptions: List[Dict[str, Any]],
...
"""
    ...
    def _simulate_llm_selection(
        self, community_descriptions: List[Dict[str, Any]], task_description: str
    ) -> List[int]:
        """
    ...
    def _calculate_semantic_score(
        self, community_desc: Dict[str, Any], task_description: str
    ) -> float:
...
class CommunityPruningSystem:
    ...
    def prune_communities_for_task(
        self, task_context: Dict[str, Any], target_community_count: int = 10
    ) -> PruningResult:
        """
        ...
        coverage_retained = self._calculate_coverage_retention(community_dicts, final_pruned)
        ...
        logger.info(
            f"✅ Two-stage pruning completed: {original_count} → {final_count} communities "
    ...
    def get_relevant_communities(
        self, task_context: Dict[str, Any], max_communities: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        relevant_communities.sort(
            key=lambda c: self.modularity_pruner._calculate_community_score(
                c, task_context
            ).combined_score,
            reverse=True,
        )
    ...
    def _calculate_coverage_retention(
        self, original_communities: List[Dict[str, Any]], pruned_communities: List[Dict[str, Any]]
    ) -> float:
    ...
    def _store_pruning_results(
        ...
        logger.debug(
            f"Pruning results: {result.original_communities} → {result.pruned_communities} "
...
if __name__ == "__main__":
    ...
    with SchemaManager() as sm:
        ...
        task_context = {
            ...
            "application": "notepad",
            "ui_elements": [{"text": "Save", "type": "button"}, {"text": "File", "type": "menu"}],
