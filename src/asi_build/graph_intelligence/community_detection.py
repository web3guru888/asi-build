...
from .schema import NodeType, RelationshipType, CommunityNode, create_community
...
class LouvainCommunityDetection:
    ...
    def detect_communities(self, graph: nx.Graph) -> Tuple[Dict[str, int], float]:
        ...
        logger.info(f"Louvain completed in {iteration} iterations, modularity: {current_modularity:.3f}")
    ...
    def _calculate_modularity_gain(self, graph: nx.Graph, node: str, 
                                  old_community: int, new_community: int,
...
class GirvanNewmanCommunityDetection:
    ...
    def detect_communities(self, graph: nx.Graph) -> Tuple[Dict[str, int], float]:
        ...
        logger.info(f"Girvan-Newman found {len(set(best_communities.values()))} communities, "
...
class CommunityDetectionEngine:
    ...
    def detect_all_communities(self, algorithm: str = "louvain") -> CommunityDetectionResult:
        ...
        if len(graph.nodes()) == 0:
            ...
            return CommunityDetectionResult(
                communities=[], algorithm=algorithm, modularity=0.0,
                processing_time=0.0, node_count=0, edge_count=0,
                community_count=0, quality_metrics={}
            )
        ...
        logger.info(f"✅ Community detection completed: {len(stored_communities)} communities "
    ...
    def _calculate_quality_metrics(self, graph: nx.Graph, 
                                  communities: Dict[str, int]) -> CommunityQualityMetrics:
