...
class SchemaManager:
    ...
    def update_node(self, node_id: str, properties: Dict[str, Any], 
        ...
        with self.driver.session() as session:
            ...
            if node_type:
                safe_label = _sanitize_label(node_type.value)
                cypher = f"MATCH (n:{safe_label} {{id: $node_id}}) SET {set_clause} RETURN n.id as id"
            else:
    ...
    def find_relationships(self, from_node: Optional[str] = None, 
        ...
        with self.driver.session() as session:
            ...
            cypher += " RETURN a.id as from_id, type(r) as rel_type, b.id as to_id, r LIMIT $query_limit"
