...
class RelationshipDetector:
    ...
    def _is_contained(self, inner_coords: List[int], outer_coords: List[int]) -> bool:
        ...
        return (
            outer_coords[0] - self.containment_threshold <= inner_coords[0]
            and outer_coords[1] - self.containment_threshold <= inner_coords[1]
        )
...
class DataIngestionPipeline:
    ...
    def ingest_ocr_results(
        self, ocr_data: List[Dict[str, Any]], context: Dict[str, Any] = None
    ) -> IngestionResult:
        ...
        try:
            ...
            logger.info(
                f"✅ OCR ingestion completed: {result.nodes_created} nodes, "
    ...
    def _create_screen_node(self, ocr_data: List[Dict[str, Any]], context: Dict[str, Any]) -> str:
    ...
    def _process_ocr_element(
        self, ocr_data: Dict[str, Any], context: Dict[str, Any], screen_id: str
        ...
        try:
            ...
            ocr_element = OCRElement(text=text, bbox=bbox, confidence=confidence)
            ...
            ui_element = create_ui_element(
                ...
                screen_id=screen_id,
                properties={"bbox": bbox, "processing_method": "ocr", "timestamp": time.time()},
            )
    ...
    def batch_ingest(
        self, data_batch: List[Dict[str, Any]], batch_type: str = "mixed"
    ) -> IngestionResult:
            ...
            try:
                if batch_type == "ocr" or data_item.get("type") == "ocr":
                    result = self.ingest_ocr_results(
                        data_item.get("ocr_data", []), data_item.get("context", {})
                    )
        ...
        logger.info(
            f"✅ Batch ingestion completed: {total_result.nodes_created} nodes, "
    ...
    def validate_ingestion(self) -> Dict[str, Any]:
        ...
        try:
            ...
            orphaned_ui_elements = len(
                self.sm.find_nodes(NodeType.UI_ELEMENT, filters={"screen_id": ""})
            )
...
if __name__ == "__main__":
    ...
    with SchemaManager() as sm:
        ...
        print(
            f"✅ OCR ingestion: {result.nodes_created} nodes, {result.relationships_created} relationships"
        )
