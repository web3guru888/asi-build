...
class CommunityReasoningEngine:
    ...
    async def reason_about_community(
        self, community_id: str, task_context: Dict[str, Any]
    ) -> CommunityReasoning:
        """
        ...
        try:
            conversion_result = self.text_generator.graph2text.convert_community_to_text(
                community_id
            )
    ...
    def _calculate_community_relevance(
        self, community: Dict[str, Any], task_context: Dict[str, Any], conversion_result
    ) -> float:
        ...
        relevance = 0.4 * purpose_score + 0.3 * content_score + 0.2 * modularity + 0.1 * frequency

        return min(1.0, relevance)

    def _generate_community_reasoning(
        self, community: Dict[str, Any], task_context: Dict[str, Any], conversion_result
    ) -> str:
        ...
        if conversion_result.node_count > 0:
            reasoning_parts.append(
                f"The community contains {conversion_result.node_count} connected elements "
                f"with {conversion_result.relationship_count} relationships."
            )

            if conversion_result.complexity_score > 2.0:
                reasoning_parts.append(
                    "This indicates a complex interaction pattern that may require careful sequencing."
                )
            elif conversion_result.complexity_score > 1.0:
                reasoning_parts.append(
                    "The interaction pattern is moderately complex with multiple pathways."
                )
            else:
                reasoning_parts.append(
                    "The interaction pattern is straightforward with clear pathways."
                )
        ...
        if frequency > 50:
            reasoning_parts.append(
                "This community is frequently used, indicating high reliability."
            )
        elif frequency > 10:
            reasoning_parts.append(
                "This community has moderate usage, suggesting proven functionality."
            )
        else:
    ...
    def _calculate_reasoning_confidence(
        ...
        if conversion_result.node_count > 0 and conversion_result.relationship_count > 0:
            completeness = min(
                1.0, (conversion_result.node_count + conversion_result.relationship_count) / 10.0
            )
...
class FastToGReasoningEngine:
    ...
    async def reason(self, request: ReasoningRequest) -> FastToGResult:
        ...
        try:
            ...
            logger.info(
                f"✅ FastToG reasoning completed in {result.processing_time:.2f}s: "
    ...
    async def _community_based_reasoning(
        self, request: ReasoningRequest, request_id: str
    ) -> FastToGResult:
        ...
        pruning_result = self.pruning_system.prune_communities_for_task(
            request.context, target_community_count=request.max_communities
        )

        relevant_communities = self.pruning_system.get_relevant_communities(
            request.context, max_communities=request.max_communities
        )
        ...
        for community in relevant_communities:
            task = self.community_engine.reason_about_community(community["id"], request.context)
        ...
        final_recommendation = self._synthesize_recommendations(valid_reasonings, request.context)
        ...
        explanation = (
            self._generate_explanation(request, valid_reasonings, final_recommendation)
        ...
        performance_metrics = {
            ...
            "avg_community_confidence": (
                sum(cr.confidence for cr in valid_reasonings) / len(valid_reasonings)
                if valid_reasonings
                else 0
            ),
    ...
    async def _traditional_reasoning(
        self, request: ReasoningRequest, request_id: str
    ) -> FastToGResult:
        ...
        return FastToGResult(
            ...
            performance_metrics={
                ...
                "elements_analyzed": len(top_elements),
            },
            explanation="Traditional element-by-element analysis",
        )

    async def _hybrid_reasoning(self, request: ReasoningRequest, request_id: str) -> FastToGResult:
        ...
        # Combine results
        all_reasonings = (
            community_result.community_reasonings + traditional_result.community_reasonings
        )
        ...
        overall_confidence = (
            community_result.overall_confidence + traditional_result.overall_confidence
        ) / 2
    ...
    def _synthesize_recommendations(
        self, reasonings: List[CommunityReasoning], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not reasonings:
            return {
                "approach": "none",
                "recommended_actions": [],
                "reasoning": "No valid reasonings",
            }

        # Sort reasonings by relevance and confidence
        sorted_reasonings = sorted(
            reasonings, key=lambda r: r.relevance_score * r.confidence, reverse=True
        )
        ...
        return {
            ...
            "primary_communities": [
                {
                    "id": cr.community_id,
                    "purpose": cr.community_purpose,
                    "relevance": cr.relevance_score,
                }
        ...
        if reasonings:
            ...
            for i, reasoning in enumerate(reasonings[:3], 1):
                ...
                if reasoning.recommended_actions:
                    explanation_parts.append(
                        f"   Key actions: {len(reasoning.recommended_actions)} recommendations"
                    )
    ...
    def _combine_action_lists(
        self, action_lists: List[List[Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
    ...
    def _create_error_result(
        self, request: ReasoningRequest, request_id: str, error_message: str
    ) -> FastToGResult:
        """Create error result."""
        return FastToGResult(
            request_id=request_id,
    ...
    def _create_no_communities_result(
        self, request: ReasoningRequest, request_id: str
    ) -> FastToGResult:
        ...
        with SchemaManager() as sm:
            ...
            if result.community_reasonings:
                ...
                for i, cr in enumerate(result.community_reasonings[:3], 1):
                    print(
                        f"     {i}. {cr.community_purpose} (rel: {cr.relevance_score:.1%}, conf: {cr.confidence:.1%})"
                    )
