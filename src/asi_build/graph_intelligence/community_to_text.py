...
class Triple2TextConverter:
    ...
    def convert_triple_to_text(self, triple: Triple) -> str:
        ...
        template = self.predicate_templates.get(
            triple.predicate, "{subject} is connected to {object}"
        )
        ...
        # Apply template
        description = template.format(subject=subject_name, object=object_name)
...
class Graph2TextConverter:
    ...
    def generate_community_summary(
        self, community_id: str, include_statistics: bool = True
    ...
    def convert_community_to_text(
        self, community_id: str, include_context: bool = True
    ) -> TextConversionResult:
        """
        ...
        logger.info(
            f"✅ Converted community {community_id[:8]} to text: "
    ...
    def _extract_community_triples(self, community_id: str) -> List[Triple]:
        ...
        try:
            members = (
                json.loads(community["members"])
                if isinstance(community["members"], str)
        ...
        for member_id in members:
            ...
            for rel in outgoing:
                triple = Triple(
                    ...
                    confidence=(
                        rel["properties"].get("confidence", 1.0) if rel["properties"] else 1.0
                    ),
            ...
            for rel in incoming:
                # Avoid duplicates
                if not any(
                    t.subject == rel["from_id"]
                    and t.object == rel["to_id"]
                    and t.predicate == rel["type"]
                    for t in triples
                ):
                    triple = Triple(
                        subject=rel["from_id"],
    ...
    def _generate_graph_description(self, community: Dict[str, Any], triples: List[Triple]) -> str:
        ...
        if triples:
            ...
            if node_connections:
                ...
                description_parts.append(
                    f"The most connected element is {node_name} with {connection_count} connections."
                )

        return " ".join(description_parts)

    def _generate_natural_summary(
        self, community: Dict[str, Any], triples: List[Triple], triple_descriptions: List[str]
    ) -> str:
        ...
        if triple_descriptions:
            ...
            ui_descriptions = [
                d for d in triple_descriptions if "UI element" in d or "button" in d or "menu" in d
            ]
            workflow_descriptions = [d for d in triple_descriptions if "workflow" in d]
            other_descriptions = [
                d
                for d in triple_descriptions
                if d not in ui_descriptions and d not in workflow_descriptions
            ]

            if ui_descriptions:
                summary_parts.append("The UI components involved include:")
                summary_parts.extend(
                    [f"- {desc}" for desc in ui_descriptions[:3]]
        ...
        if frequency > 0:
            usage_desc = (
                "frequently" if frequency > 50 else "occasionally" if frequency > 10 else "rarely"
            )
...
class CommunityTextGenerator:
    ...
    def generate_task_context_description(
        self, community_ids: List[str], task_description: str = ""
    ) -> str:
        """
        ...
        if successful_conversions:
            ...
            avg_complexity = sum(r.complexity_score for r in successful_conversions) / len(
                successful_conversions
            )

            description_parts.append(
                f"Summary: {len(successful_conversions)} communities, "
    ...
    def generate_reasoning_prompt(
        self, community_ids: List[str], task_context: Dict[str, Any]
    ) -> str:
        ...
        context_description = self.generate_task_context_description(
            community_ids, f"User wants to: {user_intent}"
        )
...
if __name__ == "__main__":
    ...
    with SchemaManager() as sm:
        ...
        if community_ids:
            ...
            task_context = {"user_intent": "save a document to file", "application": "notepad"}
            ...
            reasoning_prompt = text_generator.generate_reasoning_prompt(
                community_ids[:2], task_context
            )
