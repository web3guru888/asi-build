...
def demonstrate_optimization():
    ...
    print(
        f"Size (<{targets['size_target_mb']} MB): {'✅ ACHIEVED' if targets['size_achieved'] else '❌ NOT MET'}"
    )
    print(
        f"Latency (<{targets['latency_target_ms']} ms): {'✅ ACHIEVED' if targets['latency_achieved'] else '❌ NOT MET'}"
    )
    print(
        f"Accuracy (>{targets['accuracy_target']*100:.0f}%): {'✅ ACHIEVED' if targets['accuracy_achieved'] else '❌ NOT MET'}"
    )
    print(
        f"ISO 26262: {'✅ COMPLIANT' if targets['iso_26262_compliant'] else '⚠️  PENDING (requires full CARLA testing)'}"
    )
