            ...
            if path.exists():
                ...
                with open(path, "r") as f:
                    ...
                    if "samples" in metadata:
                        ...
                        logger.info(
                            f"Loaded {len(metadata['samples'])} samples from {path.parent.name}"
                        )
...
class VLATrainer:
    ...
    def __init__(self, config: Dict):
        ...
        # Metrics tracking
        self.metrics = {"train_loss": [], "val_loss": [], "learning_rate": [], "epoch_time": []}
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> float:
        ...
        with autocast(enabled=self.use_amp):
            ...
            # Calculate losses
            trajectory_loss = self.mse_loss(outputs["waypoints"], batch["trajectories"])

            safety_loss = self.bce_loss(outputs["safety_score"].squeeze(), batch["safety_label"])
            ...
            for batch in val_loader:
                ...
                with autocast(enabled=self.use_amp):
                    ...
                    safety_loss = self.bce_loss(
                        outputs["safety_score"].squeeze(), batch["safety_label"]
                    )
