...
class ProductionDatasetDownloader:
    ...
    def download_all_datasets(self, priority_filter: Optional[str] = None) -> Dict:
        ...
        sorted_datasets = sorted(
            self.datasets.items(), key=lambda x: priority_order.get(x[1]["priority"], 99)
        )
...
def main():
    ...
    for dataset_name, dataset_status in status["datasets"].items():
        ...
        print(
            f"  Progress: {dataset_status['components_completed']}/{dataset_status['components_total']} components ({dataset_status['percentage']:.1f}%)"
        )
