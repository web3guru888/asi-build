...
from .architecture import VLAConfig, VLAPlusPlus
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            if module.in_features >= 256 and module.out_features >= 256:
                ...
                wrapper = nn.ModuleDict({"original": original, "lora": lora})
...
class VLATrainer:
    ...
    def train(self):
        ...
        logger.info(
            f"Training on {self.config.cudo_instance_type} at ${self.config.cudo_max_price}/hour"
        )
    ...
    def _training_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        with autocast(enabled=self.config.mixed_precision):
            ...
            detection_loss = self.detection_loss(
                ...
                batch.get(
                    "detection_targets",
                    torch.zeros_like(outputs["detections"][:, :, 0], dtype=torch.long),
                ).view(-1),
            )

            trajectory_loss = self.trajectory_loss(outputs["waypoints"], batch["trajectories"])

            safety_loss = self.safety_loss(outputs["safety_score"].squeeze(), batch["safety_label"])
...
def train_vla_plus_plus():
    ...
    logger.info(
        f"Estimated cost: ${(time.time() - trainer.start_time) / 3600 * config.cudo_max_price:.2f}"
    )
