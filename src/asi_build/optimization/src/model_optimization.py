...
class KnowledgeDistiller:
    ...
    def create_student_model(self, teacher_model: nn.Module) -> nn.Module:
        ...
        class StudentVLAModel(nn.Module):
            def __init__(self, teacher_dims):
                ...
                self.language_encoder = nn.Sequential(
                    nn.Linear(teacher_dims["vocab_size"], 128), nn.ReLU(), nn.Linear(128, 256)
                )

                self.action_decoder = nn.Sequential(
                    nn.Linear(512, 128), nn.ReLU(), nn.Linear(128, teacher_dims["action_dim"])
                )
        ...
        teacher_dims = {"vocab_size": 8192, "action_dim": 10}  # From teacher config
    ...
    def distill(
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                with torch.no_grad():
                    teacher_logits = teacher_model(batch["vision_input"], batch["language_input"])

                # Get student predictions
                student_logits = student_model(batch["vision_input"], batch["language_input"])
...
class VLAOptimizationPipeline:
    ...
    def optimize_model(
        ...
        results["optimization_steps"].append(
            {"step": "pruning", "size_mb": pruned_size, "reduction": 1 - pruned_size / student_size}
        )
    ...
    def print_optimization_summary(self, results: Dict[str, Any]):
        ...
        print(
            f"  Target ({self.config.target_model_size_mb} MB): {'✅ MET' if results['target_met'] else '❌ NOT MET'}"
        )
