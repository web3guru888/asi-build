...
class BCIMotorImageryClassifier(nn.Module):
    ...
    def __init__(self, config: BCIConfig):
        ...
        self.temporal_attention = nn.MultiheadAttention(
            embed_dim=config.n_channels, num_heads=8, dropout=0.1, batch_first=True
        )
    ...
    def extract_csp_features(
        self, eeg_signal: np.ndarray, class_label: Optional[int] = None
    ) -> np.ndarray:
        """
    ...
    def forward(self, eeg_signal: torch.Tensor, return_features: bool = False) -> torch.Tensor:
        ...
        attended, _ = self.temporal_attention(
            processed_reshaped, processed_reshaped, processed_reshaped
        )
...
class VLABrainActionModel(nn.Module):
    ...
    def __init__(self, vla_config: Any, bci_config: BCIConfig):
        ...
        self.bci_vision_attention = nn.MultiheadAttention(
            embed_dim=self.vision_dim, num_heads=8, dropout=0.1, batch_first=True
        )

        self.bci_language_attention = nn.MultiheadAttention(
            embed_dim=self.language_dim, num_heads=8, dropout=0.1, batch_first=True
        )
        ...
        self.override_gate = nn.Sequential(
            nn.Linear(self.bci_dim, 64), nn.ReLU(), nn.Linear(64, 1), nn.Sigmoid()
        )
...
class BCIDataSimulator:
    ...
    def generate_motor_imagery_signal(
        self, class_label: str, duration_ms: int = 1000
    ) -> torch.Tensor:
        """
