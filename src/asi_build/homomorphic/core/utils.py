...
from .base import FHEConfiguration, SchemeType, SecurityLevel
...
class HomomorphicUtils:
    ...
    @staticmethod
    def decode_data(
        plaintexts: List[Plaintext], original_shape: Optional[Tuple] = None
    ...
    def deserialize_ciphertext(data: bytes, poly_ring) -> Ciphertext:
        ...
        return Ciphertext(polynomials, obj["scale"], obj["level"], obj["is_ntt_form"])
    ...
    def estimate_memory_usage(
        parameters: FHEParameters, num_ciphertexts: int = 1
    ) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "total_keys_mb": public_key_size_mb
            + secret_key_size_mb
            + relin_keys_size_mb
            + galois_keys_size_mb,
        }
    ...
    def validate_computation_result(
        expected: np.ndarray, actual: np.ndarray, tolerance: float = 1e-6
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "tolerance": tolerance,
            "relative_error": (
                float(max_error / np.max(np.abs(expected))) if np.max(np.abs(expected)) > 0 else 0.0
            ),
        }
    ...
    @staticmethod
    def compare_schemes(
        schemes: List[Dict[str, Any]], test_data: np.ndarray, operations: List[str]
        ...
        for scheme_info in schemes:
            ...
            try:
                ...
                plaintexts = HomomorphicUtils.encode_data(
                    test_data, config.scheme_type, scale=config.scale
                )
                ...
                results[scheme_name] = {"error": str(e), "success": False}
