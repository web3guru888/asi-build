...
class Evaluator:
            ...
            for i in range(max_size):
                if i < ciphertext1.size and i < ciphertext2.size:
                    ...
                    sum_poly = self.poly_ring.add(
                        ciphertext1.polynomials[i], ciphertext2.polynomials[i]
                    )
            ...
            for i in range(ciphertext1.size):
                for j in range(ciphertext2.size):
                    product = self.poly_ring.multiply(
                        ciphertext1.polynomials[i], ciphertext2.polynomials[j]
                    )
                    result_polys[i + j] = self.poly_ring.add(result_polys[i + j], product)
    ...
    def add_plain(self, ciphertext: Ciphertext, plaintext_poly: Polynomial) -> Ciphertext:
        ...
        try:
            ...
            result = Ciphertext(
                result_polys, ciphertext.scale, ciphertext.level, ciphertext.is_ntt_form
            )
        ...
        try:
            ...
            if ciphertext.size == 3:
                ...
                relin_result = self._apply_key_switching(
                    decomposed, self.relin_keys.key_matrices[0]
                )
                ...
                result = Ciphertext(
                    [new_c0, new_c1], ciphertext.scale, ciphertext.level, ciphertext.is_ntt_form
                )
                ...
                while current.size > 2:
                    ...
                    relin_result = self._apply_key_switching(
                        decomposed, self.relin_keys.key_matrices[level]
                    )
                    ...
                    current = Ciphertext(c_rest, current.scale, current.level, current.is_ntt_form)
    ...
    def rescale(self, ciphertext: Ciphertext) -> Ciphertext:
        ...
        try:
            ...
            result = Ciphertext(result_polys, new_scale, new_level, ciphertext.is_ntt_form)
        ...
        try:
            ...
            if len(rotated_polys) > 1:
                ...
                switched_result = self._apply_key_switching(
                    decomposed, [self.galois_keys.key_map[galois_element]]
                )
            ...
            result = Ciphertext(
                rotated_polys, ciphertext.scale, ciphertext.level, ciphertext.is_ntt_form
            )
    ...
    def _apply_key_switching(
        self, decomposed_polys: List[Polynomial], key_switching_keys: List[List[Polynomial]]
    ) -> List[Polynomial]:
        """
