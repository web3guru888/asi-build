...
class NoiseEstimator:
    ...
    def estimate_multiplication_noise(
        self, noise1: float, noise2: float, ciphertext1: Ciphertext, ciphertext2: Ciphertext
    ) -> float:
        """
    ...
    def _estimate_plaintext_norm(self, ciphertext: Ciphertext) -> float:
        ...
        if ciphertext.scale:
            ...
            if self.parameters.plaintext_modulus:
                return (
                    math.sqrt(self.parameters.polynomial_modulus_degree)
    ...
    def track_operation(
        self, operation: str, input_noises: List[float], output_noise: float, **kwargs
    ) -> None:
        """
        ...
        if noise_ratio > 0.8:
            ...
            self.logger.info(
                f"Moderate noise level after {operation}: {noise_ratio:.2%} of maximum"
            )

    def predict_remaining_operations(
        self, current_noise: float, operation_pattern: List[str]
    ) -> int:
        """
        ...
        while simulated_noise < self.max_noise_bound and operation_count < max_iterations:

            for op in operation_pattern:
                if op == "add":
                    # Simulate addition with similar noise
                    simulated_noise = self.estimate_addition_noise(simulated_noise, simulated_noise)
                elif op == "multiply":
        ...
        for op in circuit:
            ...
            if op_type == "add":
                ...
                if len(inputs) >= 2:
                    noise1 = noise_levels.get(inputs[0], current_noise)
                    noise2 = noise_levels.get(inputs[1], current_noise)
                    result_noise = self.estimate_multiplication_noise(noise1, noise2, None, None)
                    # Add relinearization
    ...
    def suggest_parameter_changes(
        self, target_depth: int, target_operations: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def get_noise_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "operation_types": {
                op_type: operation_types.count(op_type) for op_type in set(operation_types)
            },
...
class NoiseManager:
    ...
    def auto_rescale_recommendation(self, ciphertext: Ciphertext, estimated_noise: float) -> bool:
        """
        ...
        return (
            noise_ratio > self.warning_threshold
            and ciphertext.level < len(self.parameters.coefficient_modulus) - 1
        )
