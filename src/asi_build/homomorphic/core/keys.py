...
class SecretKey:
    ...
    def serialize(self) -> bytes:
        ...
        return pickle.dumps({"polynomial": self.polynomial, "metadata": self.metadata})
...
class PublicKey:
    ...
    def serialize(self) -> bytes:
        ...
        return pickle.dumps({"polynomials": self.polynomials, "metadata": self.metadata})
...
class RelinearizationKeys:
    ...
    def serialize(self) -> bytes:
        ...
        return pickle.dumps({"key_matrices": self.key_matrices, "metadata": self.metadata})
...
class GaloisKeys:
    ...
    def serialize(self) -> bytes:
        ...
        return pickle.dumps({"key_map": self.key_map, "metadata": self.metadata})
...
class KeyGenerator:
    ...
    def generate_relinearization_keys(
        self, secret_key: SecretKey, max_level: int = 2
    ) -> RelinearizationKeys:
        """
        ...
        for level in range(2, max_level + 1):
            ...
            for i, q in enumerate(self.parameters.coefficient_modulus):
                ...
                e = Polynomial(
                    self._sample_gaussian_noise(self.parameters.polynomial_modulus_degree),
                    self.poly_ring,
                )
    ...
    def export_keys(
        self, keys: Dict[str, Any], filepath: str, password: Optional[str] = None
    ) -> None:
        """
