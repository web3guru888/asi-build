...
from .base import CiphertextBase, PlaintextBase, EncryptionException, SchemeType
...
class Ciphertext(CiphertextBase):
    ...
    def __init__(self, polynomials: List[Polynomial], scale: Optional[float] = None,
                 level: int = 0, is_ntt_form: bool = False):
        """
...
class Plaintext(PlaintextBase):
    ...
    def __init__(self, data: Union[list, np.ndarray], scale: Optional[float] = None,
                 encoding: str = "coefficients", level: int = 0):
        """
...
class Encryptor:
    ...
    def _encode_ckks(self, plaintext: Plaintext) -> Polynomial:
        ...
        if plaintext.encoding == "single":
            ...
            coeffs = [int(plaintext.data[0] * plaintext.scale)] + [0] * (self.parameters.polynomial_modulus_degree - 1)
            ...
            coeffs = [int(x * plaintext.scale) if plaintext.scale else int(x) 
                     for x in plaintext.data]
...
class Decryptor:
    ...
    def _decrypt_ckks(self, ciphertext: Ciphertext, target_encoding: str) -> Plaintext:
        ...
        if target_encoding == "single":
            ...
            data = [coeff / ciphertext.scale if ciphertext.scale else coeff 
