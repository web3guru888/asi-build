...
class Polynomial:
    ...
    def derivative(self) -> "Polynomial":
        ...
        deriv_coeffs = [i * self.coefficients[i] for i in range(1, self.degree)] + [0]
...
class PolynomialRing:
    ...
    def __init__(self, degree: int, coefficient_modulus: List[int]):
        ...
        logger.debug(
            f"Initialized polynomial ring: degree={degree}, " f"moduli={len(coefficient_modulus)}"
        )
    ...
    def to_ntt_form(self, coefficients: List[int]) -> List[int]:
        ...
        for i, q in enumerate(self.coefficient_modulus):
            ...
            ntt_coeffs = self._ntt_forward([c % q for c in coefficients], q, self.ntt_roots[i])
    ...
    def from_ntt_form(self, ntt_coefficients: List[int]) -> List[int]:
        ...
        for i, coeffs in enumerate(mod_coeffs):
            ...
            inv_ntt_coeffs = self._ntt_inverse(coeffs, q, self.inv_ntt_roots[i], self.inv_degree[i])
    ...
    def _crt_combine(self, mod_coeffs: List[List[int]]) -> List[int]:
        ...
        for i in range(1, len(mod_coeffs)):
            ...
            for j in range(self.degree):
                ...
                combined = (
                    x1 * next_modulus * inv_next + x2 * current_modulus * inv_current
                ) % new_modulus
    ...
    def __eq__(self, other: "PolynomialRing") -> bool:
        ...
        return self.degree == other.degree and self.coefficient_modulus == other.coefficient_modulus
