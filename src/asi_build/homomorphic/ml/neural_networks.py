...
class ActivationFunction:
    ...
    def polynomial_approximation(
        x: CKKSCiphertext, scheme: CKKSScheme, degree: int = 3
    ) -> CKKSCiphertext:
        """
...
class EncryptedDenseLayer(EncryptedLayer):
    ...
    def set_weights(self, weights: np.ndarray, bias: Optional[np.ndarray] = None):
        ...
        if weights.shape != (self.config.input_size, self.config.output_size):
            raise ValueError(
                f"Weight shape {weights.shape} doesn't match "
        ...
        if bias is not None:
            if bias.shape != (self.config.output_size,):
                raise ValueError(
                    f"Bias shape {bias.shape} doesn't match "
    ...
    def _encrypted_matrix_multiply(
        self, x: CKKSCiphertext, matrix: np.ndarray, scheme: CKKSScheme
    ) -> CKKSCiphertext:
        """
    ...
    def _combine_results(self, results: List[CKKSCiphertext], scheme: CKKSScheme) -> CKKSCiphertext:
...
class EncryptedConv2DLayer(EncryptedLayer):
    ...
    def set_weights(self, weights: np.ndarray, bias: Optional[np.ndarray] = None):
        ...
        if weights.shape != expected_shape:
            raise ValueError(
                f"Weight shape {weights.shape} doesn't match expected {expected_shape}"
            )
        ...
        if bias is not None:
            if bias.shape != (self.config.output_size,):
                raise ValueError(
                    f"Bias shape {bias.shape} doesn't match expected {(self.config.output_size,)}"
                )
            self.bias = bias
...
class EncryptedNeuralNetwork:
    ...
    def __init__(self, scheme_config: FHEConfiguration, network_config: List[LayerConfig]):
        """
    ...
    def load_pretrained_weights(self, weights: List[Dict[str, np.ndarray]]):
        ...
        if len(weights) != len(self.layers):
            raise ValueError(
                f"Number of weight sets ({len(weights)}) doesn't match "
    ...
    def load_model(cls, filepath: str) -> "EncryptedNeuralNetwork":
        ...
        # Create network
        network = cls(model_data["scheme_config"], model_data["network_config"])
