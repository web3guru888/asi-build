...
class BFVCiphertext(CiphertextBase):
    ...
    def __init__(
        self, polynomials: List[Polynomial], level: int = 0, noise_budget: Optional[int] = None
    ):
        """
    ...
    def __str__(self) -> str:
        return (
            f"BFVCiphertext(size={self.size}, level={self.level}, noise_budget={self.noise_budget})"
        )
...
class BFVScheme(FHECore):
    ...
    def __init__(self, config: FHEConfiguration):
        ...
        logger.info(
            f"BFV scheme initialized with plaintext modulus {self.parameters.plaintext_modulus}"
        )
    ...
    def encrypt(
        self, plaintext: Union[BFVPlaintext, List[int], int], public_key: Optional[PublicKey] = None
    ) -> BFVCiphertext:
        """
    ...
    def decrypt(
        self, ciphertext: BFVCiphertext, secret_key: Optional[SecretKey] = None
    ) -> BFVPlaintext:
        """
    ...
    def add(self, ciphertext1: BFVCiphertext, ciphertext2: BFVCiphertext) -> BFVCiphertext:
        ...
        result = BFVCiphertext(
            result_std.polynomials, result_std.level if hasattr(result_std, "level") else 0
        )
    ...
    def add_plain(
        self, ciphertext: BFVCiphertext, plaintext: Union[BFVPlaintext, List[int], int]
    ) -> BFVCiphertext:
        """
