...
class BeaverTriples:
    ...
    def multiply_with_beaver(
        self, x_shares: List[MPCShare], y_shares: List[MPCShare]
    ) -> List[MPCShare]:
        ...
        d_shares = [
            MPCShare(
                s.party_id,
                (s.share_value - a.share_value) % s.modulus,
                s.polynomial_degree,
                s.modulus,
            )
        ...
        e_shares = [
            MPCShare(
                s.party_id,
                (s.share_value - b.share_value) % s.modulus,
                s.polynomial_degree,
                s.modulus,
            )
        ...
        for i, (a_share, b_share, c_share) in enumerate(zip(a_shares, b_shares, c_shares)):
            if i == 0:  # Only first party adds the constant de
                value = (
                    d * e + d * b_share.share_value + e * a_share.share_value + c_share.share_value
                ) % c_share.modulus
            else:
                value = (
                    d * b_share.share_value + e * a_share.share_value + c_share.share_value
                ) % c_share.modulus
