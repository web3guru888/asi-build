...
class MPCEngine:
    ...
    def __init__(self, protocol_type: str = "bgw", threshold: int = 2, 
                 num_parties: int = 3, modulus: int = 2**31 - 1):
        """
    ...
    def compute_polynomial(self, coefficients: List[int], variable: str, result_var: str) -> List[MPCShare]:
        """
        ...
        if len(coefficients) > 1:
            ...
            for i, coeff in enumerate(coefficients[1:], 1):
                ...
                if i < len(coefficients) - 1:
                    current_power_shares = self.protocol.multiply_shares(current_power_shares, x_shares)
