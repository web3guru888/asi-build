...
class GMWProtocol(SecretSharingProtocol):
    ...
    def evaluate_circuit(self, circuit: Dict[str, Any], input_shares: Dict[str, List[MPCShare]]) -> Dict[str, List[MPCShare]]:
        """
        ...
        for gate in circuit.get("gates", []):
            ...
            if gate_type == "XOR":
                wire_shares[output] = self.add_shares(
                    wire_shares[inputs[0]], 
                    wire_shares[inputs[1]]
                )
            elif gate_type == "AND":
