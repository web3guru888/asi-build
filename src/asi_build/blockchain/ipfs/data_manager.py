...
class DataManager:
    ...
    async def store_audit_record(self, record: AuditRecord) -> StorageResult:
        ...
        try:
            ...
            result = StorageResult(ipfs_hash=ipfs_hash, size=len(record_json), encrypted=False)
    ...
    async def store_audit_collection(
        ...
        try:
            ...
            result = StorageResult(ipfs_hash=ipfs_hash, size=len(collection_json), encrypted=False)
    ...
    async def retrieve_audit_collection(
        self, ipfs_hash: str
    ) -> Tuple[str, List[AuditRecord], Dict]:
        """
        ...
        try:
            ...
            records = [AuditRecord.from_dict(record_data) for record_data in data["records"]]
...
class EncryptedDataManager(DataManager):
    ...
    def verify_signature(
        self, data: bytes, signature: bytes, public_key: Optional[rsa.RSAPublicKey] = None
    ) -> bool:
    ...
    async def store_audit_record(self, record: AuditRecord, encrypt: bool = True) -> StorageResult:
        ...
        try:
            ...
            if encrypt:
                # Encrypt the data
                encrypted_data = self.fernet.encrypt(json.dumps(record_data, default=str).encode())
                ...
                result = StorageResult(ipfs_hash=ipfs_hash, size=len(record_json), encrypted=True)
            else:
                ipfs_hash = await self._store_json_data(record_data)
                result = StorageResult(ipfs_hash=ipfs_hash, size=len(record_json), encrypted=False)
        ...
        try:
            ...
            if record.signature:
                ...
                verification_data = json.dumps(
                    record_copy.to_dict(), sort_keys=True, default=str
                ).encode()
