...
class JWTAuth:
    ...
    def cleanup_blacklist(self):
        ...
        logger.info(f"Blacklist cleanup completed. Entries: {len(self.blacklist)} (was {initial_size})")
...
class AuthManager:
                ...
                if key_info.key_hash == key_hash:
                    ...
                    if key_info.ip_whitelist and client_ip:
                        if client_ip not in key_info.ip_whitelist:
                            logger.warning(f"API key {key_info.key_id} used from unauthorized IP: {client_ip}")
                            return None
    ...
    def _check_rate_limit(self, key_id: str, limit: int) -> bool:
        ...
        # Clean old entries
        self.rate_limit_tracking[key_id] = [
            timestamp for timestamp in self.rate_limit_tracking[key_id]
    ...
    def get_auth_stats(self) -> Dict[str, Any]:
        ...
        active_keys = sum(1 for key in self.api_keys.values() if key.is_active and not key.is_expired())
