...
class AuditTrailAPI:
    ...
    async def create_audit_record(self, request: AuditRecordRequest) -> AuditRecordResponse:
        ...
        try:
            ...
            record_id = (
                f"audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{hash(str(request.dict()))}"
            )
            ...
            if request.encrypt and isinstance(self.data_manager, EncryptedDataManager):
                storage_result = await self.data_manager.store_audit_record(
                    audit_record, encrypt=True
                )
            else:
            ...
            signature = self.signature_manager.sign_message(
                audit_record.to_dict(), key_id="default"  # Should be configurable
            )
    ...
    def get_api_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.api_metrics,
            "success_rate": (
                self.api_metrics["requests_successful"] / max(self.api_metrics["requests_total"], 1)
            )
            * 100,
            "timestamp": datetime.now().isoformat(),
        }
