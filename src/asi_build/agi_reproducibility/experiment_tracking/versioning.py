...
class VersionManager:
    ...
    async def create_version(self, experiment_id: str, version_type: VersionType = VersionType.PATCH,
                           message: str = "", changes: List[str] = None) -> VersionInfo:
    ...
    async def compare_versions(self, experiment_id: str, version1: str, 
        ...
        if self.git_repo and v1_info.commit_hash and v2_info.commit_hash:
            try:
                diffs = self.git_repo.git.diff(
                    v1_info.commit_hash, v2_info.commit_hash, 
                    f"{experiment_id}/", name_only=True
                ).splitlines()
    ...
    async def create_branch(self, experiment_id: str, branch_name: str, 
                          base_version: str = None, purpose: str = "") -> BranchInfo:
        ...
        if branch_key in branches:
            raise VersioningError(f"Branch {branch_name} already exists for experiment {experiment_id}")
    ...
    async def merge_branch(self, experiment_id: str, source_branch: str, 
                         target_branch: str = "main") -> VersionInfo:
        ...
        if self.git_repo:
            try:
                ...
                target_git_branch = f"{experiment_id}-{target_branch}" if target_branch != "main" else "main"
                
                # Switch to target branch and merge
                self.git_repo.heads[target_git_branch].checkout()
                self.git_repo.git.merge(source_git_branch, m=f"Merge {source_branch} into {target_branch}")
    ...
    async def tag_version(self, experiment_id: str, version: str, 
                         tag_name: str, message: str = "") -> None:
        ...
        if self.git_repo and version_info.commit_hash:
            try:
                ...
                self.git_repo.create_tag(git_tag_name, ref=version_info.commit_hash, message=message)
            except GitCommandError as e:
        ...
        for file_path in sorted(experiment_dir.rglob("*")):
            if file_path.is_file() and not file_path.name.startswith('.'):
                try:
                    ...
                    content_parts.append(f"{relative_path}:{hashlib.sha256(file_content).hexdigest()}")
