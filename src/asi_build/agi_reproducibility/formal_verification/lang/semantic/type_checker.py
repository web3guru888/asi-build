...
class SafetyTypeChecker(SafetyVisitor):
    ...
    def _init_builtin_environment(self):
        ...
        self.env.bind(
            "stable",
            SafetyType(TypeKind.FUNCTION, "Function", [self.GOAL_TYPE], [self.BOOLEAN_TYPE]),
        )
        ...
        self.env.bind(
            ...
            SafetyType(
                TypeKind.FUNCTION,
                "Function",
                [self.IMPACT_TYPE, self.REAL_TYPE],
                [self.BOOLEAN_TYPE],
            ),
        )
        ...
        self.env.bind(
            "corrigible",
            SafetyType(TypeKind.FUNCTION, "Function", [self.STATE_TYPE], [self.BOOLEAN_TYPE]),
        )
    ...
    def visit_binary_op(self, node: BinaryOperation) -> SafetyType:
        ...
        # Comparison operators
            ...
            if not left_type.is_compatible(right_type):
                self.add_error(
                    f"Incompatible types for comparison: {left_type} and {right_type}", node
                )
            return self.BOOLEAN_TYPE

        # Arithmetic operators
        elif node.operator in ["+", "-", "*", "/", "%"]:
            if left_type.name not in ["Integer", "Real"] or right_type.name not in [
                "Integer",
                "Real",
            ]:
                self.add_error(
                    f"Arithmetic operator {node.operator} requires numeric operands", node
                )
    ...
    def visit_binary_temporal(self, node: BinaryTemporalExpression) -> SafetyType:
        ...
        if left_type.name != "Boolean" or right_type.name != "Boolean":
            self.add_error(
                f"Binary temporal operator {node.operator} requires Boolean operands", node
            )
