...
class SafetySpecificationParser:
    ...
    def parse_specification(self) -> SafetySpecification:
        ...
        spec = SafetySpecification(name=name, version=version, target_system=target_system)
    ...
    def parse_invariant(self) -> SafetyInvariant:
        ...
        return SafetyInvariant(
            name=name_token.value, condition=condition, location=name_token.location
        )
    ...
    def parse_state(self) -> SystemState:
        ...
        return SystemState(
            name=name_token.value, variables={}, invariants=[], location=name_token.location
        )
    ...
    def parse_factor(self) -> SafetyExpression:
        ...
        if self.current_token:
            raise ParseError(
                f"Unexpected token: {self.current_token.type}", self.current_token.location
            )
        else:
