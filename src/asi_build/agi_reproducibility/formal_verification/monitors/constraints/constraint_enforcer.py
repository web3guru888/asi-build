...
class CapabilityBoundEnforcer(ConstraintEnforcer):
    ...
    def _evaluate_capability_violation(
        self,
        constraint: ConstraintSpec,
        current_state: Dict[str, Any],
        proposed_action: Dict[str, Any],
    ...
    def _limit_capability(
        self, constraint: ConstraintSpec, violation_level: float, proposed_action: Dict[str, Any]
    ) -> EnforcementResult:
    ...
    def _trigger_shutdown(
        self, constraint: ConstraintSpec, violation_level: float
    ) -> EnforcementResult:
    ...
    def _generate_safe_alternative(
        self, original_action: Dict[str, Any], violation_level: float
    ) -> Dict[str, Any]:
...
class ValueAlignmentEnforcer(ConstraintEnforcer):
    ...
    def _block_misaligned_action(
        self, constraint: ConstraintSpec, misalignment: float, proposed_action: Dict[str, Any]
    ) -> EnforcementResult:
    ...
    def _align_action(
        self, constraint: ConstraintSpec, current_alignment: float, proposed_action: Dict[str, Any]
    ) -> EnforcementResult:
        ...
        if "action_values" in proposed_action:
            aligned_values = self._apply_value_correction(
                proposed_action["action_values"], constraint.intervention_strength
            )
    ...
    def _apply_value_correction(
        self, action_values: Dict[str, Any], correction_strength: float
    ) -> Dict[str, Any]:
...
class GoalStabilityEnforcer(ConstraintEnforcer):
    ...
    def enforce_constraint(
        ...
        try:
            ...
            if constraint.enforcement_action == EnforcementAction.LIMIT:
                ...
                return self._block_destabilizing_action(
                    constraint, stability_impact, proposed_action
                )
    ...
    def _evaluate_goal_stability_impact(
        self,
        constraint: ConstraintSpec,
        current_state: Dict[str, Any],
        proposed_action: Dict[str, Any],
        ...
        coherence_impact = self._calculate_coherence_impact(current_goal, proposed_goal_change)
        stability_metrics.append(coherence_impact)

        return sum(stability_metrics) / len(stability_metrics)

    def _calculate_goal_change_magnitude(
        self, current_goal: Dict[str, Any], goal_change: Dict[str, Any]
    ) -> float:
        ...
        for component, change_value in goal_change.items():
            if component in current_goal:
                ...
                if isinstance(current_value, (int, float)) and isinstance(
                    ...
                    relative_change = abs(change_value - current_value) / max(
                        abs(current_value), 1.0
                    )
    ...
    def _measure_goal_consistency(
        self, component: str, change_value: Any, current_goal: Dict[str, Any]
    ) -> float:
    ...
    def _limit_goal_modification(
        self, constraint: ConstraintSpec, stability_impact: float, proposed_action: Dict[str, Any]
    ) -> EnforcementResult:
    ...
    def _block_destabilizing_action(
        self, constraint: ConstraintSpec, stability_impact: float, proposed_action: Dict[str, Any]
    ) -> EnforcementResult:
...
class SafetyConstraintEnforcementSystem:
    ...
    def get_enforcement_status(self) -> Dict[str, Any]:
        ...
        recent_enforcements = [
            h
            for h in self.enforcement_history
