...
class SafetyMonitor(ABC):
    ...
    def emit_alert(self, level: AlertLevel, property_name: str,
                   violation_description: str, current_state: Dict[str, Any],
...
class GoalDriftMonitor(SafetyMonitor):
        ...
        try:
            ...
            if drift_detected:
                ...
                self.emit_alert(
                    ...
                    current_state,
                    ["Review goal definition", "Check learning parameters", "Consider goal realignment"]
                )
    ...
    def _detect_drift(self) -> Tuple[bool, float]:
        ...
        recent_values = [alignment for _, alignment in list(self.goal_history)[-self.drift_window:]]
...
class CapabilityBoundaryMonitor(SafetyMonitor):
            ...
            for bound in self.impact_bounds:
                ...
                if violation:
                    ...
                    self.emit_alert(
                        ...
                        current_state,
                        ["Reduce capability scope", "Apply impact limiting", "Review boundary parameters"]
                    )
    ...
    def _evaluate_impact_metric(self, metric_expr: SafetyExpression, state: Dict[str, Any]) -> Optional[float]:
    ...
    def _evaluate_upper_bound(self, bound_expr: SafetyExpression, state: Dict[str, Any]) -> Optional[float]:
...
class MesaOptimizationMonitor(SafetyMonitor):
            ...
            for guard in self.mesa_guards:
                ...
                if detection_score > self.detection_threshold:
                    ...
                    self.emit_alert(
                        ...
                        current_state,
                        ["Apply prevention mechanism", "Review optimization process", "Emergency intervention"]
                    )
    ...
    def _evaluate_detection_condition(self, guard: MesaOptimizationGuard, 
    ...
    def initialize_from_specification(self, spec: SafetySpecification, 
        ...
        try:
            ...
            for alignment_spec in spec.value_alignments:
                monitor = ValueAlignmentMonitor(f"value_alignment_{alignment_spec.name}", alignment_spec)
