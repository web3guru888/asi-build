...
class ResolutionProver(SafetyTheoremProver):
        ...
        try:
            ...
            while step_count < self.max_resolution_steps:
                if time.time() - start_time > timeout:
                    result = ProofTrace(
                        theorem, ProofResult.TIMEOUT, steps, time.time() - start_time
                    )
                    break
                ...
                if self._contains_empty_clause(new_clauses):
                    ...
                    result = ProofTrace(
                        theorem, ProofResult.PROVED, steps, time.time() - start_time
                    )
                    break
                ...
                if not new_clauses:
                    ...
                    result = ProofTrace(
                        theorem, ProofResult.UNKNOWN, steps, time.time() - start_time
                    )
                    break
                ...
                # Max steps reached
                result = ProofTrace(theorem, ProofResult.TIMEOUT, steps, time.time() - start_time)

            self.proof_cache[cache_key] = result
            return result

        except Exception as e:
            return ProofTrace(theorem, ProofResult.ERROR, [], time.time() - start_time)
    ...
    def _get_cache_key(self, theorem: SafetyExpression, axioms: List[SafetyExpression]) -> str:
    ...
    def _resolution_step(
        self, clauses: List[SafetyExpression], steps: List[ProofStep], step_num: int
    ) -> List[SafetyExpression]:
    ...
    def _resolve(
        self, clause1: SafetyExpression, clause2: SafetyExpression
    ) -> Optional[SafetyExpression]:
        ...
        if isinstance(clause1, Variable) and isinstance(clause2, UnaryOperation):
            if (
                isinstance(clause2.operand, Variable)
                and clause1.name == clause2.operand.name
                and clause2.operator == LogicalOperator.NOT
            ):
...
class NaturalDeductionProver(SafetyTheoremProver):
    ...
    def prove(
        ...
        try:
            ...
            if self._natural_deduction_search(theorem, assumptions, steps, start_time, timeout):
                result = ProofTrace(theorem, ProofResult.PROVED, steps, time.time() - start_time)
            else:
                result = ProofTrace(theorem, ProofResult.UNKNOWN, steps, time.time() - start_time)
    ...
    def _natural_deduction_search(
        self,
        goal: SafetyExpression,
    ...
    def _modus_ponens(
        ...
        for assumption in assumptions:
            if (
                isinstance(assumption, BinaryOperation)
                and assumption.operator == LogicalOperator.IMPLIES
                and self._expressions_equal(assumption.right, goal)
            ):

                # Try to prove the antecedent
                if self._natural_deduction_search(
                    assumption.left, assumptions, steps, start_time, timeout
                ):
                    step = ProofStep(
                        rule="Modus Ponens",
    ...
    def _modus_tollens(
        ...
        for assumption in assumptions:
            if (
                isinstance(assumption, BinaryOperation)
                and assumption.operator == LogicalOperator.IMPLIES
                and self._expressions_equal(assumption.right, negated_goal)
            ):
                ...
                if self._natural_deduction_search(
                    negated_consequent, assumptions, steps, start_time, timeout
                ):
                    step = ProofStep(
                        rule="Modus Tollens",
    ...
    def _conjunction_intro(
        ...
        left_proved = self._natural_deduction_search(
            goal.left, assumptions, steps, start_time, timeout
        )
        if not left_proved:
            return False

        right_proved = self._natural_deduction_search(
            goal.right, assumptions, steps, start_time, timeout
        )
        if not right_proved:
            return False
    ...
    def _conjunction_elim(
        ...
        for assumption in assumptions:
            if (
                isinstance(assumption, BinaryOperation)
                and assumption.operator == LogicalOperator.AND
            ):

                if self._expressions_equal(assumption.left, goal) or self._expressions_equal(
                    assumption.right, goal
                ):
    ...
    def _disjunction_elim(
        ...
        for assumption in assumptions:
            if (
                isinstance(assumption, BinaryOperation)
                and assumption.operator == LogicalOperator.OR
            ):
                ...
                left_proves = self._natural_deduction_search(
                    goal, left_case_assumptions, left_steps, start_time, timeout
                )
                right_proves = self._natural_deduction_search(
                    goal, right_case_assumptions, right_steps, start_time, timeout
                )
    ...
    def _double_negation(
        ...
        for assumption in assumptions:
            if (
                isinstance(assumption, UnaryOperation)
                and assumption.operator == LogicalOperator.NOT
                and isinstance(assumption.operand, UnaryOperation)
                and assumption.operand.operator == LogicalOperator.NOT
                and self._expressions_equal(assumption.operand.operand, goal)
            ):
...
class TemporalLogicProver(SafetyTheoremProver):
    ...
    def _is_temporal_formula(self, expr: SafetyExpression) -> bool:
        ...
        if isinstance(expr, TemporalExpression):
            ...
            return self._is_temporal_formula(expr.left) or self._is_temporal_formula(expr.right)
    ...
    def _prove_unary_temporal(
        self,
        theorem: TemporalExpression,
        axioms: List[SafetyExpression],
        steps: List[ProofStep],
        start_time: float,
        timeout: float,
    ...
    def _prove_binary_temporal(
        self,
        theorem: BinaryTemporalExpression,
        axioms: List[SafetyExpression],
        steps: List[ProofStep],
        start_time: float,
        timeout: float,
        ...
        if theorem.operator == TemporalOperator.UNTIL:
            ...
            return self._prove_until(
                theorem.left, theorem.right, axioms, steps, start_time, timeout
            )

        elif theorem.operator == TemporalOperator.RELEASE:
            # p R q - q holds unless p holds
            return self._prove_release(
                theorem.left, theorem.right, axioms, steps, start_time, timeout
            )
...
class SafetyProverSuite:
    ...
    def prove_safety_property(
        self, prop: SafetyProperty, axioms: List[SafetyExpression] = None, timeout: float = 60.0
    ) -> ProofTrace:
        ...
        if type_errors:
            return ProofTrace(prop.specification, ProofResult.ERROR, [], 0.0)
        ...
        if self._is_temporal_property(prop.specification):
            ...
            if result1.result != ProofResult.PROVED:
                ...
                result2 = self.natural_deduction_prover.prove(
                    prop.specification, axioms, timeout / 3
                )
    ...
    def prove_corrigibility_maintenance(
        self,
        corrigibility_spec: CorrigibilitySpec,
        ...
        maintenance_property = TemporalExpression(
            TemporalOperator.ALWAYS, corrigibility_spec.modification_acceptance
        )

        return self.temporal_prover.prove(maintenance_property, system_axioms, timeout)

    def prove_bounded_optimization(
        self,
        impact_bound: ImpactBound,
    ...
    def check_mesa_optimization_risks(
        self,
        mesa_guard: MesaOptimizationGuard,
        system_axioms: List[SafetyExpression],
        timeout: float = 30.0,
    ) -> ProofTrace:
        ...
        intervention_property = BinaryOperation(
            mesa_guard.detection_condition, LogicalOperator.IMPLIES, mesa_guard.prevention_mechanism
        )
