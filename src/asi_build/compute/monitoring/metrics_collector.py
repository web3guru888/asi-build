...
class MetricSeries:
    ...
    def get_statistics(self, window_seconds: float = 300) -> Dict[str, float]:
        ...
        recent_values = [point.value for point in self.points if point.timestamp >= cutoff_time]
...
class MetricsCollector:
    ...
    def __init__(self, collection_interval: float = 10.0):
        ...
        self.aggregated_metrics: Dict[str, Dict[int, Dict[str, float]]] = defaultdict(
    ...
    def record_metric(
        ...
        if metric_key not in self.metrics:
            self.metrics[metric_key] = MetricSeries(
                name=name, metric_type=metric_type, labels=labels
            )
    ...
    def query_metrics(
        self, name: str, start_time: float, end_time: float, labels: Optional[Dict[str, str]] = None
    ) -> List[MetricPoint]:
            ...
            if metric and len(metric.points) >= 2:
                ...
                if time_delta > 0:
                    ...
                    if rate > thresholds["rate"]:
                        self._trigger_alert(
                            alert_key, "rate", rate, thresholds["rate"], current_time
                        )
    ...
    def _calculate_severity(
        self, threshold_type: str, current_value: float, threshold_value: float
    ) -> str:
    ...
    async def _aggregate_metrics(self) -> None:
        ...
        for metric_key, metric_series in self.metrics.items():
            ...
            for window_seconds in self.aggregation_windows:
                ...
                points_in_window = [
                    point.value for point in metric_series.points if point.timestamp >= window_start
                ]
    ...
    async def export_metrics(self, start_time: float, end_time: float, format: str = "json") -> str:
        ...
        if format == "json":
            return json.dumps(
                {"start_time": start_time, "end_time": end_time, "metrics": exported_metrics},
                indent=2,
            )
    ...
    async def cleanup_old_metrics(self, retention_days: int = 7) -> int:
        ...
        try:
            ...
            cursor = self.db_connection.execute(
                "SELECT COUNT(*) FROM metrics WHERE timestamp < ?", (cutoff_time,)
            )
            count_to_delete = cursor.fetchone()[0]

            self.db_connection.execute("DELETE FROM metrics WHERE timestamp < ?", (cutoff_time,))

            self.db_connection.execute(
                "DELETE FROM aggregated_metrics WHERE timestamp < ?", (cutoff_time,)
            )
