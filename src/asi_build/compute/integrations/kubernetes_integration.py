...
class KubernetesIntegration:
            ...
            while True:
                try:
                    for event in self.node_watcher.stream(
                        self.core_v1_api.list_node, timeout_seconds=30
                    ):
            ...
            if pod_name in self.active_pods:
                ...
                if pod.status:
                    ...
                    if hasattr(pod.status, "container_statuses") and pod.status.container_statuses:
                        ...
                        if (
                            hasattr(container_status.state, "terminated")
                            and container_status.state.terminated
                        ):
                            self.active_pods[pod_name].completion_time = (
    ...
    async def submit_job(
        self, job_id: str, pod_spec: KubernetesPodSpec, resource_requirements: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            pod_spec.labels.update({"job-id": job_id, "submitted-at": str(int(time.time()))})
            ...
            # Track job
            job_status = KubernetesJobStatus(pod_name=pod_name, phase="Pending")
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            # Delete the pod
            self.core_v1_api.delete_namespaced_pod(name=pod_name, namespace=self.namespace)
    ...
    async def get_cluster_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            used_resources = {"cpu": 0.0, "memory": 0, "gpu": 0}
                    ...
                    for container in pod.spec.containers:
                        if container.resources and container.resources.requests:
                            ...
                            if "memory" in requests:
                                used_resources["memory"] += self._parse_memory_quantity(
                                    requests["memory"]
                                )
    ...
    async def create_resource_quota(
        ...
        try:
            ...
            created_quota = self.core_v1_api.create_namespaced_resource_quota(
                namespace=self.namespace, body=resource_quota
            )
    ...
    async def apply_manifest(self, manifest: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Applied {kind}: {manifest.get('metadata', {}).get('name', 'unknown')}"
            )
            return True
