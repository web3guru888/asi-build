...
class CheckpointManager:
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(f"Checkpoint manager initialized with {len(self.checkpoints)} existing checkpoints")
            ...
            if metadata_dir.exists():
                for metadata_file in metadata_dir.glob("*.json"):
                    try:
                        ...
                        self.logger.error(f"Error loading checkpoint metadata from {metadata_file}: {e}")
    ...
    async def create_checkpoint(
        ...
        try:
            ...
            self._stats["average_checkpoint_time"] = (
                (self._stats["average_checkpoint_time"] * (self._stats["successful_checkpoints"] - 1) + checkpoint_time)
            ...
            self.logger.info(f"Checkpoint {checkpoint_id} completed in {checkpoint_time:.2f} seconds")
    ...
    async def _save_checkpoint_data(self, checkpoint: CheckpointMetadata, job_state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if checkpoint.compression_enabled:
                ...
                self._stats["compression_ratio"] = (
                    (self._stats["compression_ratio"] * (self._stats["total_checkpoints"] - 1) + compression_ratio)
                    / self._stats["total_checkpoints"]
                ) if self._stats["total_checkpoints"] > 0 else compression_ratio
    ...
    async def get_latest_checkpoint(self, job_id: str) -> Optional[CheckpointMetadata]:
        ...
        for checkpoint_id in job_checkpoint_ids:
            if checkpoint_id in self.checkpoints:
                checkpoint = self.checkpoints[checkpoint_id]
                if (checkpoint.status == CheckpointStatus.COMPLETED and 
                    checkpoint.created_at > latest_time):
    ...
    async def delete_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        try:
            ...
            if checkpoint.job_id in self.job_checkpoints:
                self.job_checkpoints[checkpoint.job_id] = [
                    cid for cid in self.job_checkpoints[checkpoint.job_id] 
    ...
    async def _cleanup_job_checkpoints(self, job_id: str) -> None:
        ...
        if len(job_checkpoint_ids) > self.policy.max_checkpoints_per_job:
            ...
            checkpoints = [
                self.checkpoints[cid] for cid in job_checkpoint_ids 
