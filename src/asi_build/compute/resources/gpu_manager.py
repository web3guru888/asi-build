...
class GPUPoolManager:
            ...
            try:
                ...
                for i in range(device_count):
                    ...
                    try:
                        ...
                        device.temperature = pynvml.nvmlDeviceGetTemperature(
                    ...
                    self.logger.info(
                        f"Discovered NVIDIA GPU: {device.model} ({device.memory_total} MB)"
                    )
    ...
    async def allocate_gpu(
        ...
        if len(suitable_gpus) < gpu_count:
            self.logger.warning(
                f"Insufficient GPU resources: need {gpu_count}, found {len(suitable_gpus)}"
            )
            return None
    ...
    async def _find_suitable_gpus(
        ...
        suitable_gpus.sort(key=lambda gpu: (gpu.memory_free, -gpu.utilization, -gpu.temperature))
    ...
    async def get_utilization(self) -> Dict[str, float]:
        ...
        avg_gpu_util = sum(device.utilization for device in self.devices.values()) / len(
            self.devices
        )
        ...
        return {
            ...
            "gpu_count_available": len(
                [d for d in self.devices.values() if d.state == GPUState.AVAILABLE]
            ),
            "gpu_count_allocated": len(
                [d for d in self.devices.values() if d.state == GPUState.ALLOCATED]
            ),
            ...
            "gpu_temperature_avg": sum(device.temperature for device in self.devices.values())
            / len(self.devices),
        }
                    ...
                    if line.strip():
                        ...
                        if len(parts) >= 5:
                            ...
                            if device_id in self.devices:
                                device = self.devices[device_id]
                                device.memory_total = (
                                    int(parts[1]) if parts[1].isdigit() else device.memory_total
                                )
                                device.memory_free = (
                                    int(parts[2]) if parts[2].isdigit() else device.memory_free
                                )
                                device.utilization = (
                                    float(parts[3])
                                    if parts[3].replace(".", "").isdigit()
                                    else device.utilization
                                )
                                device.temperature = (
                                    float(parts[4])
    ...
    def _update_device_stats(self) -> None:
        ...
        self._stats["available_gpus"] = len(
            [d for d in self.devices.values() if d.state == GPUState.AVAILABLE]
        )
        self._stats["allocated_gpus"] = len(
            [d for d in self.devices.values() if d.state == GPUState.ALLOCATED]
        )

        self._stats["total_memory"] = sum(device.memory_total for device in self.devices.values())
        self._stats["allocated_memory"] = sum(
            device.memory_used for device in self.devices.values()
        )

        if self.devices:
            self._stats["average_utilization"] = sum(
                device.utilization for device in self.devices.values()
            ) / len(self.devices)
            self._stats["average_temperature"] = sum(
                device.temperature for device in self.devices.values()
            ) / len(self.devices)
