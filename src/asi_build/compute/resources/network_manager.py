...
class NetworkManager:
    ...
    async def _check_traffic_control_support(self) -> None:
        ...
        try:
            result = subprocess.run(["tc", "--version"], capture_output=True, text=True, timeout=5)
            ...
            if result.returncode == 0:
                ...
                self.logger.warning(
                    "Traffic control (tc) not available - bandwidth limiting disabled"
                )
    ...
    async def _parse_ip_routes(self, route_output: str) -> List[NetworkRoute]:
        ...
        for line in route_output.strip().split("\n"):
            ...
            routes.append(
                NetworkRoute(
                    destination=destination, gateway=gateway, interface=interface, metric=metric
                )
            )
    ...
    async def allocate_bandwidth(
        ...
        if total_available < bandwidth_mbps:
            self.logger.warning(
                f"Insufficient bandwidth: need {bandwidth_mbps}, available {total_available}"
            )
            return None
        ...
        try:
            for interface_name in allocation.interface_names:
                ...
                cmd = [
                    ...
                    "htb",
                    "rate",
                    f"{allocation.total_bandwidth_mbps}mbit",
                    "prio",
                    str(7 - allocation.priority),  # Convert to tc priority (0 = highest)
                ]
    ...
    async def _tcp_connect_test(self, hostname: str, port: int, timeout: float) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "connect_time_ms": connect_time * 1000}
    ...
    async def get_network_status(self) -> Dict[str, Any]:
        ...
        return {
            "interfaces": {
                name: interface.to_dict() for name, interface in self.interfaces.items()
            },
            ...
            "qos_rules": {rule_id: rule.to_dict() for rule_id, rule in self.qos_rules.items()},
    ...
    async def get_utilization(self) -> Dict[str, float]:
        ...
        total_bandwidth = sum(
            iface.speed_mbps for iface in self.interfaces.values() if iface.speed_mbps > 0
        )
        total_utilization = sum(iface.total_bandwidth_mbps for iface in self.interfaces.values())

        avg_utilization = 0.0
        active_interfaces = [
            iface for iface in self.interfaces.values() if iface.state == NetworkState.UP
        ]

        if active_interfaces:
            avg_utilization = sum(iface.utilization_percent for iface in active_interfaces) / len(
                active_interfaces
            )
        ...
        return {
            ...
            "network_packets_total": self._stats["total_rx_packets"]
            + self._stats["total_tx_packets"],
    ...
    def _update_interface_stats(self) -> None:
        ...
        self._stats["total_rx_packets"] = sum(
            iface.rx_packets for iface in self.interfaces.values()
        )
        self._stats["total_tx_packets"] = sum(
            iface.tx_packets for iface in self.interfaces.values()
        )
        self._stats["total_errors"] = sum(
            iface.rx_errors + iface.tx_errors for iface in self.interfaces.values()
        )
        self._stats["total_dropped"] = sum(
            iface.rx_dropped + iface.tx_dropped for iface in self.interfaces.values()
        )
        ...
        if active_interfaces:
            self._stats["average_utilization"] = sum(
                iface.utilization_percent for iface in active_interfaces
    ...
    async def get_pool_statistics(self) -> Dict[str, Any]:
        ...
        for network_type in NetworkType:
            interfaces = [
                iface for iface in self.interfaces.values() if iface.network_type == network_type
            ]
            if interfaces:
                interface_type_distribution[network_type.value] = {
                    "count": len(interfaces),
                    "total_bandwidth_mbps": sum(iface.speed_mbps for iface in interfaces),
                    "average_utilization": sum(iface.utilization_percent for iface in interfaces)
                    / len(interfaces),
                }
