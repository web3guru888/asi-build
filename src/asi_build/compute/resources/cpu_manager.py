...
class CPUPoolManager:
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(
            f"CPU pool initialized with {total_cores} logical cores across {len(self.sockets)} sockets"
        )
        ...
        try:
            ...
            if not cpu_info:
                ...
                for core_id in range(logical_cpu_count):
                    core = CPUCore(core_id=core_id, physical_id=0, thread_id=core_id)
    ...
    async def _build_topology_from_cpuinfo(self, cpu_info: Dict[int, Dict[str, str]]) -> None:
        ...
        for cpu_id, info in cpu_info.items():
            ...
            # Create core
            core = CPUCore(core_id=cpu_id, physical_id=physical_id, thread_id=core_id)
    ...
    async def _discover_numa_topology(self) -> None:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.numa_nodes[0] = list(
                    range(sum(socket.logical_cores for socket in self.sockets.values()))
                )
    ...
    async def _initialize_cpu_monitoring(self) -> None:
        ...
        try:
            ...
            if os.path.exists("/sys/class/thermal/"):
                thermal_zones = [
                    d for d in os.listdir("/sys/class/thermal/") if d.startswith("thermal_zone")
                ]
                if thermal_zones:
                    self.logger.info(
                        f"Found {len(thermal_zones)} thermal zones for temperature monitoring"
                    )
    ...
    async def allocate_cpu(
        ...
        self.logger.info(
            f"Allocating {cpu_cores} CPU cores for job {job_id} (exclusive: {exclusive})"
        )
        ...
        if len(suitable_cores) < cpu_cores:
            self.logger.warning(
                f"Insufficient CPU cores: need {cpu_cores}, found {len(suitable_cores)}"
            )
            return None
    ...
    async def _find_suitable_cores(
        self, cpu_cores: int, numa_affinity: bool, exclusive: bool
    ) -> List[int]:
    ...
    async def get_cpu_status(self, socket_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        return {
            f"socket_{socket_id}": socket.to_dict() for socket_id, socket in self.sockets.items()
        }
    ...
    async def get_utilization(self) -> Dict[str, float]:
        ...
        avg_utilization = sum(socket.average_utilization for socket in self.sockets.values()) / len(
            self.sockets
        )
        avg_frequency = (
            sum(
                (
                    sum(core.frequency for core in socket.cores.values()) / len(socket.cores)
                    if socket.cores
                    else 0
                )
                for socket in self.sockets.values()
    ...
    def _update_cpu_stats(self) -> None:
        ...
        self._stats["total_physical_cores"] = sum(
            socket.physical_cores for socket in self.sockets.values()
        )
        self._stats["total_logical_cores"] = sum(
            socket.logical_cores for socket in self.sockets.values()
        )
        self._stats["available_cores"] = sum(
            socket.available_cores for socket in self.sockets.values()
        )
        self._stats["allocated_cores"] = (
            self._stats["total_logical_cores"] - self._stats["available_cores"]
        )
        self._stats["numa_nodes"] = len(self.numa_nodes)

        if self.sockets:
            self._stats["average_utilization"] = sum(
                socket.average_utilization for socket in self.sockets.values()
            ) / len(self.sockets)
