...
class StoragePoolManager:
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(f"Storage pool initialized with {total_gb:.1f} GB across {self._stats['device_count']} devices")
    ...
    async def allocate_storage(
        ...
        if remaining_bytes > 0:
            self.logger.warning(f"Insufficient storage: need {size_bytes}, can provide {size_bytes - remaining_bytes}")
            return None
        ...
        self.logger.info(f"Allocated {allocation.allocated_bytes / (1024**3):.2f} GB to job {job_id}")
        return allocation
    ...
    async def _find_suitable_devices(
        ...
        suitable_devices.sort(key=lambda d: (
    ...
    async def create_dataset(
        ...
        self.logger.info(f"Created dataset {name}: {size_bytes / (1024**3):.2f} GB, {file_count} files")
        return dataset_id
    ...
    async def move_dataset(
        ...
        target_devices = [
            ...
            if device.storage_class == target_storage_class and device.free_bytes > dataset.size_bytes
        ]
                    ...
                    if partition.device == device.device_path:
                        try:
                            ...
                            if device.free_bytes < (device.total_bytes * 0.05):
                                device.state = StorageState.FULL
                            elif device.state == StorageState.FULL and device.free_bytes > (device.total_bytes * 0.10):
                                device.state = StorageState.AVAILABLE
    ...
    def _update_device_stats(self) -> None:
        ...
        self._stats["total_storage_bytes"] = sum(device.total_bytes for device in self.devices.values())
        self._stats["available_storage_bytes"] = sum(device.free_bytes for device in self.devices.values())
        
        # Count devices by storage class
        self._stats["nvme_devices"] = len([d for d in self.devices.values() if d.storage_class == StorageClass.NVME_SSD])
        self._stats["ssd_devices"] = len([d for d in self.devices.values() if d.storage_class == StorageClass.SATA_SSD])
        self._stats["hdd_devices"] = len([d for d in self.devices.values() if d.storage_class == StorageClass.HDD])
        self._stats["network_devices"] = len([d for d in self.devices.values() if d.storage_class == StorageClass.NETWORK])
        
        # Calculate average utilization
        if self.devices:
            self._stats["average_utilization"] = sum(device.utilization_percent for device in self.devices.values()) / len(self.devices)
            
            # Calculate total I/O metrics
            self._stats["total_iops"] = sum(device.read_iops + device.write_iops for device in self.devices.values())
            self._stats["total_bandwidth_mbps"] = sum(device.read_bandwidth_mbps + device.write_bandwidth_mbps for device in self.devices.values())
        ...
        for storage_class in StorageClass:
            ...
            if devices:
                storage_class_distribution[storage_class.value] = {
                    ...
                    "average_utilization": sum(d.utilization_percent for d in devices) / len(devices)
                }
