...
class MemoryPoolManager:
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(f"Memory pool initialized with {total_gb:.1f} GB across {self._stats['numa_nodes']} NUMA nodes")
    ...
    async def _discover_numa_memory_topology(self) -> None:
        ...
        try:
            ...
            if os.path.exists(numa_path):
                node_dirs = [d for d in os.listdir(numa_path) if d.startswith("node") and d[4:].isdigit()]
    ...
    async def _discover_swap_spaces(self) -> None:
        ...
        try:
            ...
            if swap.total > 0:
                ...
                self.logger.info(f"Discovered {len(self.swap_spaces)} swap spaces totaling {swap.total / (1024**3):.1f} GB")
    ...
    async def allocate_memory(
        ...
        total_available = sum(
            segment.size_bytes for segment in suitable_segments 
            if segment.state == MemoryState.AVAILABLE
        )
        
        if total_available < size_bytes:
            self.logger.warning(f"Insufficient memory: need {size_bytes}, available {total_available}")
            return None
    ...
    async def _find_suitable_segments(
        ...
        suitable_segments.sort(key=lambda s: (
    ...
    async def get_memory_status(self) -> Dict[str, Any]:
        ...
        self._stats["memory_pressure"] = max(0, memory_utilization - 80) / 20  # 0-1 scale, starts at 80% usage
        
        # Calculate swap pressure
        if self._stats["swap_total_bytes"] > 0:
            swap_utilization = (self._stats["swap_used_bytes"] / self._stats["swap_total_bytes"]) * 100
            self._stats["swap_pressure"] = max(0, swap_utilization - 50) / 50  # 0-1 scale, starts at 50% usage
        ...
        return {
            ...
            "swap_spaces": {
                device: swap.to_dict() 
                for device, swap in self.swap_spaces.items()
            },
    ...
    async def enable_swap_for_allocation(self, allocation_id: str, enable: bool = True) -> bool:
        ...
        self.logger.info(f"{'Enabled' if enable else 'Disabled'} swap for allocation {allocation_id}")
        return True
