    ...
    def _backfill_schedule(self, resource_availability: Dict[str, Any]) -> Optional[Job]:
        ...
        if high_priority_job:
            ...
            if self._can_job_run(high_priority_job, resource_availability):
                ...
                self._pending_jobs.insert(
                    0,
                    (high_priority_job.priority, high_priority_job.submitted_at, high_priority_job),
                )
                heapq.heapify(self._pending_jobs)
        ...
        if job_id in self._running_jobs:
            ...
            if job.started_at:
                ...
                self._stats["average_wait_time"] = (
                    self._stats["average_wait_time"] * (count - 1) + wait_time
                ) / count
                self._stats["average_turnaround_time"] = (
                    self._stats["average_turnaround_time"] * (count - 1) + turnaround_time
                ) / count
...
class JobScheduler:
    ...
    def __init__(self, max_concurrent_jobs: int = 1000):
        ...
        # Statistics
        self._global_stats = {"total_jobs": 0, "active_queues": 0, "total_throughput": 0.0}
