    ...
    async def allocate_resources(self, request: ResourceRequest) -> Optional[ResourceAllocation]:
        ...
        for resource, amount in required.items():
            if amount > 0 and self.available_resources.get(resource, 0) < amount:
                self.logger.warning(
                    f"Insufficient {resource}: requested {amount}, available {self.available_resources.get(resource, 0)}"
                )
                return None
...
class ResourceAllocator:
    ...
    async def allocate_resources(
        self, request: ResourceRequest, strategy: Optional[str] = None
        ...
        try:
            ...
            if allocation:
                ...
                self.logger.warning(
                    f"Failed to allocate resources for request {request.request_id}"
                )

            # Update timing statistics
            allocation_time = time.time() - start_time
            self._stats["average_allocation_time"] = (
                self._stats["average_allocation_time"] * (self._stats["total_requests"] - 1)
                + allocation_time
    ...
    async def _cost_optimized_allocation(
