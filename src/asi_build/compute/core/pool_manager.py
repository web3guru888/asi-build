...
class ComputePoolManager:
    ...
    async def submit_job(self, job_spec: Dict[str, Any]) -> str:
        ...
        if self.fair_share_manager:
            if not await self.fair_share_manager.check_quota(
                job.user_id, job.resource_requirements
            ):
    ...
    async def get_pool_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_jobs": len(
                [j for j in self._active_jobs.values() if j.status == JobStatus.RUNNING]
            ),
    ...
    async def _cleanup_job(self, job_id: str) -> None:
        ...
        if job_id in self._active_jobs:
            ...
            if self.fair_share_manager and job.status == JobStatus.COMPLETED:
                ...
                await self.fair_share_manager.update_usage(
                    job.user_id, job.resource_requirements, duration
                )
    ...
    async def _checkpoint_loop(self) -> None:
        ...
        while self._running and not self._shutdown_event.is_set():
            try:
                checkpoint_data = {
                    "active_jobs": {
                        job_id: job.to_dict() for job_id, job in self._active_jobs.items()
                    },
        ...
        while self._running and not self._shutdown_event.is_set():
            try:
                preemption_candidates = await self.preemption_manager.find_preemption_candidates(
                    list(self._active_jobs.values()), list(self._pending_jobs)
                )
    ...
    async def shutdown(self) -> None:
        ...
        running_jobs = [
            job for job in self._active_jobs.values() if job.status == JobStatus.RUNNING
        ]
