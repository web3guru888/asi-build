...
from integrations.slurm_integration import (
    SlurmIntegration,
    SlurmJobSpec
)
...
async def slurm_integration_demo():
    ...
    try:
        ...
        slurm_integration = SlurmIntegration(
            slurm_bin_path="/usr/bin"  # Adjust path as needed
        )
...
""")
            ...
            for _ in range(30):  # Monitor for ~5 minutes
                ...
                for job_id in job_ids:
                    ...
                    if job_status:
                        logger.info(f"Job {job_id}: {job_status['state']} on {job_status.get('nodes', 'unassigned')}")
            ...
            for job_type, filename in [("CPU", "cpu_job.out"), ("Memory", "memory_job.out"), ("GPU", "gpu_job.out")]:
        ...
        if "slurm command" in str(e).lower() and "not available" in str(e).lower():
            logger.warning("Slurm commands not available. Make sure Slurm is installed and in PATH.")
