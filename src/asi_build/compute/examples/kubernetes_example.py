...
async def kubernetes_integration_demo():
    ...
    try:
        ...
        cpu_pod_spec = KubernetesPodSpec(
            ...
            command=["python", "-c"],
            args=[
                "import time; [x**2 for x in range(1000000)]; time.sleep(30); print('CPU job completed')"
            ],
            labels={"job-type": "cpu-intensive", "demo": "kenny-compute-pool"},
        )

        cpu_job_id = "cpu-job-" + str(int(time.time()))
        resource_requirements = {"cpu_cores": 2, "memory": 4}  # GB

        success = await k8s_integration.submit_job(cpu_job_id, cpu_pod_spec, resource_requirements)
        ...
        memory_pod_spec = KubernetesPodSpec(
            ...
            command=["python", "-c"],
            args=[
                "data = [i for i in range(5000000)]; import time; time.sleep(20); print(f'Memory job completed with {len(data)} items')"
            ],
            labels={"job-type": "memory-intensive", "demo": "kenny-compute-pool"},
        )

        memory_job_id = "memory-job-" + str(int(time.time()))
        memory_requirements = {"cpu_cores": 1, "memory": 6}  # GB

        success = await k8s_integration.submit_job(
            memory_job_id, memory_pod_spec, memory_requirements
        )
        ...
        gpu_pod_spec = KubernetesPodSpec(
            ...
            command=["python", "-c"],
            args=[
                "import tensorflow as tf; print('GPU devices:', tf.config.list_physical_devices('GPU')); import time; time.sleep(25)"
            ],
            labels={"job-type": "gpu-computation", "demo": "kenny-compute-pool"},
        )

        gpu_job_id = "gpu-job-" + str(int(time.time()))
        gpu_requirements = {"cpu_cores": 1, "memory": 4, "gpu_count": 1}

        success = await k8s_integration.submit_job(gpu_job_id, gpu_pod_spec, gpu_requirements)
        ...
        for _ in range(20):  # Monitor for ~2 minutes
            ...
            for job_id in job_ids:
                ...
                if job_status:
                    logger.info(
                        f"Job {job_id}: {job_status['phase']} on node {job_status.get('node_name', 'unassigned')}"
                    )
...
async def kubernetes_manifest_demo():
    ...
    try:
        ...
        configmap_manifest = {
            "apiVersion": "v1",
            "kind": "ConfigMap",
            "metadata": {"name": "kenny-compute-pool-config", "namespace": "kenny-compute-pool"},
            "data": {
        ...
        custom_job_manifest = {
            ...
            "metadata": {
                ...
                "labels": {"app": "kenny-compute-pool", "job-type": "custom", "priority": "high"},
                "annotations": {
                    "kenny.compute-pool/job-id": f"custom-job-{int(time.time())}",
                    "kenny.compute-pool/user": "advanced-user",
                    "kenny.compute-pool/deadline": str(
                        int(time.time() + 300)
                    ),  # 5 minutes from now
                },
            },
