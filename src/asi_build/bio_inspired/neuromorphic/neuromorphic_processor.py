...
class NeuromorphicChip:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized neuromorphic chip {chip_id} with {num_cores} cores, "
    ...
    def get_chip_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "core_utilization": len([c for c in self.cores.values() if c.throughput_counter > 0])
            / self.num_cores,
        }
...
class NeuromorphicProcessor(BioCognitiveModule):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized neuromorphic processor with {num_chips} chips, "
    ...
    async def process(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output = {
            ...
            "chip_metrics": {
                chip_id: chip.get_chip_metrics() for chip_id, chip in self.chips.items()
            },
    ...
    def _distribute_spikes(
        self, spike_events: List[Dict[str, Any]]
    ...
    async def _apply_thermal_management(self, thermal_data: Dict[str, Any]):
        ...
        if current_temp > thermal_limit:
            ...
            logger.warning(
                f"Applied thermal throttling: reduced frequency by {(1-reduction_factor)*100:.1f}%"
            )
    ...
    def _calculate_efficiency_metrics(self) -> Dict[str, float]:
        ...
        total_energy = sum(
            chip.get_chip_metrics()["total_energy_consumption"] for chip in self.chips.values()
        )
        total_throughput = sum(
            chip.get_chip_metrics()["total_throughput"] for chip in self.chips.values()
        )

        metrics = {
            "energy_efficiency": total_throughput / (total_energy + 1e-6),  # spikes/joule
            "computational_efficiency": (
                np.mean(list(self.computation_efficiency)) if self.computation_efficiency else 0.0
            ),
            "thermal_efficiency": 1.0
            / (self.thermal_model.get_temperature() / 25.0),  # Relative to room temp
            "utilization_efficiency": np.mean(
                [chip.get_chip_metrics()["core_utilization"] for chip in self.chips.values()]
            ),
        }
