...
class Force:
    ...
    def is_active(self, current_time: float) -> bool:
        ...
        return current_time >= self.start_time and (
            self.duration == float("inf") or current_time <= self.start_time + self.duration
        )
...
class HolographicPhysicsManager(HolographicBase):
    ...
    async def _detect_collisions(self) -> List[CollisionEvent]:
        ...
        for body_a_id, body_b_id in collision_pairs:
            if body_a_id in self.physics_bodies and body_b_id in self.physics_bodies:
                ...
                if (
                    body_a.body_type == PhysicsBodyType.GHOST
                    or body_b.body_type == PhysicsBodyType.GHOST
                ):
                    continue
    ...
    async def _apply_forces(self, dt: float):
        ...
        for body in self.physics_bodies.values():
            ...
            if body.properties.mass > 0:
                ...
                body.angular_acceleration = total_torque * (
                    1.0 / body.properties.mass
                )  # Simplified
    ...
    async def _integrate_motion(self, dt: float):
        ...
        for body in self.physics_bodies.values():
            ...
            new_position = body.position + body.velocity * dt + body.acceleration * (0.5 * dt * dt)
    ...
    async def _resolve_collision(self, collision: CollisionEvent):
        ...
        if (
            body_a.body_type != PhysicsBodyType.DYNAMIC
            and body_b.body_type != PhysicsBodyType.DYNAMIC
        ):
            return
        ...
        mass_a = (
            body_a.properties.mass if body_a.body_type == PhysicsBodyType.DYNAMIC else float("inf")
        )
        mass_b = (
            body_b.properties.mass if body_b.body_type == PhysicsBodyType.DYNAMIC else float("inf")
        )
    ...
    async def _separate_bodies(
        self, body_a: PhysicsBody, body_b: PhysicsBody, collision: CollisionEvent
    ):
        """Separate overlapping bodies"""
    ...
    async def _apply_spring_constraint(self, constraint: Constraint, dt: float):
        ...
        if body_b:
            ...
            if current_length > 0:
                ...
                # Damping force
                relative_velocity = body_a.velocity - body_b.velocity
                damping_force = direction * (
                    -damping * SpatialMath.dot_product(relative_velocity, direction)
                )
                ...
                # Apply forces
                if body_a.body_type == PhysicsBodyType.DYNAMIC:
                    body_a.acceleration = body_a.acceleration + total_force * (
                        1.0 / body_a.properties.mass
                    )

                if body_b.body_type == PhysicsBodyType.DYNAMIC:
                    body_b.acceleration = body_b.acceleration - total_force * (
                        1.0 / body_b.properties.mass
                    )
        ...
        for body in self.physics_bodies.values():
            ...
            for pattern in self.interference_patterns.values():
                if body.body_id in pattern["bodies"]:
                    interference_factor *= (
                        1.0 - pattern["strength"] * body.properties.interference_sensitivity
                    )
            ...
            body.properties.coherence_stability += (
                ...
                * stability_change_rate
                * dt
            )

            # Clamp stability
            body.properties.coherence_stability = max(
                0.0, min(1.0, body.properties.coherence_stability)
            )
    ...
    async def _sleep_inactive_bodies(self):
        ...
        for body in self.physics_bodies.values():
            ...
            if (
                body.velocity.magnitude() < self.velocity_threshold
                and body.acceleration.magnitude() < self.velocity_threshold
            ):
    ...
    async def add_physics_body(
        self,
        body_id: str,
        body_type: PhysicsBodyType,
        position: Vector3D,
        properties: PhysicsProperties = None,
    ) -> bool:
    ...
    async def remove_physics_body(self, body_id: str) -> bool:
        ...
        if body_id in self.physics_bodies:
            ...
            self.active_forces = {
                fid: f for fid, f in self.active_forces.items() if body_id not in fid
            }
            self.constraints = {
                cid: c
    ...
    async def apply_force(
        self, body_id: str, force: Vector3D, point: Vector3D = None, duration: float = 0.1
    ) -> str:
