...
from .base import Quaternion, Transform3D, Vector3D
...
class SpatialMath:
    ...
    def sphere_in_frustum(
        center: Vector3D, radius: float, frustum_planes: List[np.ndarray]
    ) -> bool:
    ...
    def ray_sphere_intersection(
        ray_origin: Vector3D, ray_direction: Vector3D, sphere_center: Vector3D, sphere_radius: float
    ...
    def ray_plane_intersection(
        ray_origin: Vector3D, ray_direction: Vector3D, plane_point: Vector3D, plane_normal: Vector3D
    ) -> Optional[float]:
