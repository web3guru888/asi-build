...
class TelepresenceManager(HolographicBase):
    ...
    async def _handle_user_join(self, websocket, client_id: str, data: Dict[str, Any]):
        ...
        await self._broadcast_message(
            {"type": "user_joined", "user": asdict(user)}, exclude_client=client_id
        )
        ...
        for existing_user in self.remote_users.values():
            if existing_user.user_id != user_id:
                await websocket.send(
                    json.dumps({"type": "user_joined", "user": asdict(existing_user)})
                )

        # Send existing content
        for content in self.shared_content.values():
            await websocket.send(
                json.dumps({"type": "content_shared", "content": self._serialize_content(content)})
            )
    ...
    async def _handle_user_left(self, client_id: str):
        ...
        if user_to_remove:
            ...
            # Broadcast to other clients
            await self._broadcast_message({"type": "user_left", "user_id": user_to_remove})
    ...
    async def _handle_share_content(self, client_id: str, data: Dict[str, Any]):
        ...
        await self._broadcast_message(
            {"type": "content_shared", "content": self._serialize_content(content)}
        )
    ...
    async def _handle_start_stream(self, client_id: str, data: Dict[str, Any]):
        ...
        await self._broadcast_message(
            {"type": "stream_started", "stream": asdict(stream)}, exclude_client=client_id
        )
    ...
    async def _adjust_stream_quality(self, stream: SpatialStream):
        ...
        if stream.latency_ms > target_latency * 1.5:
            ...
            if stream.quality_level > 1:
                ...
                logger.info(
                    f"Reduced quality for stream {stream.stream_id} to level {stream.quality_level}"
                )
            ...
            if stream.quality_level < max(self.quality_levels.keys()):
                ...
                stream.bitrate = quality_settings["bitrate"]
                logger.info(
                    f"Increased quality for stream {stream.stream_id} to level {stream.quality_level}"
                )
    ...
    def _serialize_content(self, content: HolographicContent) -> Dict[str, Any]:
        ...
        return {
            ...
            "orientation": {
                "position": {
                    "x": content.orientation.position.x,
                    "y": content.orientation.position.y,
                    "z": content.orientation.position.z,
                },
                "rotation": {
                    "w": content.orientation.rotation.w,
                    "x": content.orientation.rotation.x,
                    "y": content.orientation.rotation.y,
                    "z": content.orientation.rotation.z,
                },
                "scale": {
                    "x": content.orientation.scale.x,
                    "y": content.orientation.scale.y,
                    "z": content.orientation.scale.z,
                },
            },
    ...
    async def update_local_position(self, position: Vector3D, orientation: Transform3D = None):
        ...
        if orientation:
            message["orientation"] = {
                ...
                "rotation": {
                    "w": orientation.rotation.w,
                    "x": orientation.rotation.x,
                    "y": orientation.rotation.y,
                    "z": orientation.rotation.z,
                },
                "scale": {
                    "x": orientation.scale.x,
                    "y": orientation.scale.y,
                    "z": orientation.scale.z,
                },
            }

        await self._broadcast_message(message)

    async def share_holographic_content(
        self,
        content_type: HologramType,
        data: bytes,
        position: Vector3D,
    ...
    async def start_holographic_stream(
        self, stream_type: str = "hologram", quality_level: int = 2
    ) -> str:
        ...
        # Broadcast stream start
        await self._broadcast_message({"type": "stream_started", "stream": asdict(stream)})
    ...
    async def stop_streaming(self):
        ...
        await self._broadcast_message({"type": "streams_stopped", "user_id": self.local_user_id})
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_latency": (
                np.mean([s.latency_ms for s in active_streams.values()]) if active_streams else 0
            ),
