...
class VolumeRenderer:
    ...
    def add_voxel(
        self, position: Vector3D, color: Tuple[float, float, float, float], intensity: float = 1.0
    ):
    ...
    def render_slice(
        self, slice_depth: float, view_matrix: np.ndarray, projection_matrix: np.ndarray
    ) -> np.ndarray:
    ...
    def render_volume(
        self, view_matrix: np.ndarray, projection_matrix: np.ndarray, camera_position: Vector3D
    ) -> np.ndarray:
...
class VolumetricDisplay(HolographicBase):
    ...
    async def create_layer(
        self, layer_id: str, depth: float, resolution: Optional[Tuple[int, int]] = None
    ) -> VolumetricLayer:
        ...
        layer = VolumetricLayer(layer_id=layer_id, depth=depth, resolution=resolution, voxels=[])
    ...
    async def add_voxel(
        ...
        voxel = VoxelData(position=position, color=color, intensity=intensity, size=size)
        ...
        for layer_id in self.layer_order:
            ...
            for voxel in layer.voxels:
                if voxel.active:
                    ...
                    self.volume_renderer.add_voxel(voxel.position, tuple(color), voxel.intensity)

    async def animate_voxel(
        self, layer_id: str, voxel_index: int, target_position: Vector3D, duration: float
    ):
