...
class NeuromorphicChip(ABC):
    ...
    def get_power_consumption(self) -> Dict[str, float]:
        ...
        return {
            ...
            "power_efficiency": (
                self.total_spikes / (static_power + dynamic_power) if dynamic_power > 0 else 0.0
            ),
        }
    ...
    def _add_to_routing_table(
        self, src_core: int, src_neuron: int, dst_core: int, dst_neuron: int
    ) -> None:
    ...
    def _update_power_model(self, dt: float) -> None:
        ...
        for core in self.cores.values():
            ...
            spike_power = core.active_neurons * self.config.dynamic_power_per_spike / dt

            # Dynamic power from synaptic operations
            synapse_power = core.num_synapses * self.config.dynamic_power_per_synapse / dt
    ...
    def _initialize_neuron_hardware(
        self, core_id: int, neuron_id: int, neuron_params: Dict[str, Any]
    ) -> None:
    ...
    def _update_performance_metrics(self, step_time: float) -> None:
        ...
        if real_time_factor < 1.0:
            self.logger.warning(
                f"Simulation running slower than real-time: {real_time_factor:.2f}x"
            )
