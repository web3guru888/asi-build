...
class TemporalDynamics:
    ...
    def get_synchrony_measure(self, neuron_ids: List[int], time_window: float = 50.0e-3) -> float:
        ...
        for neuron_id in neuron_ids:
            if neuron_id in self.spike_history:
                ...
                recent_spikes[neuron_id] = [
                    t for t in self.spike_history[neuron_id] if t >= cutoff_time
                ]
            ...
            for neuron_b in neuron_ids[i + 1 :]:
                if neuron_a in recent_spikes and neuron_b in recent_spikes:
                    sync = self._calculate_pairwise_synchrony(
                        recent_spikes[neuron_a], recent_spikes[neuron_b]
                    )
    ...
    def get_firing_rate(self, neuron_id: int, time_window: float = 1.0) -> float:
        ...
        cutoff_time = self.current_time - time_window
        recent_spikes = [t for t in self.spike_history[neuron_id] if t >= cutoff_time]
    ...
    def get_isi_statistics(self, neuron_id: int) -> Dict[str, float]:
        ...
        return {"mean": mean_isi, "std": std_isi, "cv": cv_isi}
    ...
    def _update_oscillations(self, dt: float) -> None:
        ...
        self.global_oscillation_state.phase += (
            2 * np.pi * self.global_oscillation_state.frequency * dt
        )
    ...
    def _detect_burst_pattern(self, template: TemporalPattern) -> Optional[Dict[str, Any]]:
        ...
        for neuron_id, spike_times in self.spike_history.items():
            ...
            recent_spikes = [t for t in spike_times if t >= self.current_time - burst_window]
    ...
    def _calculate_pairwise_synchrony(self, spikes_a: List[float], spikes_b: List[float]) -> float:
    ...
    def _cleanup_old_data(self) -> None:
        ...
        for pattern_id in list(self.detected_patterns.keys()):
            old_detections = [
                d
                for d in self.detected_patterns[pattern_id]
