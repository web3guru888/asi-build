...
class SpikeMonitor:
    ...
    def record_spike(
        ...
        spike_record = SpikeRecord(
            neuron_id=neuron_id, timestamp=timestamp, amplitude=amplitude, metadata=metadata or {}
        )
    ...
    def get_firing_rate(self, neuron_id: int, time_window: float = 1.0) -> float:
        ...
        recent_spikes = [t for t in self.spike_times_by_neuron[neuron_id] if t >= cutoff_time]
    ...
    def get_synchrony_index(self, neuron_ids: List[int] = None, time_window: float = 1.0) -> float:
        ...
        for neuron_id in neuron_ids:
            if neuron_id in self.spike_times_by_neuron:
                recent_spikes = [
                    t for t in self.spike_times_by_neuron[neuron_id] if t >= cutoff_time
                ]
    ...
    def get_burst_analysis(self, neuron_id: int) -> Dict[str, Any]:
                ...
                bursts.append(
                    {
                        ...
                        "inter_burst_interval": (
                            bursts[-1]["start_time"] - burst_start if bursts else 0.0
                        ),
                    }
                )
        ...
        burst_rate = (
            len(bursts) / (self.current_time - self.start_time)
            if self.current_time > self.start_time
            else 0.0
        )

        return {"bursts": bursts, "burst_rate": burst_rate, "total_bursts": len(bursts)}
    ...
    def get_monitor_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage_mb": (
                sum(self.memory_usage) / len(self.memory_usage) if self.memory_usage else 0.0
            ),
            "recording_duration": self.current_time,
            "records_per_second": (
                len(self.spike_records) / self.current_time if self.current_time > 0 else 0.0
            ),
        }
    ...
    def _update_realtime_data(self, spike_record: SpikeRecord) -> None:
        ...
        if len(self.raster_data[spike_record.neuron_id]) > max_raster_points:
            self.raster_data[spike_record.neuron_id] = self.raster_data[spike_record.neuron_id][
                -max_raster_points:
            ]

    def _calculate_synchrony(self, spike_trains: List[List[float]], time_window: float) -> float:
    ...
    def _export_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if (
                    self.is_recording
                    and current_time - self.last_export_time > self.export_interval
                ):
