...
class NeuronBase(NeuralBase):
    ...
    def get_firing_rate(self, time_window: float = 1.0) -> float:
        ...
        recent_spikes = [t for t in self.state['spike_times'] 
...
class SynapseBase(NeuralBase):
    ...
    def __init__(self, synapse_id: int, pre_neuron: NeuronBase, 
                 post_neuron: NeuronBase, **kwargs):
        """Initialize synapse."""
    ...
    def receive_spike(self, spike_event: SpikeEvent) -> None:
        ...
        delayed_spike = SpikeEvent(
            ...
            amplitude=spike_event.amplitude
        )
    ...
    def _process_delayed_spikes(self) -> None:
        ...
        while (self.state['delayed_spikes'] and 
               self.state['delayed_spikes'][0].timestamp <= self.current_time):
