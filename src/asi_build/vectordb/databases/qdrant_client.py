...
class QdrantClient:
        ...
        try:
            ...
            self.client.create_collection(
                ...
                vectors_config=VectorParams(
                    size=vector_size, distance=distance_enum, hnsw_config=hnsw_config
                ),
        ...
        try:
            ...
            for i in range(0, len(points), batch_size):
                ...
                for point_id, vector, payload in batch:
                    point_struct = PointStruct(id=point_id, vector=vector, payload=payload or {})
    ...
    def upsert_single(
        ...
        return self.upsert_points(
            collection_name=collection_name, points=[(point_id, vector, payload or {})]
        )
        ...
        for field, condition in filter_dict.items():
            if isinstance(condition, dict):
                ...
                if "gte" in condition or "lte" in condition:
                    ...
                    field_condition = FieldCondition(key=field, range=range_condition)
                ...
                # Exact match
                field_condition = FieldCondition(key=field, match=MatchValue(value=condition))
                conditions.append(field_condition)
        ...
        try:
            if replace:
                result = self.client.overwrite_payload(
                    collection_name=collection_name, points=point_ids, payload=payload, wait=wait
                )
            else:
    ...
    def scroll_points(
        ...
        with_payload: bool = True,
        with_vectors: bool = False,
    ) -> Tuple[List[QdrantSearchResult], Optional[Union[str, int]]]:
        """
    ...
    def count_points(
        ...
        try:
            ...
            result = self.client.count(
                collection_name=collection_name, count_filter=search_filter, exact=exact
            )
                ...
                "default_collection_stats": (
                    {
                        ...
                        "status": (
                            collection_stats.collection_status if collection_stats else "not_found"
                        ),
                    }
                    if collection_stats
                    else None
