...
from kenny_vectordb import UnifiedVectorDB, VectorDBConfig
    ...
    json_docs = [
        ...
        },
        {
            ...
            "data": {
                ...
                "metadata": {"product_count": 250, "suppliers": 45, "last_updated": "2023-08-15"},
            },
        },
    ]
...
def demonstrate_single_document_batch(indexing_api: IndexingAPI):
    ...
    topics = [
        (
            ...
            "technology",
            "Machine learning is being applied across industries from healthcare to finance. Predictive models help doctors diagnose diseases, while algorithmic trading systems manage financial portfolios automatically.",
        ),
        (
            "Sustainable Agriculture",
            "environment",
            "Modern agriculture is adopting sustainable practices to reduce environmental impact. Precision farming, vertical growing, and organic methods are helping feed growing populations while protecting ecosystems.",
        ),
        (
            "Quantum Sensors",
            "science",
            "Quantum sensors use quantum mechanical effects to achieve unprecedented precision in measurements. These devices are revolutionizing fields like navigation, medical imaging, and fundamental physics research.",
        ),
        (
            "Digital Health Platforms",
            "health",
            "Digital health platforms are transforming patient care through telemedicine, remote monitoring, and AI-assisted diagnosis. Patients can now access medical expertise from anywhere in the world.",
        ),
        (
            "Smart Cities Infrastructure",
            "urban",
            "Smart cities use IoT sensors, data analytics, and automation to optimize urban services. Traffic management, energy distribution, and waste collection are becoming more efficient and responsive.",
        ),
    ]
    ...
    job = indexing_api.index_documents_batch(
        documents=documents, batch_size=2, parallel=True  # Small batch size for demonstration
    )
    ...
    while job.status in ["pending", "processing"]:
        ...
        if current_job:
            print(
                f"📊 Progress: {current_job.progress:.1%} ({current_job.processed_count}/{current_job.total_count})"
            )
...
def demonstrate_directory_processing(indexing_api: IndexingAPI):
    ...
    job = indexing_api.index_directory(
        directory_path=data_dir, file_patterns=["*.txt", "*.json"], recursive=True
    )
...
def demonstrate_large_batch_processing(indexing_api: IndexingAPI):
    ...
    for batch_size in batch_sizes:
        ...
        job = indexing_api.index_documents_batch(
            documents=test_docs, batch_size=batch_size, parallel=True
        )
...
def demonstrate_job_management(indexing_api: IndexingAPI):
    ...
    for job in all_jobs[-10:]:  # Show last 10 jobs
        ...
        print(
            f"  {job.job_id[:12]}... | {job.status:10} | {job.processed_count:2}/{job.total_count:2} docs | {duration:.2f}s"
        )
