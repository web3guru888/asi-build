...
def demonstrate_basic_search(retrieval_api: RetrievalAPI):
    ...
    for query in queries:
        ...
        simple_query = RetrievalQuery(query=query, search_mode="semantic", top_k=3)
...
def demonstrate_hybrid_search(vector_db: UnifiedVectorDB):
    ...
    for query, vector_weight, keyword_weight in queries:
        ...
        results = vector_db.hybrid_search(
            query=query, top_k=3, vector_weight=vector_weight, keyword_weight=keyword_weight
        )
...
def demonstrate_recommendations(retrieval_api: RetrievalAPI, vector_db: UnifiedVectorDB):
    ...
    if user_history:
        ...
        recommendations = retrieval_api.get_recommendations(
            user_history=user_history, top_k=5, diversity_factor=0.3
        )
...
def demonstrate_query_optimization(retrieval_api: RetrievalAPI):
    ...
    partial_queries = ["artificial intell", "quantum comput", "gene edit"]
...
def demonstrate_analytics(
    retrieval_api: RetrievalAPI, indexing_api: IndexingAPI, vector_db: UnifiedVectorDB
):
