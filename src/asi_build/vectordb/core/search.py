...
class SearchQuery:
    ...
    rerank: bool = True
    database_weights: Dict[str, float] = field(
        default_factory=lambda: {"pinecone": 1.0, "weaviate": 1.0, "qdrant": 1.0}
    )
...
class ResultReranker:
    ...
    def __init__(self):
        self.boost_factors = {"recency": 0.1, "popularity": 0.15, "relevance": 0.75}

    def rerank_results(
        self, results: List[SearchResult], query: str, rerank_top_k: int = 100
    ) -> List[SearchResult]:
        """
...
class SemanticSearchEngine(BaseSearchEngine):
    ...
    def search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        if self.pinecone_client and query.database_weights.get("pinecone", 0) > 0:
            ...
            weighted_results = self._apply_database_weight(
                pinecone_results, query.database_weights.get("pinecone", 1.0)
            )
        ...
        if self.weaviate_client and query.database_weights.get("weaviate", 0) > 0:
            ...
            weighted_results = self._apply_database_weight(
                weaviate_results, query.database_weights.get("weaviate", 1.0)
            )
            all_results.extend(weighted_results)
        ...
        if self.qdrant_client and query.database_weights.get("qdrant", 0) > 0:
            ...
            weighted_results = self._apply_database_weight(
                qdrant_results, query.database_weights.get("qdrant", 1.0)
            )
            all_results.extend(weighted_results)
        ...
        if query.rerank and len(all_results) > 1:
            ...
            all_results = self.reranker.rerank_results(
                all_results, query.query, self.config.rerank_top_k
            )
    ...
    def _apply_database_weight(
        self, results: List[SearchResult], weight: float
    ) -> List[SearchResult]:
    ...
    def _deduplicate_results(
        ...
        for result in results:
            ...
            for existing_content in seen_content:
                if (
                    self._content_similarity(result.content, existing_content)
