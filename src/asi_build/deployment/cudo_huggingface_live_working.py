...
# Configuration
CUDO_API_KEY = os.environ.get("CUDO_API_KEY", "")
...
class CudoHuggingFaceDeployer:
    ...
    async def __aenter__(self):
        self.session = aiohttp.ClientSession(
            headers=self.headers, timeout=aiohttp.ClientTimeout(total=300)
        )
    ...
    def get_optimal_datacenter(self, machine_type: str = None) -> Optional[Dict[str, Any]]:
        ...
        if machine_type:
            for dc in self.data_centers:
                ...
                for machine in self.machine_types:
                    if (
                        machine.get("machineType") == machine_type
                        and machine.get("dataCenterId") == dc_id
                    ):
                        return dc
...
"""
    ...
    async def deploy_model(
        self, model_name: str, config: Dict[str, Any] = None, dry_run: bool = True
    ) -> Dict[str, Any]:
        ...
        if not gpu_machine:
            ...
            cpu_machines = sorted(
                self.cpu_machines, key=lambda x: float(x.get("vcpuPriceHr", {}).get("value", 999))
            )
            if not cpu_machines:
        ...
        try:
            async with self.session.post(
                f"{self.base_url}/projects/{self.project_id}/vms", json=vm_config
            ) as response:
                ...
                if response.status in [200, 201, 202]:
                    ...
                    return {
                        ...
                        "api_endpoint": (
                            f"http://{deployment_result.get('public_ip', 'pending')}:8000"
                            if deployment_result.get("public_ip")
                            else None
                        ),
                        "health_check": (
                            f"http://{deployment_result.get('public_ip', 'pending')}:8000/health"
                            if deployment_result.get("public_ip")
                            else None
                        ),
                        "estimated_cost_per_hour": estimated_cost,
            ...
            return {"success": False, "error": f"Deployment exception: {str(e)}"}
                ...
                async with self.session.get(
                    ...
                    if response.status == 200:
                        ...
                        if status == "running" and public_ip:
                                ...
                                async with aiohttp.ClientSession(
                                    ...
                                    async with test_session.get(
                                        ...
                                        if health_response.status == 200:
                                            ...
                                            print(
                                                f"   ⚠️  API not ready yet (status: {health_response.status})"
                                            )
                            except:
                            ...
                            print(
                                f'        -d \'{{"prompt": "Kenny\\\'s agent army", "max_length": 50}}\''
                            )

                            return {"status": "running", "public_ip": public_ip, "vm_id": vm_id}
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    hf_deployments = [
                        ...
                        if vm.get("metadata", {})
                        .get("deployer", "")
                        .startswith("kenny-universal-deployer")
                    ]
                    ...
                    for vm in hf_deployments:
                        ...
                        deployment = {
                            ...
                            "api_endpoint": (
                                f"http://{public_ip}:8000/generate" if public_ip else None
                            ),
                        }
