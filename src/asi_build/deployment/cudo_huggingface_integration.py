...
class CUDOHuggingFaceIntegration:
    ...
    async def provision_gpu_for_model(
        self, model_name: str, model_size_gb: float, task: str, **kwargs
    ) -> CUDOInstance:
        """
        ...
        gpu_type = self._select_gpu_for_model(
            model_size_gb, task, kwargs.get("budget_priority", True)
        )
    ...
    async def _create_cudo_instance(self, config: Dict[str, Any]) -> Optional[CUDOInstance]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/v1/instances", headers=headers, json=config, timeout=30
            )
    ...
    async def scale_to_zero(self, instance_id: str) -> bool:
        ...
        if self.api_key:
            ...
            response = requests.delete(
                f"{self.base_url}/v1/instances/{instance_id}", headers=headers
            )
...
async def test_cudo_huggingface_integration():
    ...
    for model_name, size_gb, task in test_models[:2]:  # Test first 2
        ...
        instance = await integration.provision_gpu_for_model(
            model_name=model_name, model_size_gb=size_gb, task=task, budget_priority=True
        )
