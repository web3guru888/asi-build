    ...
    async def get_graph_stats(self) -> str:
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                node_types = [
                    {"labels": record["labels"], "count": record["count"]} for record in result
                ]
                ...
                stats = {
                    ...
                    "agent_army": {"total_agents": 1405, "teams": 48, "status": "operational"},
                }
    ...
    async def get_node_summary(self) -> str:
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                return json.dumps(
                    {"node_types": node_summary, "total_types": len(node_summary)}, indent=2
                )
    ...
    async def get_relationship_summary(self) -> str:
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                return json.dumps(
                    {"relationship_types": rel_summary, "total_types": len(rel_summary)}, indent=2
                )
    ...
    async def execute_cypher_query(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                response = {"query": query, "results": records, "count": len(records)}
        ...
        try:
            ...
            with self.driver.session() as session:
                if analysis_type == "centrality":
                    ...
                    result = session.run(query)
                    analysis = [
                        {"name": r["name"], "labels": r["labels"], "connections": r["connections"]}
                        for r in result
                    ]
                ...
                response = {"analysis_type": analysis_type, "results": analysis}
