...
class ContextualLearner:
        ...
        try:
            ...
            learning_insights = {
                ...
                "recommendations": self._generate_learning_recommendations(
                    patterns_learned, adaptations
                ),
            }

            self.logger.debug(
                f"Learning completed in {learning_time:.3f}s with {len(patterns_learned)} patterns"
            )
            ...
            return {"error": str(e), "learning_time": time.time() - start_time}
    ...
    async def _extract_query_patterns(self, learning_event: LearningEvent) -> List[LearningPattern]:
        ...
        length_category = (
            "short" if query_length < 20 else "medium" if query_length < 100 else "long"
        )
    ...
    async def _extract_performance_patterns(
        self, learning_event: LearningEvent
        ...
        quality_time_ratio = learning_event.result_quality / max(
            learning_event.processing_time, 1.0
        )
        efficiency_category = (
            "high"
            if quality_time_ratio > 0.05
            else "medium" if quality_time_ratio > 0.02 else "low"
        )
    ...
    async def _extract_quality_patterns(
        self, learning_event: LearningEvent
    ) -> List[LearningPattern]:
        ...
        patterns.append(
            LearningPattern(
                ...
                pattern_data={
                    ...
                    "source_count": learning_event.system_metrics.get("source_count", 0),
                    "query_complexity": learning_event.system_metrics.get(
        ...
        if source_count > 0:
            ...
            source_efficiency = (
                "high"
                if quality_per_source > 0.1
                else "medium" if quality_per_source > 0.05 else "low"
            )
    ...
    async def _update_learning_models(
        self, learning_event: LearningEvent, patterns: List[LearningPattern]
    ):
    ...
    def _evaluate_adaptation_condition(
        self, rule: AdaptationRule, learning_event: LearningEvent
    ) -> bool:
        ...
        if rule.rule_type == "performance":
            ...
            query_key = self._get_query_key(learning_event.query)
            current_metrics = {"query_frequency": self.query_patterns.get(query_key, 0)}
        else:
            current_metrics = {
    ...
    def _update_learning_metrics(
        ...
        if learning_event.result_quality > 0.7:
            ...
            self.learning_metrics["learning_accuracy"] = (
                current_accuracy * 0.9 + learning_event.result_quality * 0.1
            )
        ...
        if adaptations:
            ...
            self.learning_metrics["adaptation_effectiveness"] = (
                current_effectiveness * 0.9 + avg_effectiveness * 0.1
            )

    def _generate_learning_recommendations(
        self, patterns: List[LearningPattern], adaptations: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if patterns:
            ...
            if high_frequency_patterns:
                recommendations.append(
                    f"Found {len(high_frequency_patterns)} recurring patterns - consider optimization"
                )
        ...
        if self.learning_metrics.get("learning_accuracy", 0.0) < 0.6:
            recommendations.append(
                "Learning accuracy is low - consider adjusting learning parameters"
            )
    ...
    def _update_performance_model(self, pattern: LearningPattern):
        ...
        if "processing_time" in data:
            model["avg_time"] = (
                model["avg_time"] * (model["count"] - 1) + data["processing_time"]
            ) / model["count"]

        if "quality" in data:
            model["avg_quality"] = (
                model["avg_quality"] * (model["count"] - 1) + data["quality"]
            ) / model["count"]
    ...
    def _update_quality_model(self, pattern: LearningPattern):
        ...
        if "quality_score" in data:
            model["avg_quality"] = (
                model["avg_quality"] * (model["count"] - 1) + data["quality_score"]
            ) / model["count"]
    ...
    def _load_learning_data(self):
        ...
        if persistence_file and os.path.exists(persistence_file):
            try:
                ...
                self.logger.info(
                    f"Loaded {len(self.learning_patterns)} learning patterns from persistence"
                )
            except Exception as e:
    ...
    def _get_performance_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fast_queries_ratio": (
                len([t for t in recent_times if t < 5.0]) / len(recent_times)
                if recent_times
                else 0.0
            ),
        }
    ...
    def _get_quality_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_quality_ratio": (
                len([q for q in recent_qualities if q > 0.8]) / len(recent_qualities)
                if recent_qualities
                else 0.0
            ),
        }
