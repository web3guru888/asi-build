...
class QualityController:
    ...
    async def validate_knowledge(self, synthesis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            final_result = {
                "validation_passed": aggregated_results["overall_score"]
                >= self.config.get("min_validation_score", 0.6),
            ...
            adjusted_result = self._apply_confidence_adjustments(
                synthesis_result, confidence_adjustments
            )
            ...
            validated_synthesis = {
                **adjusted_result,
                ...
                "confidence": adjusted_result.get("confidence", 0.5)
                * aggregated_results["overall_score"],
            }

            self.logger.info(
                f"Knowledge validation completed in {validation_time:.2f}s with score {aggregated_results['overall_score']:.2f}"
            )
            ...
            return {
                **synthesis_result,
                "validation": {
                    ...
                    "validation_metadata": {"processing_time": validation_time, "success": False},
                },
            }
    ...
    async def _execute_validation_strategies(
        self, knowledge_components: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_validation_rule(
        self, rule: ValidationRule, components: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _check_source_consistency(self, components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "issues": issues,
            "metadata": {"source_count": len(sources), "source_diversity": len(set(sources))},
        }
    ...
    async def _check_factual_accuracy(self, components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "issues": issues,
            "metadata": {"findings_analyzed": len(key_findings), "confidence_score": confidence},
        }
    ...
    async def _check_temporal_consistency(self, components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = synthesis_data.get("summary", "")
        temporal_references = re.findall(
            r"\b(today|yesterday|tomorrow|recent|current|latest)\b", summary.lower()
        )
    ...
    def _generate_validation_recommendations(
        self, validation_results: Dict[str, Any], quality_metrics: Dict[str, float]
    ) -> List[str]:
        ...
        if avg_score < 0.6:
            recommendations.append(
                "Overall validation score is low - consider gathering additional sources"
            )
        ...
        for rule in self.validation_rules:
            ...
            if not result.get("passed", False):
                if rule.rule_type == "consistency":
                    recommendations.append(
                        "Improve source consistency by cross-referencing information"
                    )
                elif rule.rule_type == "completeness":
                    recommendations.append(
                        "Gather more comprehensive information to improve completeness"
                    )
                    ...
                    recommendations.append(
                        "Verify factual accuracy through additional fact-checking"
                    )
        ...
        if total_issues > len(validation_results) * 2:
            recommendations.append(
                "Multiple validation issues detected - consider regenerating synthesis"
            )

        return recommendations[:5]  # Limit recommendations

    def _apply_confidence_adjustments(
        self, synthesis_result: Dict[str, Any], adjustments: Dict[str, float]
    ) -> Dict[str, Any]:
        """Apply confidence adjustments to synthesis result."""
            ...
            if "synthesis" in adjusted_result:
                ...
                if hasattr(synthesis, "confidence_score"):
                    synthesis.confidence_score = max(
                        0.1, synthesis.confidence_score + accuracy_penalty
                    )
                elif isinstance(synthesis, dict) and "confidence_score" in synthesis:
                    synthesis["confidence_score"] = max(
                        0.1, synthesis["confidence_score"] + accuracy_penalty
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        avg_validation_time = (
            self.validation_time / self.total_validations if self.total_validations > 0 else 0.0
        )

        # Calculate success rate from history
        recent_history = self.validation_history[-100:]  # Last 100 validations
        success_rate = (
            sum(1 for v in recent_history if v["passed"]) / len(recent_history)
            if recent_history
            else 0.0
        )
