...
class IntelligentSearch:
    ...
    async def search_comprehensive(self, query, aggregated_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Search completed in {search_time:.2f}s with {len(filtered_results)} results"
            )
            return result_dict
            ...
            return {"error": str(e), "search_time": search_time, "search_metadata": {"error": True}}
    ...
    async def _execute_parallel_search(
        self, search_query: SearchQuery, corpus: List[Dict[str, Any]]
    ...
    async def _execute_sequential_search(
        ...
        for strategy_name in enabled_strategies:
            try:
                strategy_results = await self._execute_search_strategy(
                    strategy_name, search_query, corpus
                )
    ...
    async def _execute_search_strategy(
        self, strategy_name: str, search_query: SearchQuery, corpus: List[Dict[str, Any]]
    ) -> List[SearchResult]:
    ...
    def _expand_query_with_context(self, search_query: SearchQuery) -> Dict[str, float]:
        ...
        if "domain" in context:
            ...
            if domain == "kenny":
                expanded.update({"automation": 0.8, "screen": 0.7, "workflow": 0.6})
            elif domain == "technical":
                expanded.update({"system": 0.6, "code": 0.5, "api": 0.5})
    ...
    def _merge_search_results(
        self, strategy_results: Dict[str, List[SearchResult]]
    ) -> List[SearchResult]:
    ...
    def _rank_search_results(
        ...
        for result in results:
            ...
            score = (
                ...
                + self._get_source_authority_boost(result.source)
                * weights["source_authority_weight"]
            )
    ...
    def _cache_search_result(self, cache_key: str, response: SearchResponse):
        ...
        self.semantic_cache[cache_key] = {"data": cacheable_data, "timestamp": time.time()}

        # Limit cache size
        if len(self.semantic_cache) > self.config.get("max_semantic_cache_size", 10000):
            # Remove oldest entries
            oldest_keys = sorted(
                self.semantic_cache.keys(), key=lambda k: self.semantic_cache[k]["timestamp"]
            )[:1000]
