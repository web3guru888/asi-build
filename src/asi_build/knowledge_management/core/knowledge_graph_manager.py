...
class KnowledgeGraphManager:
    ...
    async def _build_graph_from_concepts(
    ...
    async def _create_relationships_from_context(
        self, concepts: List[str], aggregated_info: Dict[str, Any]
    ):
    ...
    async def _add_domain_relationships(self, concepts: List[str]):
        ...
        for concept1 in concepts:
            for concept2 in concepts:
                if concept1 != concept2:
                    for (rule_concept1, rule_concept2), (
                        rel_type,
                        strength,
                    ) in domain_rules.items():
                        if (
                            rule_concept1 in concept1.lower() and rule_concept2 in concept2.lower()
                        ) or (
    ...
    async def _generate_graph_insights(
        self, analysis_result: GraphAnalysisResult, patterns: Dict[str, Any]
    ) -> List[str]:
        ...
        if analysis_result.nodes_analyzed > 20:
            ...
            insights.append(
                "Limited knowledge connections - may need broader information gathering"
            )
        ...
        if patterns.get("strong_relationships"):
            insights.append(
                f"Found {len(patterns['strong_relationships'])} high-confidence relationships"
            )
        ...
        if total_rels > 0:
            ...
            if conf_ratio > 0.7:
                insights.append(
                    "High confidence in relationship analysis - reliable knowledge base"
                )
            elif conf_ratio < 0.3:
    ...
    def _generate_analysis_insights(
        self, key_concepts: List[str], relationship_patterns: Dict[str, int]
    ) -> List[str]:
        ...
        if relationship_patterns:
            ...
            insights.append(
                f"Dominant relationship type: {most_common[0]} ({most_common[1]} instances)"
            )
