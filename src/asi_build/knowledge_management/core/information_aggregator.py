...
class InformationAggregator:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "web_sources": {"user_agent": "Kenny-Omniscience/1.0", "timeout": 10.0},
            "kenny_integration": {
                "mem0_enabled": True,
    ...
    async def _fetch_from_source(
        self, source: InformationSource, query, semaphore
    ) -> Dict[str, Any]:
    ...
    def _process_aggregation_results(
        self, results: List[Any], sources: List[InformationSource], query
    ) -> Dict[str, Any]:
        ...
        processed_data = {
            ...
            "aggregated_content": {},
            "confidence_scores": {},
            "metadata": {"aggregation_timestamp": time.time(), "processing_version": "1.0.0"},
        }
        ...
        for i, result in enumerate(results):
            ...
            processed_data["confidence_scores"][source.name] = result.get("data", {}).get(
                "confidence", 0.5
            )

        # Calculate overall confidence
        if processed_data["confidence_scores"]:
            avg_confidence = sum(processed_data["confidence_scores"].values()) / len(
        ...
        self.logger.info(
            f"Processed {processed_data['sources_successful']}/{len(sources)} sources successfully"
        )
    ...
    def _cache_result(self, cache_key: str, data: Dict[str, Any]):
        ...
        self.cache["memory_cache"][cache_key] = {"data": data, "timestamp": time.time()}
    ...
    def get_aggregation_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (
            self.successful_aggregations / self.total_queries if self.total_queries > 0 else 0.0
        )
