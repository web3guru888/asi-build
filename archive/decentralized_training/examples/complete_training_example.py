...
from systems.decentralized_training.monitoring.dashboard import MetricsCollector, DashboardApp
from systems.decentralized_training.core.error_handling import ErrorHandler, get_global_error_handler
...
class TrainingNode:
    ...
    def train_local_model(self, epochs: int = 1) -> Dict[str, Any]:
        ...
        logger.info(f"Node {self.node_id} completed local training - Loss: {avg_loss:.4f}, Accuracy: {accuracy:.3f}")
...
class DecentralizedTrainingCoordinator:
    ...
    async def setup_system(self):
        ...
        try:
            ...
            self.error_handler.handle_error(
                e, 'system_setup', 
                from systems.decentralized_training.core.error_handling import ErrorSeverity, ErrorCategory
                ErrorSeverity.CRITICAL, ErrorCategory.PROTOCOL
            )
            raise
    ...
    async def distribute_dataset(self, dataset: TensorDataset, node_ids: List[str], 
                               strategy: str = 'noniid'):
        ...
        try:
            ...
            for node_id, node in self.training_nodes.items():
                try:
                    ...
                    self.error_handler.handle_error(
                        e, f'local_training_{node_id}', 
                        ErrorSeverity.MEDIUM, ErrorCategory.COMPUTATION
                    )
                    continue
            ...
            for node_id, gradients in node_gradients.items():
                try:
                    ...
                    self.error_handler.handle_error(
                        e, f'compression_{node_id}', 
                        ErrorSeverity.LOW, ErrorCategory.COMPUTATION
                    )
            ...
            aggregation_result = await self.byzantine_defense.adaptive_aggregate(decompressed_gradients)
            
            if not aggregation_result.aggregated_gradients:
                logger.error("Aggregation failed")
                return False
            
            logger.info(f"Aggregation completed - Method: {aggregation_result.aggregation_method}, "
            ...
            if self.checkpoint_manager and self.training_rounds % 10 == 0:
                try:
                    ...
                    self.error_handler.handle_error(
                        e, 'checkpoint_save', 
                        ErrorSeverity.LOW, ErrorCategory.STORAGE
                    )
            ...
            if self.reward_manager:
                try:
                    ...
                    for node_id, metrics in node_metrics.items():
                        from systems.decentralized_training.blockchain.agix_rewards import ComputeContribution
                    ...
                    logger.info(f"Distributed {reward_result['total_agix_distributed']} AGIX to {reward_result['successful_distributions']} nodes")
                    
                except Exception as e:
                    self.error_handler.handle_error(
                        e, 'reward_distribution', 
                        ErrorSeverity.MEDIUM, ErrorCategory.PROTOCOL
                    )
            ...
            logger.info(f"Round {self.training_rounds} completed in {round_time:.2f}s - "
            ...
            self.error_handler.handle_error(
                e, 'training_round', 
                ErrorSeverity.HIGH, ErrorCategory.PROTOCOL
            )
            return False
...
async def main():
    ...
    try:
        ...
        if config['monitoring']['enable_dashboard']:
            ...
            logger.info(f"Dashboard available at http://localhost:{config['monitoring']['dashboard_port']}")
