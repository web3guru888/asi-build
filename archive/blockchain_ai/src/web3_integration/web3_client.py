...
class Web3Client:
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.info(f"Connected to {self.network.name} with {len(self.w3_instances)} RPC endpoints")
    ...
    async def _test_connection(self):
        ...
        async def _test(w3):
            ...
            if chain_id != self.network.chain_id:
                raise Web3Error(f"Chain ID mismatch: expected {self.network.chain_id}, got {chain_id}")
