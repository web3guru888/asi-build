...
class PinataService(PinningService):
    ...
    async def list_pins(
        ...
        params = {
            'pageLimit': min(limit, 1000),  # Pinata max is 1000
            'pageOffset': offset
        }
                ...
                if response.status == 200:
                    ...
                    for row in result['rows']:
                        pins.append(PinStatus(
                            ...
                            status=row['status'],
                            created=datetime.fromisoformat(row['date_pinned'].replace('Z', '+00:00')),
                            size=row.get('size'),
                            service=self.name
                        ))
...
class InfuraService(PinningService):
    ...
    async def __aenter__(self):
        ...
        credentials = base64.b64encode(
            f"{self.project_id}:{self.project_secret}".encode()
        ).decode()
...
class MultiPinningManager:
    ...
    async def pin_with_redundancy(
        ...
        successful_pins = [
            status for status in all_results.values()
