...
class QuantizationCompressor(ModelCompressor):
    ...
    def decompress(self, compressed_data: Dict[str, Any], metadata: Dict[str, Any]) -> List[np.ndarray]:
    ...
    def _uniform_quantization(self, weight: np.ndarray) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        quantized = np.clip(quantized, 0, 2**self.bits - 1).astype(np.uint8 if self.bits <= 8 else np.uint16)
...
class CompressionManager:
    ...
    def decompress_weights(self, compressed_data: Any, metadata: Dict[str, Any]) -> List[np.ndarray]:
