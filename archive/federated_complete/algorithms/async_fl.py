    ...
    def add_update(self, update: Dict[str, Any]) -> bool:
        ...
        with self.lock:
            ...
            if staleness > self.staleness_threshold:
                self.logger.warning(f"Rejecting stale update from client {update.get('client_id')} "
            ...
            self.logger.debug(f"Added update from client {update.get('client_id')} "
...
class AsyncFedAvg(BaseAggregator):
    ...
    def __init__(self, aggregator_id: str = "async_fedavg", config: Dict[str, Any] = None):
        ...
        self.logger.info(f"AsyncFedAvg aggregator initialized with min_updates={self.min_updates_for_aggregation}")
    
    def start_async_aggregation(self, global_model_update_callback: Callable[[List[np.ndarray]], None]):
        ...
        self.aggregation_thread = threading.Thread(
            target=self._async_aggregation_loop,
            args=(global_model_update_callback,),
            daemon=True
        )
    ...
    def _async_aggregation_loop(self, global_model_update_callback: Callable[[List[np.ndarray]], None]):
            ...
            try:
                ...
                if len(updates) >= self.min_updates_for_aggregation:
                    ...
                    if result and "aggregated_weights" in result:
                        ...
                        self.logger.info(f"Async aggregation completed (version {self.global_version}, "
    ...
    def _compute_staleness_aware_weights(self, client_updates: List[Dict[str, Any]],
                                       staleness_values: List[int]) -> List[float]:
...
class AsyncFederatedServer(FederatedServer):
    ...
    def handle_client_update_async(self, client_id: str, client_update: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                if self.server:
                    ...
                    if response["status"] == "accepted":
                        ...
                        self.logger.debug(f"Update rejected from client {client.client_id}: {response.get('reason')}")
        ...
        while True:
            try:
                ...
                if self.server:
                    ...
                    evaluation_result = {
                        "timestamp": time.time(),
                        "global_version": getattr(self.server.async_aggregator, 'global_version', 0),
                    ...
                    self.logger.info(f"Async evaluation completed (version {evaluation_result.get('global_version')})")
        ...
        while True:
            try:
                ...
                if self.server and len(self.evaluation_history) >= 2:
                    ...
                    version_changes = [
                        eval_result.get("global_version", 0) 
                    ...
                    if len(set(version_changes)) <= 1:
                        ...
                        version_rate = (version_changes[-1] - version_changes[0]) / len(version_changes)
                        convergence_result = {
                            "timestamp": time.time(),
    ...
    def get_async_training_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "current_global_version": (
                getattr(self.server.async_aggregator, 'global_version', 0)
                if self.server else 0
            ),
            "server_status": (
                self.server.get_server_status_async()
                if self.server else {}
            )
        }
