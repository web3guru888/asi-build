...
from agi_economics.core.types import (
    TokenType, ResourceType, AgentType, ValueCategory,
    Agent, Resource, ServiceRequest, ValueMeasurement,
    ReputationEvent
)

# Engine imports
from agi_economics.engines.token_economics import TokenEconomicsEngine
from agi_economics.engines.bonding_curves import BondingCurveEngine, BondingCurveConfig, CurveType
...
from agi_economics.systems.reputation_system import (
    ReputationSystem, ReputationValidator, ReputationDimension
)

# Simulation imports
from agi_economics.simulation.marketplace_dynamics import (
    MarketplaceDynamics, MarketOrder, OrderType
)
...
class AGIEconomicsDemo:
    ...
    async def demo_value_alignment(self):
        ...
        feedback_result = self.value_engine.provide_human_feedback("provider_ai_001", {
            ...
            'overall_rating': 4.4,
            'suggestions': ['Continue current approach', 'Increase transparency in decision-making']
        })
        ...
        if profile:
            ...
            for value, data in sorted_alignments[:3]:
                print(f"    {value}: {data['alignment_score']:.3f} (confidence: {data['confidence']:.3f})")
    ...
    async def demo_reputation_system(self):
        ...
        transitive_trust = self.reputation_system.calculate_transitive_trust(
            source_id="consumer_002",
            target_id="provider_ai_001"
        )
        ...
        if reputation:
            ...
            for dimension, score in sorted(dimensions.items(), key=lambda x: x[1], reverse=True)[:4]:
    ...
    async def show_results(self):
        ...
        if 'marketplace' in self.results:
            ...
            if market_data:
                print(f"  Current Price: {market_data.price} AGIX")
                print(f"  Market State: {market_data.state.value}")
                print(f"  Supply/Demand Ratio: {float(market_data.supply / market_data.demand):.2f}")
