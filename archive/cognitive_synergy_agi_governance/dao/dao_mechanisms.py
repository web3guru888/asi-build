...
class QuadraticVoting:
    ...
    def cast_quadratic_vote(self, voter_id: str, proposal_id: str, 
                           vote_intensity: int, vote_direction: str) -> Dict[str, Any]:
...
class ReputationSystem:
    ...
    def update_reputation(self, stakeholder_id: str, action: str, 
    ...
    def decay_reputation(self, days_passed: int = 1):
        ...
        for stakeholder_id in self.reputation_scores:
            ...
            if abs(old_score - new_score) > 0.1:  # Only log significant changes
                logger.info(f"Reputation decay for {stakeholder_id}: {old_score:.2f} -> {new_score:.2f}")
    
    def _calculate_reputation_change(self, action: str, impact: float, 
...
class DAOGovernance:
    ...
    def create_dao_proposal(self, proposal_data: Dict[str, Any]) -> Optional[DAOProposal]:
        ...
        try:
            ...
            self._lock_tokens(proposer_id, self.min_proposal_tokens, 
                            reason=f"proposal_{proposal_data['title']}")
    ...
    def cast_dao_vote(self, proposal_id: str, voter_id: str, 
                     vote_intensity: int, vote_direction: str) -> bool:
        ...
        try:
            ...
            self._lock_tokens(voter_id, vote_data['token_cost'], 
                            reason=f"vote_{proposal_id}")
            ...
            logger.info(f"DAO vote cast: {voter_id} voted {vote_direction} "
        ...
        try:
            ...
            if success:
                ...
                self.reputation_system.update_reputation(
                    proposal.proposer_id, 'good_proposal', 
                    results['approval_rate'], {'quality_score': 1.0})
    ...
    def distribute_tokens(self, stakeholder_id: str, token_type: TokenType, 
    ...
    def _lock_tokens(self, stakeholder_id: str, amount: float, reason: str):
        ...
        current_time = datetime.utcnow()
        lock_until = current_time + timedelta(days=self.voting_period_days + self.execution_delay_days)
    ...
    def _calculate_dao_results(self, proposal: DAOProposal) -> Dict[str, Any]:
        ...
        total_possible_votes = sum(self._get_available_tokens(sid, TokenType.GOVERNANCE) 
                                 for sid in self.tokens.keys())
        ...
        approved = (participation_rate >= self.quorum_threshold and 
                   approval_rate > 0.5)
    ...
    def get_dao_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_proposals': sum(1 for p in self.proposals.values() 
                                  if datetime.utcnow() <= p.voting_ends_at),
            ...
            'average_reputation': sum(self.reputation_system.reputation_scores.values()) / 
                                len(self.reputation_system.reputation_scores) if self.reputation_system.reputation_scores else 0
        }