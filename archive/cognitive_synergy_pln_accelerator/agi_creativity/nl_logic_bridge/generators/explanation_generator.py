...
class ExplanationGenerator:
            ...
            if rule_type in self.templates:
                ...
                if template:
                    ...
                    return Explanation(
                        ...
                        counterexamples=[],
                        confidence=0.8,
                        metadata={"method": "template_based", "rule_type": rule_type.value if rule_type else "unknown"}
                    )
        ...
        try:
            ...
            if components and "type" in components:
                ...
                if rule_type == PLNRuleType.INHERITANCE and premise and conclusion:
                    examples.append(f"Example: If we know that '{premise}' includes dogs, and dogs are animals, then dogs would also be '{conclusion}'.")
                
                elif rule_type == PLNRuleType.IMPLICATION and premise and conclusion:
                    examples.append(f"Example: If '{premise}' is true (like it's raining), then '{conclusion}' might be true (like the ground gets wet).")
                
                elif rule_type == PLNRuleType.CAUSAL and premise and conclusion:
                    examples.append(f"Example: '{premise}' directly causes '{conclusion}' - like how pressing a light switch causes the light to turn on.")
                
                elif rule_type == PLNRuleType.SIMILARITY and premise and conclusion:
                    examples.append(f"Example: '{premise}' and '{conclusion}' share many characteristics, like how cats and dogs are both pets.")
    ...
    async def _explain_pln_expression(self, expression: str, context: Optional[Dict[str, Any]]) -> List[str]:
        ...
        if components:
            ...
            strength_desc = "very strong" if strength > 0.8 else "strong" if strength > 0.6 else "moderate" if strength > 0.4 else "weak"
            confidence_desc = "very confident" if confidence > 0.8 else "confident" if confidence > 0.6 else "somewhat confident" if confidence > 0.4 else "uncertain"
            
            explanations.append(f"The relationship is {strength_desc} (strength: {strength:.2f}) and I am {confidence_desc} about this (confidence: {confidence:.2f}).")
        
        return explanations
    
    async def _explain_fol_expression(self, expression: str, context: Optional[Dict[str, Any]]) -> List[str]:
    ...
    async def _explain_temporal_logic(self, expression: str, context: Optional[Dict[str, Any]]) -> List[str]:
        """Explain temporal logic expressions."""
        explanations = []
        
        # Simple temporal patterns
        if "◊" in expression:  # Eventually
            explanations.append("This means that at some point in the future, the condition will be true.")
            ...
            explanations.append("This describes a situation that continues until another condition becomes true.")
        
        return explanations
    
    async def _explain_modal_logic(self, expression: str, context: Optional[Dict[str, Any]]) -> List[str]:
        """Explain modal logic expressions."""
        explanations = []
        
        if "◊" in expression:  # Possibly
            explanations.append("This expresses that something is possibly true - it could happen in some scenarios.")
        elif "□" in expression:  # Necessarily
            explanations.append("This expresses that something is necessarily true - it must be true in all scenarios.")
    ...
    async def _explain_generic_logic(
        ...
        if "→" in expression or "->" in expression:
            explanations.append("This is a conditional statement: if the first part is true, then the second part follows.")
        elif "∧" in expression:
            explanations.append("This is a conjunction: both conditions must be true simultaneously.")
        elif "∨" in expression:
            explanations.append("This is a disjunction: at least one of the conditions must be true.")
        elif "¬" in expression:
            explanations.append("This is a negation: the condition is false or does not hold.")
        else:
            explanations.append(f"This is a {formalism.value} expression that establishes logical relationships.")
