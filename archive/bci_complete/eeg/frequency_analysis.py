...
from scipy.signal import welch, coherence, hilbert, periodogram
...
class FrequencyAnalyzer:
    ...
    def _compute_spectral_features(self, data: np.ndarray, sampling_rate: float) -> Dict[str, Any]:
        ...
        for ch_idx, channel in enumerate(self.channels):
            ...
            # Spectral skewness
            spectral_skewness = np.sum(((f - spectral_centroid) ** 3) * psd) / (np.sum(psd) * spectral_spread ** 3)
            features[f'{channel}_spectral_skewness'] = float(spectral_skewness)
            
            # Spectral kurtosis
            spectral_kurtosis = np.sum(((f - spectral_centroid) ** 4) * psd) / (np.sum(psd) * spectral_spread ** 4)
    ...
    def _compute_coherence_features(self, data: np.ndarray, sampling_rate: float) -> Dict[str, float]:
            ...
            for i in range(n_channels):
                for j in range(i + 1, n_channels):
                    ...
                    if np.any(band_mask):
                        ...
                        features[f'{ch1_name}_{ch2_name}_{band_name}_coherence'] = float(avg_coherence)
    ...
    def _compute_cross_frequency_coupling(self, data: np.ndarray, sampling_rate: float) -> Dict[str, float]:
        ...
        for ch_idx, channel in enumerate(self.channels):
            ...
            for low_band, high_band in coupling_pairs:
                ...
                low_filtered = self._bandpass_filter(ch_data, low_freq_range[0], low_freq_range[1], sampling_rate)
                low_analytic = hilbert(low_filtered)
                low_phase = np.angle(low_analytic)
                
                # Extract amplitude from high frequency band
                high_filtered = self._bandpass_filter(ch_data, high_freq_range[0], high_freq_range[1], sampling_rate)
    ...
    def compute_time_frequency_analysis(self, data: np.ndarray, sampling_rate: float) -> Dict[str, Any]:
            ...
            for ch_idx, channel in enumerate(self.channels):
                ...
                for band_name, (low_freq, high_freq) in self.frequency_bands.items():
                    ...
                    if np.any(band_mask):
                        ...
                        features[f'{channel}_{band_name}_power_mean'] = float(np.mean(band_power_time))
                        features[f'{channel}_{band_name}_power_std'] = float(np.std(band_power_time))
                        features[f'{channel}_{band_name}_power_max'] = float(np.max(band_power_time))
                        features[f'{channel}_{band_name}_power_min'] = float(np.min(band_power_time))
    ...
    def compute_frequency_stability(self, data: np.ndarray, sampling_rate: float, window_length: float = 2.0) -> Dict[str, float]:
        ...
        for ch_idx, channel in enumerate(self.channels):
            ...
            if len(peak_frequencies) > 1:
                ...
                features[f'{channel}_peak_freq_stability'] = float(1.0 / (1.0 + np.std(peak_frequencies)))
