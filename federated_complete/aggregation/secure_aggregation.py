...
class SecureAggregator(BaseAggregator):
    ...
    def generate_client_masks(self, client_ids: List[str], 
                            weight_shapes: List[Tuple]) -> Dict[str, List[np.ndarray]]:
    ...
    def mask_weights(self, weights: List[np.ndarray], 
    ...
    def unmask_aggregated_weights(self, masked_weights: List[np.ndarray],
                                total_masks: List[np.ndarray]) -> List[np.ndarray]:
    ...
    def _secure_aggregate_with_secret_sharing(self, client_weights_list: List[List[np.ndarray]],
    ...
    def _secure_aggregate_with_homomorphic(self, client_weights_list: List[List[np.ndarray]],
                                         data_sizes: List[int]) -> List[np.ndarray]:
    ...
    def _secure_aggregate_with_masking(self, client_weights_list: List[List[np.ndarray]],
                                     data_sizes: List[int], 
                                     client_ids: List[str]) -> List[np.ndarray]:
        ...
        for layer_idx in range(num_layers):
            ...
            for i, weights in enumerate(client_weights_list):
                ...
                masked_contribution = self.mask_weights(
                    [weight_contribution], 
                    [client_masks[client_id][layer_idx]]
                )[0]
        ...
        # Remove total mask
        aggregated_weights = self.unmask_aggregated_weights(
            aggregated_masked_weights, total_masks
        )
