...
class MetaLearningStrategy(ABC):
    ...
    @abstractmethod
    def meta_update(self, global_weights: List[np.ndarray],
                   client_gradients: List[List[np.ndarray]],
                   meta_lr: float) -> List[np.ndarray]:
        """Perform meta-update given client gradients."""
        pass
    
    @abstractmethod
    def inner_loop_update(self, initial_weights: List[np.ndarray],
                         task_data: tf.data.Dataset,
                         inner_lr: float,
                         inner_steps: int) -> Tuple[List[np.ndarray], List[np.ndarray]]:
...
class FedMAMLAggregator(BaseAggregator):
    ...
    def _compute_meta_learning_stats(self, client_updates: List[Dict[str, Any]],
        ...
        if meta_metrics:
            ...
            adaptation_losses = [m.get("adaptation_loss", 0) for m in meta_metrics if "adaptation_loss" in m]
            inner_loop_steps = [m.get("inner_steps_taken", 0) for m in meta_metrics if "inner_steps_taken" in m]
            task_similarities = [m.get("task_similarity", 0) for m in meta_metrics if "task_similarity" in m]
        ...
        if self.meta_learning_history:
            ...
            if adaptation_losses:
                meta_summary["recent_adaptation_loss"] = {
                    "mean": np.mean(adaptation_losses),
                    "trend": "decreasing" if len(adaptation_losses) > 1 and adaptation_losses[-1] < adaptation_losses[0] else "stable"
                }
...
class FedReptile:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.aggregator = FedMAMLAggregator("fedreptile_main", reptile_config.get("aggregator", reptile_config))
...
class FederatedMetaLearning:
    ...
    def get_comprehensive_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "meta_learner_summary": self.meta_learner.get_training_summary() if hasattr(self.meta_learner, 'get_training_summary') else {}
        }