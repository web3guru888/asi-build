...
class CARLASimulationSuite:
    ...
    def run_full_test_suite(self, save_results: bool = True) -> Dict[str, Any]:
        ...
        for scenario_type, count in self.scenarios.items():
            ...
            # Log results
            logger.info(f"  Passed: {results['passed']:,}/{count:,} ({results['passed']/count*100:.2f}%)")
            logger.info(f"  Collision-free: {results['metrics']['collision_free_rate']*100:.2f}%")
            logger.info(f"  Traffic compliance: {results['metrics']['traffic_compliance_rate']*100:.2f}%")
        ...
        overall_results["compliance"] = {
            "collision_free": metrics["collision_free_rate"] >= self.success_criteria["collision_free"],
            "traffic_compliance": metrics["traffic_compliance_rate"] >= self.success_criteria["traffic_compliance"],
            "comfort": metrics["avg_comfort_score"] >= self.success_criteria["comfort_score"],
            "intervention": metrics["intervention_rate"] <= self.success_criteria["intervention_rate"],
