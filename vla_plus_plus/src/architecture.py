...
class VisionModule(nn.Module):
    ...
    def __init__(self, config: VLAConfig):
        ...
        self.vision_encoder = nn.Linear(
            config.vision_fpn_channels * 7 * 7,
...
class LanguageModule(nn.Module):
    ...
    def __init__(self, config: VLAConfig):
        ...
        self.transformer = nn.TransformerEncoder(
            encoder_layer,
            num_layers=config.language_num_layers
        )
        
        # Language encoder for fusion
        self.language_encoder = nn.Linear(
            config.language_hidden_size,
            config.fusion_hidden_size
        )
...
class ActionModule(nn.Module):
    ...
    def forward(
        ...
        waypoint_queries = torch.zeros(
            batch_size, num_waypoints, self.config.action_hidden_size,
            device=action_features.device
        )
        
        # Decode trajectory
        trajectory_features = self.trajectory_decoder(
            tgt=waypoint_queries,
            memory=action_features
        )
...
class VLAPlusPlus(nn.Module):
    ...
    def forward(
        ...
        action_output = self.action_module(
            vision_output["vision_embedding"],
            language_output["language_embedding"]
        )
        
        # Safety check
        safety_score = self.safety_checker(
            action_output["trajectory_features"].mean(dim=1)
        )
